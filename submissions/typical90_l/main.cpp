...
static void k_f();
static void k_g();
using k_bb=type_(k_ar)(*)(type_(k_ar), int64_t);
using k_cc=bool(*)(type_(k_ar), int64_t, int64_t);
static void k_i();
static void k_j();
...
static int64_t k_ak();
static int64_t k_an();
static type_(k_ar) k_as(type_(k_ar) k_fp /*me*/, int64_t k_fq /*n*/);
static int64_t k_at(type_(k_ar) k_ft /*me*/, int64_t k_fu /*x*/);
using k_fv=int64_t(*)(type_(k_ar), int64_t);
static bool k_au(type_(k_ar) k_fw /*me*/, int64_t k_fx /*x*/, int64_t k_fy /*y*/);
static bool k_av(type_(k_ar) k_fz /*me*/, int64_t k_ga /*x*/, int64_t k_gb /*y*/);
static int64_t k_aw(type_(k_ar) k_gl /*me*/, int64_t k_gm /*x*/);
static void k_ax(k_ar* k_gq);
static type_(k_ar) k_ay(type_(k_ar) k_gr);
static type_(Array_<uint8_t>) k_az(type_(k_ar) k_gt);
static type_(k_ar) k_ba(type_(k_ar) k_gw, type_(Array_<uint8_t>) k_gx, int64_t* k_gy);
static void k_cl(type_(Array_<char16_t>) k_ha /*str*/);
template<typename T_>static T_ k_dx(T_ k_hb /*me_*/, type_(Array_<int64_t>) k_hc /*type*/, T_ k_hd /*n*/);
template<typename T_>static T_ k_dy(T_ k_he /*me_*/, type_(Array_<int64_t>) k_hf /*type*/, int64_t k_hg /*n*/);
template<typename T_>static T_ k_ek(T_ k_hj /*me_*/, type_(Array_<int64_t>) k_hk /*type*/, int64_t k_hl /*n*/);
static type_(Array_<char16_t>) k_fj();
static int64_t k_fm(type_(Array_<char16_t>) k_ii /*me_*/, bool* k_ij /*success*/);
template<typename T_>static T_ k_fs(T_ k_ik /*me_*/, type_(Array_<int64_t>) k_il /*type*/, int64_t k_im /*len*/);
static char16_t k_hu();
template<typename T_>static T_ k_ih(T_ k_jd /*me_*/, type_(Array_<int64_t>) k_je /*type*/, int64_t k_jf /*start*/, int64_t k_jg /*len*/);
static char16_t k_it();
...
k_ar();
virtual ~k_ar();
type_(Array_<int64_t>) k_fr; // parents
};
...
int64_t k_am; // h
int64_t k_ao; // w
type_(Array_<type_(Array_<bool>)>) k_ap; // m
type_(k_ar) k_aq; // uni
int64_t k_bc; // q
int64_t k_be;
int64_t k_bg; // t
...
int64_t k_bk; // c
int64_t k_bl; // u
type_(Array_<int64_t>) k_bm; // dx
type_(Array_<int64_t>) k_bn; // dy
int64_t k_br;
int64_t k_bt; // c2
int64_t k_bu; // r2
int64_t k_cb; // v
...
(k_am) = ((k_an)());
(k_ao) = ((k_an)());
(k_ap) = (newArrays_<type_(Array_<type_(Array_<bool>)>)>((k_am), (k_ao)));
(k_aq) = ((reinterpret_cast<k_bb>(classTable_[(new_(k_ar)())->Y + 8]))((new_(k_ar)()), ((k_am) * (k_ao))));
(k_bc) = ((k_an)());
for(k_be = (1LL), k_bf = (k_bc); k_be <= k_bf; k_be += (1LL)){
(k_bg) = ((k_an)());
if((k_bg) == (1LL)){
(k_bj) = (((k_an)()) - (1LL));
(k_bk) = (((k_an)()) - (1LL));
(((k_ap)->At(k_bj))->At(k_bk)) = (true);
(k_bl) = (((k_bj) * (k_ao)) + (k_bk));
(k_bm) = (new_(Array_<int64_t>)((1LL), (0LL), (-1LL), (0LL)));
(k_bn) = (new_(Array_<int64_t>)((0LL), (1LL), (0LL), (-1LL)));
for(k_br = (0LL), k_bs = (3LL); k_br <= k_bs; k_br += (1LL)){
(k_bt) = ((k_bk) + ((k_bm)->At(k_br)));
(k_bu) = ((k_bj) + ((k_bn)->At(k_br)));
if(((k_bt) < (0LL)) || ((k_ao) <= (k_bt))){
continue;
}
if(((k_bu) < (0LL)) || ((k_am) <= (k_bu))){
continue;
}
if(!(((k_ap)->At(k_bu))->At(k_bt))){
continue;
}
(k_cb) = (((k_bu) * (k_ao)) + (k_bt));
(reinterpret_cast<k_cc>(classTable_[(k_aq)->Y + 11]))((k_aq), (k_bl), (k_cb));
}
}
else{
(k_ce) = (((k_an)()) - (1LL));
...
(k_ch) = (((k_an)()) - (1LL));
(k_ci) = (false);
if((((k_ap)->At(k_ce))->At(k_cf)) && (((k_ap)->At(k_cg))->At(k_ch))){
(k_ci) = ((reinterpret_cast<k_cc>(classTable_[(k_aq)->Y + 10]))((k_aq), (((k_ce) * (k_ao)) + (k_cf)), (((k_cg) * (k_ao)) + (k_ch))));
}
(k_cl)(((k_ci) ? (newArrayBin_<char16_t>(4,"\x59\x00\x65\x00\x73\x00\x0A\x00")) : (newArrayBin_<char16_t>(3,"\x4E\x00\x6F\x00\x0A\x00"))));
}
}
}
// _finVars
...
// init
static type_(k_ar) k_as(type_(k_ar) k_fp /*me*/, int64_t k_fq /*n*/){
((k_fp)->k_fr) = (k_fs<type_(Array_<int64_t>)>((new_(Array_<int64_t>)((-1LL))), (nullptr), (k_fq)));
return k_fp;
}
// size
static int64_t k_at(type_(k_ar) k_ft /*me*/, int64_t k_fu /*x*/){
return -(((k_ft)->k_fr)->At((reinterpret_cast<k_fv>(classTable_[(k_ft)->Y + 12]))((k_ft), (k_fu))));
}
// same
static bool k_au(type_(k_ar) k_fw /*me*/, int64_t k_fx /*x*/, int64_t k_fy /*y*/){
return ((reinterpret_cast<k_fv>(classTable_[(k_fw)->Y + 12]))((k_fw), (k_fx))) == ((reinterpret_cast<k_fv>(classTable_[(k_fw)->Y + 12]))((k_fw), (k_fy)));
}
// unite
static bool k_av(type_(k_ar) k_fz /*me*/, int64_t k_ga /*x*/, int64_t k_gb /*y*/){
type_(Array_<int64_t>) k_gh;
type_(Array_<int64_t>) k_gk;
(k_ga) = ((reinterpret_cast<k_fv>(classTable_[(k_fz)->Y + 12]))((k_fz), (k_ga)));
(k_gb) = ((reinterpret_cast<k_fv>(classTable_[(k_fz)->Y + 12]))((k_fz), (k_gb)));
if((k_ga) == (k_gb)){
return false;
}
if(((reinterpret_cast<k_fv>(classTable_[(k_fz)->Y + 9]))((k_fz), (k_ga))) < ((reinterpret_cast<k_fv>(classTable_[(k_fz)->Y + 9]))((k_fz), (k_gb)))){
(k_gh) = ((k_fz)->k_fr);
((k_gh)->At(k_gb)) = (((k_gh)->At(k_gb)) + (((k_fz)->k_fr)->At(k_ga)));
(((k_fz)->k_fr)->At(k_ga)) = (k_gb);
}
else{
(k_gk) = ((k_fz)->k_fr);
((k_gk)->At(k_ga)) = (((k_gk)->At(k_ga)) + (((k_fz)->k_fr)->At(k_gb)));
(((k_fz)->k_fr)->At(k_gb)) = (k_ga);
}
return true;
}
// root
static int64_t k_aw(type_(k_ar) k_gl /*me*/, int64_t k_gm /*x*/){
if((((k_gl)->k_fr)->At(k_gm)) < (0LL)){
return k_gm;
}
else{
(((k_gl)->k_fr)->At(k_gm)) = ((reinterpret_cast<k_fv>(classTable_[(k_gl)->Y + 12]))((k_gl), (((k_gl)->k_fr)->At(k_gm))));
return ((k_gl)->k_fr)->At(k_gm);
}
}
// _dtor
static void k_ax(k_ar* k_gq){
...
type_(k_ar) k_gs;
(k_gs) = (new_(k_ar)());
((k_gs)->k_fr) = (copy_<type_(Array_<int64_t>)>()((k_gr)->k_fr));
return as_<k_ar>(classTable_, (k_gs), 21);
}
// _toBin
static type_(Array_<uint8_t>) k_az(type_(k_ar) k_gt){
type_(Array_<uint8_t>) k_gu;
(k_gu) = (newArrays_<type_(Array_<uint8_t>)>((0LL)));
(k_gu) = ((k_gu)->Cat(toBin_<type_(Array_<int64_t>)>()(((k_gt)->k_fr))));
return k_gu;
}
// _fromBin
static type_(k_ar) k_ba(type_(k_ar) k_gw, type_(Array_<uint8_t>) k_gx, int64_t* k_gy){
type_(k_ar) k_gz;
(k_gz) = (new_(k_ar)());
((k_gz)->k_fr) = (fromBin_<type_(Array_<int64_t>)>()((k_gx), (*k_gy)));
return k_gz;
}
// print
static void k_cl(type_(Array_<char16_t>) k_ha /*str*/){
...
#if defined(_WIN32)
fputws(reinterpret_cast<const wchar_t*>(k_ha->B),stdout);
#else
std::u16string s_=k_ha->B;
...
return k_ih<type_(Array_<char16_t>)>((k_ho), (nullptr), (0LL), (k_hp));
}
// _toInt
static int64_t k_fm(type_(Array_<char16_t>) k_ii /*me_*/, bool* k_ij /*success*/){
std::u16string u_=k_ii->B;
...
while(true){
(k_is) = ((k_it)());
for(k_ix = (0LL), k_iy = (((k_al)->Len()) - (1LL)); k_ix <= k_iy; k_ix += (1LL)){
if(((k_is) == ((k_al)->At(k_ix))) || (((k_is) == (u'\u000D')) && (((k_al)->At(k_ix)) == (u'\u000A')))){
return u'\u0000';
}
}
if((k_is) == (u'\u000D')){
...
classTable_[23] = reinterpret_cast<int64_t>(k_ax);
classTable_[24] = reinterpret_cast<int64_t>(k_r);
classTable_[25] = reinterpret_cast<int64_t>(k_ay);
classTable_[26] = reinterpret_cast<int64_t>(k_az);
classTable_[27] = reinterpret_cast<int64_t>(k_ba);
...
classTable_[33] = reinterpret_cast<int64_t>(k_aw);
init_();
k_main_();
