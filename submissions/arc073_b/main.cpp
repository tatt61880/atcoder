...
inline void dump(){ cerr << " \n"; }
template<class T> inline void dump(const T& t){ debug_print(t); cerr << " \n"; }
template<class Head,class... Tail> inline void dump(const Head& head, const Tail&... tail){ debug_print(head); cerr<<" "; dump(tail...); }
#define debug(...) do{ setColor(COL_WHITE, COL_DARK_BLUE); cerr<<"[L."<<__LINE__<<"] "<<#__VA_ARGS__<<": "; setColor(); dump(__VA_ARGS__); }while(0)
#else
#define dump(...)
#define debug(...)
#endif
...
int main(){
  ...
  // n個の物を入れる前提。
  rep(n, 1, N + 1){
    ll room = W - w0 * n; // あまったスペース
    if(room < 0) continue;
    chmin(room, 4 * n); // 多めに見積もっても 4 * n 以上のスペースは必要ない。
    debug(n, room);
    // dp[j][k] スペースをj使って物をk個入れた時の最大価値V
    vvi dp = Vector(room + 1, n + 1);
    rep(j, room + 1){
      rep(k, n + 1){
        dp[j][k] = -1;
      }
    ...
    rep(i, N){
      rrep(j, room + 1){
        rrep(k, n){
          ...
          if(j + w[i] > room) continue;
          chmax(dp[j + w[i]][k + 1], dp[j][k] + V[i]);
        }
      }
    }
    rep(j, room + 1){
