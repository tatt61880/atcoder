#if 0 // Kuin Programming Language v.2021.9.17
{
func main()
	var s: []char :: cui@input()
	var a: regex@Regex :: regex@makeRegex("(.*)\\|.*\\|(.*)")
	do s :: a.replace(s, "$1$2", true)
	do cui@print("\{s}\n")
end func
}
...
//{{{ C++ code below is transpiled from Kuin code above by Kuin Programming Language v.2021.9.17
#if defined(_WIN32)
...
static void k_ba(type_(Array_<char16_t>) k_es /*str*/);
template<typename T_>static type_(Array_<char16_t>) k_bb(T_ k_et /*me_*/, type_(Array_<int64_t>) k_eu /*type*/);
template<typename T_>static T_ k_cn(T_ k_ev /*me_*/, type_(Array_<int64_t>) k_ew /*type*/, T_ k_ex /*n*/);
template<typename T_>static T_ k_co(T_ k_ey /*me_*/, type_(Array_<int64_t>) k_ez /*type*/, int64_t k_fa /*n*/);
template<typename T_>static T_ k_da(T_ k_fd /*me_*/, type_(Array_<int64_t>) k_fe /*type*/, int64_t k_ff /*n*/);
static char16_t k_ee();
template<typename T_>static T_ k_er(T_ k_fi /*me_*/, type_(Array_<int64_t>) k_fj /*type*/, int64_t k_fk /*start*/, int64_t k_fl /*len*/);
...
type_(Array_<char16_t>) k_ao; // ans
bool k_ap; // f
int64_t k_at;
int64_t k_au;
(k_am) = ((k_an)());
(k_ao) = (newArrayBin_<char16_t>(0,""));
(k_ap) = (true);
for(k_at = (0LL), k_au = (((k_am)->Len()) - (1LL)); k_at <= k_au; k_at += (1LL)){
if(((k_am)->At(k_at)) == (u'\u007C')){
(k_ap) = (!(k_ap));
continue;
}
if(k_ap){
(k_ao) = ((k_ao)->Cat(new_(Array_<char16_t>)(((k_am)->At(k_at)))));
}
}
(k_ba)((((newArrayBin_<char16_t>(0,""))->Cat(k_bb<type_(Array_<char16_t>)>((k_ao), (nullptr))))->Cat(newArrayBin_<char16_t>(1,"\x0A\x00"))));
}
// _finVars
static void k_i(){
(k_al) = (nullptr);
...
// input
static type_(Array_<char16_t>) k_an(){
type_(Array_<char16_t>) k_dy; // buf
int64_t k_dz; // ptr
char16_t k_ed; // c
(k_dy) = (newArrays_<type_(Array_<char16_t>)>((1024LL)));
(k_dz) = (0LL);
while(true){
(k_ed) = ((k_ee)());
if((k_ed) == (u'\u000D')){
...
if((k_dz) == ((k_dy)->Len())){
(k_dy) = ((k_dy)->Cat(newArrays_<type_(Array_<char16_t>)>(((k_dy)->Len()))));
}
((k_dy)->At(k_dz)) = (k_ed);
(k_dz) = ((k_dz) + (1LL));
}
return k_er<type_(Array_<char16_t>)>((k_dy), (nullptr), (0LL), (k_dz));
}
// print
static void k_ba(type_(Array_<char16_t>) k_es /*str*/){
if(k_es==nullptr){
...
#if defined(_WIN32)
fputws(reinterpret_cast<const wchar_t*>(k_es->B),stdout);
#else
std::u16string s_=k_es->B;
