...
static void k_i();
static void k_j();
static void k_l(int64_t k_ct /*excpt*/);
static void k_p(type_(k_o) k_cw /*me*/);
static void k_q(k_o* k_cx);
static int64_t k_r(type_(k_o) k_cy /*me*/, type_(k_o) k_cz /*t*/);
static type_(k_o) k_s(type_(k_o) k_da /*me*/);
static type_(Array_<uint8_t>) k_t(type_(k_o) k_dd /*me*/);
static type_(k_o) k_u(type_(k_o) k_df /*me*/, type_(Array_<uint8_t>) k_dg /*bin*/, int64_t* k_dh /*idx*/);
static type_(Array_<char16_t>) k_v(type_(k_o) k_dk /*me*/);
static void k_w(type_(k_n) k_dl /*me*/);
using k_dn=void(*)(type_(k_o));
static int64_t k_x(type_(k_n) k_do /*me*/, int64_t k_dp /*min*/, int64_t k_dq /*max*/);
using k_dy=uint64_t(*)(type_(k_n));
static uint64_t k_y(type_(k_n) k_eb /*me*/);
using k_ed=uint32_t(*)(type_(k_n));
static double k_z(type_(k_n) k_eg /*me*/, double k_eh /*min*/, double k_ei /*max*/);
static void k_aa(type_(k_n) k_el /*me*/, uint32_t k_em /*seed*/);
static uint32_t k_ab(type_(k_n) k_es /*me*/);
static void k_ac(k_n* k_eu);
static type_(k_n) k_ad(type_(k_n) k_ev);
static type_(Array_<uint8_t>) k_ae(type_(k_n) k_ey);
static type_(k_n) k_af(type_(k_n) k_fe, type_(Array_<uint8_t>) k_ff, int64_t* k_fg);
static type_(k_n) k_ag(type_(k_n) k_fj /*me2*/, uint32_t k_fk /*seed*/);
using k_fl=void(*)(type_(k_n), uint32_t);
template<typename T_>static T_ k_ai(T_ k_fm /*me_*/, type_(Array_<int64_t>) k_fn /*type*/, T_ k_fo /*n*/);
static int64_t k_aj();
static int64_t k_ak();
static int64_t k_an();
static int64_t k_cj(int64_t k_fw /*n1*/, int64_t k_fx /*n2*/);
static int64_t k_cq(int64_t k_fy /*n1*/, int64_t k_fz /*n2*/);
static void k_cr(type_(Array_<char16_t>) k_ga /*str*/);
template<typename T_>static type_(Array_<char16_t>) k_cs(T_ k_gb /*me_*/, type_(Array_<int64_t>) k_gc /*type*/);
template<typename T_>static T_ k_ee(T_ k_gd /*me_*/, type_(Array_<int64_t>) k_ge /*type*/, T_ k_gf /*n*/);
template<typename T_>static T_ k_ef(T_ k_gg /*me_*/, type_(Array_<int64_t>) k_gh /*type*/, int64_t k_gi /*n*/);
template<typename T_>static T_ k_er(T_ k_gl /*me_*/, type_(Array_<int64_t>) k_gm /*type*/, int64_t k_gn /*n*/);
static type_(Array_<char16_t>) k_fq();
static int64_t k_ft(type_(Array_<char16_t>) k_hk /*me_*/, bool* k_hl /*success*/);
...
k_n::k_n(): k_o(), k_en(), k_eo(), k_ep(), k_eq(){ Y = 8; }
k_n::~k_n(){
k_ac(this);
}
// k_main_
...
(k_am) = ((k_an)());
(k_ao) = ((k_an)());
(k_ap) = (newArrays_<type_(Array_<int64_t>)>((25010001LL)));
for(k_at = (0LL), k_au = ((k_am) - (1LL)); k_at <= k_au; k_at += (1LL)){
(k_av) = ((k_an)());
(k_aw) = ((k_an)());
(k_ay) = (((k_av) * (5000LL)) + (k_aw));
((k_ap)->At(k_ay)) = (((k_ap)->At(k_ay)) + (1LL));
}
for(k_bc = (1LL), k_bd = (5000LL); k_bc <= k_bd; k_bc += (1LL)){
for(k_bh = (1LL), k_bi = (5000LL); k_bh <= k_bi; k_bh += (1LL)){
(k_bk) = (((k_bc) * (5000LL)) + (k_bh));
((k_ap)->At(k_bk)) = (((k_ap)->At(k_bk)) + ((k_ap)->At((((k_bc) * (5000LL)) + (k_bh)) - (1LL))));
}
}
for(k_bo = (1LL), k_bp = (5000LL); k_bo <= k_bp; k_bo += (1LL)){
for(k_bt = (1LL), k_bu = (5000LL); k_bt <= k_bu; k_bt += (1LL)){
(k_bw) = (((k_bt) * (5000LL)) + (k_bo));
((k_ap)->At(k_bw)) = (((k_ap)->At(k_bw)) + ((k_ap)->At((((k_bt) - (1LL)) * (5000LL)) + (k_bo))));
}
}
(k_bx) = (0LL);
for(k_cb = (0LL), k_cc = (4999LL); k_cb <= k_cc; k_cb += (1LL)){
for(k_cg = (0LL), k_ch = (4999LL); k_cg <= k_ch; k_cg += (1LL)){
(k_ci) = ((k_cj)((((k_cg) + (k_ao)) + (1LL)), (5000LL)));
(k_ck) = ((k_cj)((((k_cb) + (k_ao)) + (1LL)), (5000LL)));
(k_cl) = (0LL);
(k_cl) = ((k_cl) + ((k_ap)->At(((k_ck) * (5000LL)) + (k_ci))));
(k_cl) = ((k_cl) - ((k_ap)->At(((k_cb) * (5000LL)) + (k_ci))));
(k_cl) = ((k_cl) - ((k_ap)->At(((k_ck) * (5000LL)) + (k_cg))));
(k_cl) = ((k_cl) + ((k_ap)->At(((k_cb) * (5000LL)) + (k_cg))));
(k_bx) = ((k_cq)((k_bx), (k_cl)));
}
}
(k_cr)((((newArrayBin_<char16_t>(0,""))->Cat(k_cs<int64_t>((k_bx), (nullptr))))->Cat(newArrayBin_<char16_t>(1,"\x0A\x00"))));
}
// _finVars
static void k_i(){
(k_al) = (nullptr);
...
return as_<k_o>(classTable_, (k_db), 0);
}
// _toBin
static type_(Array_<uint8_t>) k_t(type_(k_o) k_dd /*me*/){
...
type_(k_o) k_di;
type_(k_o) k_dj;
(k_di) = (k_dj = new_(k_o)(), k_p(k_dj), k_dj);
return k_di;
}
// toStr
static type_(Array_<char16_t>) k_v(type_(k_o) k_dk /*me*/){
...
uint64_t k_du; // m
uint64_t k_dv; // r
(k_dt) = (static_cast<uint64_t>(((k_dq) - (k_dp)) + (1LL)));
(k_du) = ((0x0000000000000000ULL) - (((0x0000000000000000ULL) - (k_dt)) % (k_dt)));
(k_dv) = (0x0000000000000000ULL);
if((k_du) == (0x0000000000000000ULL)){
(k_dv) = ((reinterpret_cast<k_dy>(classTable_[(k_do)->Y + 9]))((k_do)));
}
else{
do{
(k_dv) = ((reinterpret_cast<k_dy>(classTable_[(k_do)->Y + 9]))((k_do)));
...
return (static_cast<int64_t>((k_dv) % (k_dt))) + (k_dp);
}
// rndBit64
static uint64_t k_y(type_(k_n) k_eb /*me*/){
uint32_t k_ec; // t
(k_ec) = ((reinterpret_cast<k_ed>(classTable_[(k_eb)->Y + 12]))((k_eb)));
return k_ee<uint64_t>((k_ef<uint64_t>((static_cast<uint64_t>(k_ec)), (nullptr), (32LL))), (nullptr), (static_cast<uint64_t>((reinterpret_cast<k_ed>(classTable_[(k_eb)->Y + 12]))((k_eb)))));
}
// rndFloat
static double k_z(type_(k_n) k_eg /*me*/, double k_eh /*min*/, double k_ei /*max*/){
return (((static_cast<double>((reinterpret_cast<k_dy>(classTable_[(k_eg)->Y + 9]))((k_eg)))) / (1.844674407370955e+19)) * ((k_ei) - (k_eh))) + (k_eh);
}
// setSeed
static void k_aa(type_(k_n) k_el /*me*/, uint32_t k_em /*seed*/){
((k_el)->k_en) = (0x075BCD15U);
((k_el)->k_eo) = (0x159A55E5U);
((k_el)->k_ep) = ((0x1F123BB5U) * (k_em));
((k_el)->k_eq) = ((0x05491333U) * (k_er<uint32_t>(((k_el)->k_ep), (nullptr), (1LL))));
}
// xs128
static uint32_t k_ab(type_(k_n) k_es /*me*/){
uint32_t k_et; // t
(k_et) = (k_ai<uint32_t>(((k_es)->k_en), (nullptr), (k_ef<uint32_t>(((k_es)->k_en), (nullptr), (11LL)))));
((k_es)->k_en) = ((k_es)->k_eo);
((k_es)->k_eo) = ((k_es)->k_ep);
((k_es)->k_ep) = ((k_es)->k_eq);
((k_es)->k_eq) = (k_ai<uint32_t>((k_ai<uint32_t>(((k_es)->k_eq), (nullptr), (k_er<uint32_t>(((k_es)->k_eq), (nullptr), (19LL))))), (nullptr), (k_ai<uint32_t>((k_et), (nullptr), (k_er<uint32_t>((k_et), (nullptr), (8LL)))))));
return (k_es)->k_eq;
}
// _dtor
static void k_ac(k_n* k_eu){
...
type_(k_n) k_ew;
type_(k_n) k_ex;
(k_ew) = (k_ex = new_(k_n)(), k_w(k_ex), k_ex);
((k_ew)->k_en) = ((k_ev)->k_en);
((k_ew)->k_eo) = ((k_ev)->k_eo);
((k_ew)->k_ep) = ((k_ev)->k_ep);
((k_ew)->k_eq) = ((k_ev)->k_eq);
return as_<k_n>(classTable_, (k_ew), 8);
}
// _toBin
static type_(Array_<uint8_t>) k_ae(type_(k_n) k_ey){
type_(Array_<uint8_t>) k_ez;
(k_ez) = (newArrays_<type_(Array_<uint8_t>)>((0LL)));
(k_ez) = ((k_ez)->Cat(toBin_<uint32_t>()(((k_ey)->k_en))));
(k_ez) = ((k_ez)->Cat(toBin_<uint32_t>()(((k_ey)->k_eo))));
(k_ez) = ((k_ez)->Cat(toBin_<uint32_t>()(((k_ey)->k_ep))));
(k_ez) = ((k_ez)->Cat(toBin_<uint32_t>()(((k_ey)->k_eq))));
return k_ez;
}
// _fromBin
static type_(k_n) k_af(type_(k_n) k_fe, type_(Array_<uint8_t>) k_ff, int64_t* k_fg){
type_(k_n) k_fh;
type_(k_n) k_fi;
(k_fh) = (k_fi = new_(k_n)(), k_w(k_fi), k_fi);
((k_fh)->k_en) = (fromBin_<uint32_t>()((k_ff), (*k_fg)));
((k_fh)->k_eo) = (fromBin_<uint32_t>()((k_ff), (*k_fg)));
((k_fh)->k_ep) = (fromBin_<uint32_t>()((k_ff), (*k_fg)));
((k_fh)->k_eq) = (fromBin_<uint32_t>()((k_ff), (*k_fg)));
return k_fh;
}
// makeRnd
static type_(k_n) k_ag(type_(k_n) k_fj /*me2*/, uint32_t k_fk /*seed*/){
(reinterpret_cast<k_fl>(classTable_[(k_fj)->Y + 11]))((k_fj), (k_fk));
return k_fj;
}
// _xor
template<typename T_>static T_ k_ai(T_ k_fm /*me_*/, type_(Array_<int64_t>) k_fn /*type*/, T_ k_fo /*n*/){
...
// min
static int64_t k_cj(int64_t k_fw /*n1*/, int64_t k_fx /*n2*/){
return ((k_fw) <= (k_fx)) ? (k_fw) : (k_fx);
}
// max
static int64_t k_cq(int64_t k_fy /*n1*/, int64_t k_fz /*n2*/){
return ((k_fy) >= (k_fz)) ? (k_fy) : (k_fz);
}
// print
static void k_cr(type_(Array_<char16_t>) k_ga /*str*/){
if(k_ga==nullptr){
