...
#endif
//{{{ C++ translated by Kuin Programming Language v.2021.4.17
#if defined(_WIN32)
...
static void k_i();
static void k_j();
static void k_l(int64_t k_bs /*excpt*/);
static void k_p(type_(k_o) k_bv /*me*/);
static void k_q(k_o* k_bw);
static int64_t k_r(type_(k_o) k_bx /*me*/, type_(k_o) k_by /*t*/);
static type_(k_o) k_s(type_(k_o) k_bz /*me*/);
static type_(Array_<uint8_t>) k_t(type_(k_o) k_cc /*me*/);
static type_(k_o) k_u(type_(k_o) k_ce /*me*/, type_(Array_<uint8_t>) k_cf /*bin*/, int64_t* k_cg /*idx*/);
static type_(Array_<char16_t>) k_v(type_(k_o) k_cj /*me*/);
static void k_w(type_(k_n) k_ck /*me*/);
using k_cm=void(*)(type_(k_o));
static int64_t k_x(type_(k_n) k_cn /*me*/, int64_t k_co /*min*/, int64_t k_cp /*max*/);
using k_cx=uint64_t(*)(type_(k_n));
static uint64_t k_y(type_(k_n) k_da /*me*/);
using k_dc=uint32_t(*)(type_(k_n));
static double k_z(type_(k_n) k_df /*me*/, double k_dg /*min*/, double k_dh /*max*/);
static void k_aa(type_(k_n) k_dk /*me*/, uint32_t k_dl /*seed*/);
static uint32_t k_ab(type_(k_n) k_dr /*me*/);
static void k_ac(k_n* k_dt);
static type_(k_n) k_ad(type_(k_n) k_du);
static type_(Array_<uint8_t>) k_ae(type_(k_n) k_dx);
static type_(k_n) k_af(type_(k_n) k_ed, type_(Array_<uint8_t>) k_ee, int64_t* k_ef);
static type_(k_n) k_ag(type_(k_n) k_ei /*me2*/, uint32_t k_ej /*seed*/);
using k_ek=void(*)(type_(k_n), uint32_t);
template<typename T_>static T_ k_ai(T_ k_el /*me_*/, type_(Array_<int64_t>) k_em /*type*/, T_ k_en /*n*/);
static int64_t k_aj();
static int64_t k_ak();
static int64_t k_an();
static int64_t k_az(int64_t k_ev /*me_*/);
static void k_bq(type_(Array_<char16_t>) k_ew /*str*/);
template<typename T_>static type_(Array_<char16_t>) k_br(T_ k_ex /*me_*/, type_(Array_<int64_t>) k_ey /*type*/);
template<typename T_>static T_ k_dd(T_ k_ez /*me_*/, type_(Array_<int64_t>) k_fa /*type*/, T_ k_fb /*n*/);
template<typename T_>static T_ k_de(T_ k_fc /*me_*/, type_(Array_<int64_t>) k_fd /*type*/, int64_t k_fe /*n*/);
template<typename T_>static T_ k_dq(T_ k_fh /*me_*/, type_(Array_<int64_t>) k_fi /*type*/, int64_t k_fj /*n*/);
static type_(Array_<char16_t>) k_ep();
static int64_t k_es(type_(Array_<char16_t>) k_gg /*me_*/, bool* k_gh /*success*/);
static char16_t k_fs();
template<typename T_>static T_ k_gf(T_ k_gw /*me_*/, type_(Array_<int64_t>) k_gx /*type*/, int64_t k_gy /*start*/, int64_t k_gz /*len*/);
static char16_t k_gm();
...
k_n::k_n(): k_o(), k_dm(), k_dn(), k_do(), k_dp(){ Y = 8; }
k_n::~k_n(){
k_ac(this);
}
// k_main_
...
(k_ao) = ((k_an)());
(k_ap) = ((k_an)());
(k_aq) = (newArrays_<type_(Array_<int64_t>)>(((k_am) + (1LL))));
...
(k_aw) = (0LL);
(k_ax) = ((k_ao) + (1LL));
while(((k_az)(((k_aw) - (k_ax)))) > (1LL)){
(k_ba) = (((k_aw) + (k_ax)) / (2LL));
(k_bb) = (0LL);
(k_bc) = (0LL);
for(k_bg = (0LL), k_bh = (k_am); k_bg <= k_bh; k_bg += (1LL)){
if((((k_aq)->At(k_bg)) - (k_bb)) >= (k_ba)){
(k_bc) = ((k_bc) + (1LL));
(k_bb) = ((k_aq)->At(k_bg));
}
}
...
(k_bp) = (k_aw);
(k_bq)((((newArrayBin_<char16_t>(0,""))->Cat(k_br<int64_t>((k_bp), (nullptr))))->Cat(newArrayBin_<char16_t>(1,"\x0A\x00"))));
}
// _finVars
static void k_i(){
(k_al) = (nullptr);
...
return as_<k_o>(classTable_, (k_ca), 0);
}
// _toBin
static type_(Array_<uint8_t>) k_t(type_(k_o) k_cc /*me*/){
type_(Array_<uint8_t>) k_cd;
(k_cd) = (newArrays_<type_(Array_<uint8_t>)>((0LL)));
return k_cd;
}
// _fromBin
static type_(k_o) k_u(type_(k_o) k_ce /*me*/, type_(Array_<uint8_t>) k_cf /*bin*/, int64_t* k_cg /*idx*/){
type_(k_o) k_ch;
type_(k_o) k_ci;
(k_ch) = (k_ci = new_(k_o)(), k_p(k_ci), k_ci);
return k_ch;
}
// toStr
static type_(Array_<char16_t>) k_v(type_(k_o) k_cj /*me*/){
...
uint64_t k_ct; // m
uint64_t k_cu; // r
(k_cs) = (static_cast<uint64_t>(((k_cp) - (k_co)) + (1LL)));
(k_ct) = ((0x0000000000000000ULL) - (((0x0000000000000000ULL) - (k_cs)) % (k_cs)));
(k_cu) = (0x0000000000000000ULL);
if((k_ct) == (0x0000000000000000ULL)){
(k_cu) = ((reinterpret_cast<k_cx>(classTable_[(k_cn)->Y + 9]))((k_cn)));
}
else{
do{
(k_cu) = ((reinterpret_cast<k_cx>(classTable_[(k_cn)->Y + 9]))((k_cn)));
...
return (static_cast<int64_t>((k_cu) % (k_cs))) + (k_co);
}
// rndBit64
static uint64_t k_y(type_(k_n) k_da /*me*/){
uint32_t k_db; // t
(k_db) = ((reinterpret_cast<k_dc>(classTable_[(k_da)->Y + 12]))((k_da)));
return k_dd<uint64_t>((k_de<uint64_t>((static_cast<uint64_t>(k_db)), (nullptr), (32LL))), (nullptr), (static_cast<uint64_t>((reinterpret_cast<k_dc>(classTable_[(k_da)->Y + 12]))((k_da)))));
}
// rndFloat
static double k_z(type_(k_n) k_df /*me*/, double k_dg /*min*/, double k_dh /*max*/){
return (((static_cast<double>((reinterpret_cast<k_cx>(classTable_[(k_df)->Y + 9]))((k_df)))) / (1.844674407370955e+19)) * ((k_dh) - (k_dg))) + (k_dg);
}
// setSeed
static void k_aa(type_(k_n) k_dk /*me*/, uint32_t k_dl /*seed*/){
((k_dk)->k_dm) = (0x075BCD15U);
((k_dk)->k_dn) = (0x159A55E5U);
((k_dk)->k_do) = ((0x1F123BB5U) * (k_dl));
((k_dk)->k_dp) = ((0x05491333U) * (k_dq<uint32_t>(((k_dk)->k_do), (nullptr), (1LL))));
}
// xs128
static uint32_t k_ab(type_(k_n) k_dr /*me*/){
uint32_t k_ds; // t
(k_ds) = (k_ai<uint32_t>(((k_dr)->k_dm), (nullptr), (k_de<uint32_t>(((k_dr)->k_dm), (nullptr), (11LL)))));
((k_dr)->k_dm) = ((k_dr)->k_dn);
((k_dr)->k_dn) = ((k_dr)->k_do);
((k_dr)->k_do) = ((k_dr)->k_dp);
((k_dr)->k_dp) = (k_ai<uint32_t>((k_ai<uint32_t>(((k_dr)->k_dp), (nullptr), (k_dq<uint32_t>(((k_dr)->k_dp), (nullptr), (19LL))))), (nullptr), (k_ai<uint32_t>((k_ds), (nullptr), (k_dq<uint32_t>((k_ds), (nullptr), (8LL)))))));
return (k_dr)->k_dp;
}
// _dtor
static void k_ac(k_n* k_dt){
...
((k_dv)->k_do) = ((k_du)->k_do);
((k_dv)->k_dp) = ((k_du)->k_dp);
return as_<k_n>(classTable_, (k_dv), 8);
}
// _toBin
static type_(Array_<uint8_t>) k_ae(type_(k_n) k_dx){
type_(Array_<uint8_t>) k_dy;
(k_dy) = (newArrays_<type_(Array_<uint8_t>)>((0LL)));
(k_dy) = ((k_dy)->Cat(toBin_<uint32_t>()(((k_dx)->k_dm))));
(k_dy) = ((k_dy)->Cat(toBin_<uint32_t>()(((k_dx)->k_dn))));
(k_dy) = ((k_dy)->Cat(toBin_<uint32_t>()(((k_dx)->k_do))));
(k_dy) = ((k_dy)->Cat(toBin_<uint32_t>()(((k_dx)->k_dp))));
return k_dy;
}
// _fromBin
static type_(k_n) k_af(type_(k_n) k_ed, type_(Array_<uint8_t>) k_ee, int64_t* k_ef){
...
((k_eg)->k_dm) = (fromBin_<uint32_t>()((k_ee), (*k_ef)));
((k_eg)->k_dn) = (fromBin_<uint32_t>()((k_ee), (*k_ef)));
((k_eg)->k_do) = (fromBin_<uint32_t>()((k_ee), (*k_ef)));
((k_eg)->k_dp) = (fromBin_<uint32_t>()((k_ee), (*k_ef)));
return k_eg;
}
// makeRnd
static type_(k_n) k_ag(type_(k_n) k_ei /*me2*/, uint32_t k_ej /*seed*/){
(reinterpret_cast<k_ek>(classTable_[(k_ei)->Y + 11]))((k_ei), (k_ej));
return k_ei;
}
// _xor
template<typename T_>static T_ k_ai(T_ k_el /*me_*/, type_(Array_<int64_t>) k_em /*type*/, T_ k_en /*n*/){
...
while(true){
(k_gl) = ((k_gm)());
for(k_gq = (0LL), k_gr = (((k_al)->Len()) - (1LL)); k_gq <= k_gr; k_gq += (1LL)){
if(((k_gl) == ((k_al)->At(k_gq))) || (((k_gl) == (u'\u000D')) && (((k_al)->At(k_gq)) == (u'\u000A')))){
return u'\u0000';
}
}
if((k_gl) == (u'\u000D')){
