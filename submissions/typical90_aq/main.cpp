...
static void k_i();
static void k_j();
static void k_l(int64_t k_cy /*excpt*/);
static void k_p(type_(k_o) k_db /*me*/);
static void k_q(k_o* k_dc);
static int64_t k_r(type_(k_o) k_dd /*me*/, type_(k_o) k_de /*t*/);
static type_(k_o) k_s(type_(k_o) k_df /*me*/);
static type_(Array_<uint8_t>) k_t(type_(k_o) k_di /*me*/);
static type_(k_o) k_u(type_(k_o) k_dk /*me*/, type_(Array_<uint8_t>) k_dl /*bin*/, int64_t* k_dm /*idx*/);
static type_(Array_<char16_t>) k_v(type_(k_o) k_dp /*me*/);
static void k_w(type_(k_n) k_dq /*me*/);
using k_ds=void(*)(type_(k_o));
static int64_t k_x(type_(k_n) k_dt /*me*/, int64_t k_du /*min*/, int64_t k_dv /*max*/);
using k_ed=uint64_t(*)(type_(k_n));
static uint64_t k_y(type_(k_n) k_eg /*me*/);
using k_ei=uint32_t(*)(type_(k_n));
static double k_z(type_(k_n) k_el /*me*/, double k_em /*min*/, double k_en /*max*/);
static void k_aa(type_(k_n) k_eq /*me*/, uint32_t k_er /*seed*/);
static uint32_t k_ab(type_(k_n) k_ex /*me*/);
static void k_ac(k_n* k_ez);
static type_(k_n) k_ad(type_(k_n) k_fa);
static type_(Array_<uint8_t>) k_ae(type_(k_n) k_fd);
static type_(k_n) k_af(type_(k_n) k_fj, type_(Array_<uint8_t>) k_fk, int64_t* k_fl);
static type_(k_n) k_ag(type_(k_n) k_fo /*me2*/, uint32_t k_fp /*seed*/);
using k_fq=void(*)(type_(k_n), uint32_t);
template<typename T_>static T_ k_ai(T_ k_fr /*me_*/, type_(Array_<int64_t>) k_fs /*type*/, T_ k_ft /*n*/);
static int64_t k_aj();
static int64_t k_ak();
static int64_t k_an();
static type_(Array_<char16_t>) k_az();
template<typename T_,typename C_>static void k_bt(T_ k_gv /*me_*/, type_(Array_<int64_t>) k_gw /*type*/, C_ k_gx /*item*/);
template<typename T_,typename R_>static R_ k_bz(T_ k_gy /*me_*/, type_(Array_<int64_t>) k_gz /*type*/);
template<typename T_,typename R_>static R_ k_ch(T_ k_hc /*me_*/, type_(Array_<int64_t>) k_hd /*type*/);
static void k_cw(type_(Array_<char16_t>) k_he /*str*/);
template<typename T_>static type_(Array_<char16_t>) k_cx(T_ k_hf /*me_*/, type_(Array_<int64_t>) k_hg /*type*/);
template<typename T_>static T_ k_ej(T_ k_hh /*me_*/, type_(Array_<int64_t>) k_hi /*type*/, T_ k_hj /*n*/);
template<typename T_>static T_ k_ek(T_ k_hk /*me_*/, type_(Array_<int64_t>) k_hl /*type*/, int64_t k_hm /*n*/);
template<typename T_>static T_ k_ew(T_ k_hp /*me_*/, type_(Array_<int64_t>) k_hq /*type*/, int64_t k_hr /*n*/);
static type_(Array_<char16_t>) k_fv();
static int64_t k_fy(type_(Array_<char16_t>) k_in /*me_*/, bool* k_io /*success*/);
static char16_t k_gh();
template<typename T_>static T_ k_gu(T_ k_ip /*me_*/, type_(Array_<int64_t>) k_iq /*type*/, int64_t k_ir /*start*/, int64_t k_is /*len*/);
static char16_t k_ia();
...
k_n::k_n(): k_o(), k_es(), k_et(), k_eu(), k_ev(){ Y = 8; }
k_n::~k_n(){
k_ac(this);
}
// k_main_
...
int64_t k_ar; // rt
int64_t k_as; // ct
type_(Array_<type_(Array_<char16_t>)>) k_at; // s
int64_t k_ax;
type_(Array_<int64_t>) k_ba; // dx
type_(Array_<int64_t>) k_bb; // dy
type_(Queue_<int64_t>) k_bc; // qn
type_(Array_<type_(Array_<type_(Array_<int64_t>)>)>) k_bd; // v
int64_t k_bh;
int64_t k_bm;
int64_t k_br;
int64_t k_bu; // cnt
type_(Queue_<int64_t>) k_bw; // q
int64_t k_by; // val
int64_t k_ca; // pos
int64_t k_cb; // y
int64_t k_cc; // x
...
(k_ar) = (((k_an)()) - (1LL));
(k_as) = (((k_an)()) - (1LL));
(k_at) = (newArrays_<type_(Array_<type_(Array_<char16_t>)>)>((k_am)));
for(k_ax = (0LL), k_ay = ((k_am) - (1LL)); k_ax <= k_ay; k_ax += (1LL)){
((k_at)->At(k_ax)) = ((k_az)());
}
(k_ba) = (new_(Array_<int64_t>)((1LL), (0LL), (-1LL), (0LL)));
(k_bb) = (new_(Array_<int64_t>)((0LL), (1LL), (0LL), (-1LL)));
(k_bc) = (new_(Queue_<int64_t>)());
(k_bd) = (newArrays_<type_(Array_<type_(Array_<type_(Array_<int64_t>)>)>)>((k_am), (k_ao), (2LL)));
for(k_bh = (0LL), k_bi = ((k_am) - (1LL)); k_bh <= k_bi; k_bh += (1LL)){
for(k_bm = (0LL), k_bn = ((k_ao) - (1LL)); k_bm <= k_bn; k_bm += (1LL)){
((((k_bd)->At(k_bh))->At(k_bm))->At(0LL)) = (9223372036854775807LL);
((((k_bd)->At(k_bh))->At(k_bm))->At(1LL)) = (9223372036854775807LL);
}
}
((((k_bd)->At(k_ap))->At(k_aq))->At(0LL)) = (0LL);
((((k_bd)->At(k_ap))->At(k_aq))->At(1LL)) = (0LL);
for(k_br = (0LL), k_bs = (3LL); k_br <= k_bs; k_br += (1LL)){
k_bt<type_(Queue_<int64_t>), int64_t>((k_bc), (nullptr), (((((k_ap) * (k_ao)) + (k_aq)) * (4LL)) + (k_br)));
}
(k_bu) = (0LL);
while(((k_bc)->Len()) != (0LL)){
(k_bw) = (copy_<type_(Queue_<int64_t>)>()(k_bc));
(k_bc) = (new_(Queue_<int64_t>)());
while(((k_bw)->Len()) != (0LL)){
(k_by) = (k_bz<type_(Queue_<int64_t>), int64_t>((k_bw), (nullptr)));
(k_ca) = ((k_by) / (4LL));
(k_cb) = ((k_ca) / (k_ao));
(k_cc) = ((k_ca) % (k_ao));
(k_cd) = ((k_by) % (4LL));
while(true){
((((k_bd)->At(k_cb))->At(k_cc))->At((k_cd) % (2LL))) = (k_ch<type_(Array_<int64_t>), int64_t>((new_(Array_<int64_t>)(((((k_bd)->At(k_cb))->At(k_cc))->At((k_cd) % (2LL))), (k_bu))), (nullptr)));
if(((((k_bd)->At(k_cb))->At(k_cc))->At((1LL) - ((k_cd) % (2LL)))) == (9223372036854775807LL)){
k_bt<type_(Queue_<int64_t>), int64_t>((k_bc), (nullptr), (((((k_cb) * (k_ao)) + (k_cc)) * (4LL)) + (((k_cd) + (1LL)) % (4LL))));
k_bt<type_(Queue_<int64_t>), int64_t>((k_bc), (nullptr), (((((k_cb) * (k_ao)) + (k_cc)) * (4LL)) + (((k_cd) + (3LL)) % (4LL))));
}
(k_cb) = ((k_cb) + ((k_bb)->At(k_cd)));
(k_cc) = ((k_cc) + ((k_ba)->At(k_cd)));
if(((k_cc) == (-1LL)) || ((k_cc) == (k_ao))){
break;
}
if(((k_cb) == (-1LL)) || ((k_cb) == (k_am))){
break;
}
if((((k_at)->At(k_cb))->At(k_cc)) == (u'\u0023')){
break;
}
if(((((k_bd)->At(k_cb))->At(k_cc))->At((k_cd) % (2LL))) <= (k_bu)){
break;
}
}
}
...
(k_cv) = (k_ch<type_(Array_<int64_t>), int64_t>((((k_bd)->At(k_ar))->At(k_as)), (nullptr)));
(k_cw)((((newArrayBin_<char16_t>(0,""))->Cat(k_cx<int64_t>((k_cv), (nullptr))))->Cat(newArrayBin_<char16_t>(1,"\x0A\x00"))));
}
// _finVars
static void k_i(){
(k_al) = (nullptr);
...
type_(k_o) k_dg;
type_(k_o) k_dh;
(k_dg) = (k_dh = new_(k_o)(), k_p(k_dh), k_dh);
return as_<k_o>(classTable_, (k_dg), 0);
}
// _toBin
static type_(Array_<uint8_t>) k_t(type_(k_o) k_di /*me*/){
...
type_(k_o) k_dn;
type_(k_o) k_do;
(k_dn) = (k_do = new_(k_o)(), k_p(k_do), k_do);
return k_dn;
}
// toStr
static type_(Array_<char16_t>) k_v(type_(k_o) k_dp /*me*/){
...
uint64_t k_dz; // m
uint64_t k_ea; // r
(k_dy) = (static_cast<uint64_t>(((k_dv) - (k_du)) + (1LL)));
(k_dz) = ((0x0000000000000000ULL) - (((0x0000000000000000ULL) - (k_dy)) % (k_dy)));
(k_ea) = (0x0000000000000000ULL);
if((k_dz) == (0x0000000000000000ULL)){
(k_ea) = ((reinterpret_cast<k_ed>(classTable_[(k_dt)->Y + 9]))((k_dt)));
}
else{
do{
(k_ea) = ((reinterpret_cast<k_ed>(classTable_[(k_dt)->Y + 9]))((k_dt)));
...
return (static_cast<int64_t>((k_ea) % (k_dy))) + (k_du);
}
// rndBit64
static uint64_t k_y(type_(k_n) k_eg /*me*/){
uint32_t k_eh; // t
(k_eh) = ((reinterpret_cast<k_ei>(classTable_[(k_eg)->Y + 12]))((k_eg)));
return k_ej<uint64_t>((k_ek<uint64_t>((static_cast<uint64_t>(k_eh)), (nullptr), (32LL))), (nullptr), (static_cast<uint64_t>((reinterpret_cast<k_ei>(classTable_[(k_eg)->Y + 12]))((k_eg)))));
}
// rndFloat
static double k_z(type_(k_n) k_el /*me*/, double k_em /*min*/, double k_en /*max*/){
return (((static_cast<double>((reinterpret_cast<k_ed>(classTable_[(k_el)->Y + 9]))((k_el)))) / (1.844674407370955e+19)) * ((k_en) - (k_em))) + (k_em);
}
// setSeed
static void k_aa(type_(k_n) k_eq /*me*/, uint32_t k_er /*seed*/){
((k_eq)->k_es) = (0x075BCD15U);
((k_eq)->k_et) = (0x159A55E5U);
((k_eq)->k_eu) = ((0x1F123BB5U) * (k_er));
((k_eq)->k_ev) = ((0x05491333U) * (k_ew<uint32_t>(((k_eq)->k_eu), (nullptr), (1LL))));
}
// xs128
static uint32_t k_ab(type_(k_n) k_ex /*me*/){
uint32_t k_ey; // t
(k_ey) = (k_ai<uint32_t>(((k_ex)->k_es), (nullptr), (k_ek<uint32_t>(((k_ex)->k_es), (nullptr), (11LL)))));
((k_ex)->k_es) = ((k_ex)->k_et);
((k_ex)->k_et) = ((k_ex)->k_eu);
((k_ex)->k_eu) = ((k_ex)->k_ev);
((k_ex)->k_ev) = (k_ai<uint32_t>((k_ai<uint32_t>(((k_ex)->k_ev), (nullptr), (k_ew<uint32_t>(((k_ex)->k_ev), (nullptr), (19LL))))), (nullptr), (k_ai<uint32_t>((k_ey), (nullptr), (k_ew<uint32_t>((k_ey), (nullptr), (8LL)))))));
return (k_ex)->k_ev;
}
// _dtor
static void k_ac(k_n* k_ez){
...
type_(k_n) k_fb;
type_(k_n) k_fc;
(k_fb) = (k_fc = new_(k_n)(), k_w(k_fc), k_fc);
((k_fb)->k_es) = ((k_fa)->k_es);
((k_fb)->k_et) = ((k_fa)->k_et);
((k_fb)->k_eu) = ((k_fa)->k_eu);
((k_fb)->k_ev) = ((k_fa)->k_ev);
return as_<k_n>(classTable_, (k_fb), 8);
}
// _toBin
static type_(Array_<uint8_t>) k_ae(type_(k_n) k_fd){
type_(Array_<uint8_t>) k_fe;
(k_fe) = (newArrays_<type_(Array_<uint8_t>)>((0LL)));
(k_fe) = ((k_fe)->Cat(toBin_<uint32_t>()(((k_fd)->k_es))));
(k_fe) = ((k_fe)->Cat(toBin_<uint32_t>()(((k_fd)->k_et))));
(k_fe) = ((k_fe)->Cat(toBin_<uint32_t>()(((k_fd)->k_eu))));
(k_fe) = ((k_fe)->Cat(toBin_<uint32_t>()(((k_fd)->k_ev))));
return k_fe;
}
// _fromBin
static type_(k_n) k_af(type_(k_n) k_fj, type_(Array_<uint8_t>) k_fk, int64_t* k_fl){
type_(k_n) k_fm;
type_(k_n) k_fn;
(k_fm) = (k_fn = new_(k_n)(), k_w(k_fn), k_fn);
((k_fm)->k_es) = (fromBin_<uint32_t>()((k_fk), (*k_fl)));
((k_fm)->k_et) = (fromBin_<uint32_t>()((k_fk), (*k_fl)));
((k_fm)->k_eu) = (fromBin_<uint32_t>()((k_fk), (*k_fl)));
((k_fm)->k_ev) = (fromBin_<uint32_t>()((k_fk), (*k_fl)));
return k_fm;
}
// makeRnd
static type_(k_n) k_ag(type_(k_n) k_fo /*me2*/, uint32_t k_fp /*seed*/){
(reinterpret_cast<k_fq>(classTable_[(k_fo)->Y + 11]))((k_fo), (k_fp));
return k_fo;
}
// _xor
template<typename T_>static T_ k_ai(T_ k_fr /*me_*/, type_(Array_<int64_t>) k_fs /*type*/, T_ k_ft /*n*/){
...
// input
static type_(Array_<char16_t>) k_az(){
type_(Array_<char16_t>) k_gb; // buf
int64_t k_gc; // ptr
char16_t k_gg; // c
(k_gb) = (newArrays_<type_(Array_<char16_t>)>((1024LL)));
(k_gc) = (0LL);
while(true){
(k_gg) = ((k_gh)());
if((k_gg) == (u'\u000D')){
...
if((k_gc) == ((k_gb)->Len())){
(k_gb) = ((k_gb)->Cat(newArrays_<type_(Array_<char16_t>)>(((k_gb)->Len()))));
}
((k_gb)->At(k_gc)) = (k_gg);
(k_gc) = ((k_gc) + (1LL));
}
return k_gu<type_(Array_<char16_t>)>((k_gb), (nullptr), (0LL), (k_gc));
}
// _addQueue
template<typename T_,typename C_>static void k_bt(T_ k_gv /*me_*/, type_(Array_<int64_t>) k_gw /*type*/, C_ k_gx /*item*/){
k_gv->B.push(k_gx);
...
if(cmp_(r_,k_hc->B[i_])>0)r_=k_hc->B[i_];
}
return r_;
}
// print
...
#if defined(_WIN32)
fputws(reinterpret_cast<const wchar_t*>(k_he->B),stdout);
#else
std::u16string s_=k_he->B;
...
// inputStr
static type_(Array_<char16_t>) k_fv(){
type_(Array_<char16_t>) k_hu; // buf
int64_t k_hv; // ptr
char16_t k_hz; // c
(k_hu) = (newArrays_<type_(Array_<char16_t>)>((1024LL)));
(k_hv) = (0LL);
while(true){
(k_hz) = ((k_ia)());
if((k_hz) == (u'\uFFFF')){
...
if((k_hv) == ((k_hu)->Len())){
(k_hu) = ((k_hu)->Cat(newArrays_<type_(Array_<char16_t>)>(((k_hu)->Len()))));
}
((k_hu)->At(k_hv)) = (k_hz);
(k_hv) = ((k_hv) + (1LL));
}
return k_gu<type_(Array_<char16_t>)>((k_hu), (nullptr), (0LL), (k_hv));
}
// _toInt
static int64_t k_fy(type_(Array_<char16_t>) k_in /*me_*/, bool* k_io /*success*/){
std::u16string u_=k_in->B;
...
while(true){
(k_iw) = ((k_gh)());
for(k_ja = (0LL), k_jb = (((k_al)->Len()) - (1LL)); k_ja <= k_jb; k_ja += (1LL)){
if(((k_iw) == ((k_al)->At(k_ja))) || (((k_iw) == (u'\u000D')) && (((k_al)->At(k_ja)) == (u'\u000A')))){
return u'\u0000';
}
}
if((k_iw) == (u'\u000D')){
