...
using k_ci=uint64_t(*)(type_(k_n));
static uint64_t k_y(type_(k_n) k_cl /*me*/);
using k_cn=uint32_t(*)(type_(k_n));
static double k_z(type_(k_n) k_cq /*me*/, double k_cr /*min*/, double k_cs /*max*/);
...
static int64_t k_ak();
static int64_t k_an();
static type_(Array_<char16_t>) k_ap();
static void k_bb(type_(Array_<char16_t>) k_fa /*str*/);
template<typename T_>static type_(Array_<char16_t>) k_bc(T_ k_fb /*me_*/, type_(Array_<int64_t>) k_fc /*type*/);
template<typename T_>static T_ k_co(T_ k_fd /*me_*/, type_(Array_<int64_t>) k_fe /*type*/, T_ k_ff /*n*/);
template<typename T_>static T_ k_cp(T_ k_fg /*me_*/, type_(Array_<int64_t>) k_fh /*type*/, int64_t k_fi /*n*/);
template<typename T_>static T_ k_db(T_ k_fl /*me_*/, type_(Array_<int64_t>) k_fm /*type*/, int64_t k_fn /*n*/);
static type_(Array_<char16_t>) k_ea();
static int64_t k_ed(type_(Array_<char16_t>) k_gj /*me_*/, bool* k_gk /*success*/);
static char16_t k_em();
template<typename T_>static T_ k_ez(T_ k_gl /*me_*/, type_(Array_<int64_t>) k_gm /*type*/, int64_t k_gn /*start*/, int64_t k_go /*len*/);
static char16_t k_fw();
...
static void k_g(){
int64_t k_am; // n
type_(Array_<char16_t>) k_ao; // s
int64_t k_aq; // cnt
int64_t k_ar; // ans
int64_t k_av;
int64_t k_aw;
(k_am) = ((k_an)());
(k_ao) = (((k_ap)())->Cat(newArrayBin_<char16_t>(1,"\x2D\x00")));
(k_aq) = (1LL);
(k_ar) = (((k_am) * ((k_am) + (1LL))) / (2LL));
for(k_av = (1LL), k_aw = (k_am); k_av <= k_aw; k_av += (1LL)){
if(((k_ao)->At(k_av)) != ((k_ao)->At((k_av) - (1LL)))){
(k_ar) = ((k_ar) - (((k_aq) * ((k_aq) + (1LL))) / (2LL)));
(k_aq) = (0LL);
}
(k_aq) = ((k_aq) + (1LL));
}
(k_bb)((((newArrayBin_<char16_t>(0,""))->Cat(k_bc<int64_t>((k_ar), (nullptr))))->Cat(newArrayBin_<char16_t>(1,"\x0A\x00"))));
}
// _finVars
static void k_i(){
(k_al) = (nullptr);
...
uint64_t k_ce; // m
uint64_t k_cf; // r
(k_cd) = (static_cast<uint64_t>(((k_ca) - (k_bz)) + (1LL)));
(k_ce) = ((0x0000000000000000ULL) - (((0x0000000000000000ULL) - (k_cd)) % (k_cd)));
(k_cf) = (0x0000000000000000ULL);
if((k_ce) == (0x0000000000000000ULL)){
(k_cf) = ((reinterpret_cast<k_ci>(classTable_[(k_by)->Y + 9]))((k_by)));
}
else{
do{
(k_cf) = ((reinterpret_cast<k_ci>(classTable_[(k_by)->Y + 9]))((k_by)));
...
return (static_cast<int64_t>((k_cf) % (k_cd))) + (k_bz);
}
// rndBit64
static uint64_t k_y(type_(k_n) k_cl /*me*/){
uint32_t k_cm; // t
(k_cm) = ((reinterpret_cast<k_cn>(classTable_[(k_cl)->Y + 12]))((k_cl)));
return k_co<uint64_t>((k_cp<uint64_t>((static_cast<uint64_t>(k_cm)), (nullptr), (32LL))), (nullptr), (static_cast<uint64_t>((reinterpret_cast<k_cn>(classTable_[(k_cl)->Y + 12]))((k_cl)))));
}
// rndFloat
static double k_z(type_(k_n) k_cq /*me*/, double k_cr /*min*/, double k_cs /*max*/){
return (((static_cast<double>((reinterpret_cast<k_ci>(classTable_[(k_cq)->Y + 9]))((k_cq)))) / (1.844674407370955e+19)) * ((k_cs) - (k_cr))) + (k_cr);
}
// setSeed
static void k_aa(type_(k_n) k_cv /*me*/, uint32_t k_cw /*seed*/){
...
static uint32_t k_ab(type_(k_n) k_dc /*me*/){
uint32_t k_dd; // t
(k_dd) = (k_ai<uint32_t>(((k_dc)->k_cx), (nullptr), (k_cp<uint32_t>(((k_dc)->k_cx), (nullptr), (11LL)))));
((k_dc)->k_cx) = ((k_dc)->k_cy);
((k_dc)->k_cy) = ((k_dc)->k_cz);
((k_dc)->k_cz) = ((k_dc)->k_da);
...
#if defined(_WIN32)
fputws(reinterpret_cast<const wchar_t*>(k_fa->B),stdout);
#else
std::u16string s_=k_fa->B;
...
// inputStr
static type_(Array_<char16_t>) k_ea(){
type_(Array_<char16_t>) k_fq; // buf
int64_t k_fr; // ptr
char16_t k_fv; // c
(k_fq) = (newArrays_<type_(Array_<char16_t>)>((1024LL)));
(k_fr) = (0LL);
while(true){
(k_fv) = ((k_fw)());
if((k_fv) == (u'\uFFFF')){
...
if((k_fr) == ((k_fq)->Len())){
(k_fq) = ((k_fq)->Cat(newArrays_<type_(Array_<char16_t>)>(((k_fq)->Len()))));
}
((k_fq)->At(k_fr)) = (k_fv);
(k_fr) = ((k_fr) + (1LL));
}
return k_ez<type_(Array_<char16_t>)>((k_fq), (nullptr), (0LL), (k_fr));
}
// _toInt
static int64_t k_ed(type_(Array_<char16_t>) k_gj /*me_*/, bool* k_gk /*success*/){
...
while(true){
(k_gs) = ((k_em)());
for(k_gw = (0LL), k_gx = (((k_al)->Len()) - (1LL)); k_gw <= k_gx; k_gw += (1LL)){
if(((k_gs) == ((k_al)->At(k_gw))) || (((k_gs) == (u'\u000D')) && (((k_al)->At(k_gw)) == (u'\u000A')))){
return u'\u0000';
}
}
if((k_gs) == (u'\u000D')){
