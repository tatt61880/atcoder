...
static void k_i();
static void k_j();
static void k_l(int64_t k_br /*excpt*/);
static void k_p(type_(k_o) k_bu /*me*/);
static void k_q(k_o* k_bv);
static int64_t k_r(type_(k_o) k_bw /*me*/, type_(k_o) k_bx /*t*/);
static type_(k_o) k_s(type_(k_o) k_by /*me*/);
static type_(Array_<uint8_t>) k_t(type_(k_o) k_cb /*me*/);
static type_(k_o) k_u(type_(k_o) k_cd /*me*/, type_(Array_<uint8_t>) k_ce /*bin*/, int64_t* k_cf /*idx*/);
static type_(Array_<char16_t>) k_v(type_(k_o) k_ci /*me*/);
static void k_w(type_(k_n) k_cj /*me*/);
using k_cl=void(*)(type_(k_o));
static int64_t k_x(type_(k_n) k_cm /*me*/, int64_t k_cn /*min*/, int64_t k_co /*max*/);
using k_cw=uint64_t(*)(type_(k_n));
static uint64_t k_y(type_(k_n) k_cz /*me*/);
using k_db=uint32_t(*)(type_(k_n));
static double k_z(type_(k_n) k_de /*me*/, double k_df /*min*/, double k_dg /*max*/);
static void k_aa(type_(k_n) k_dj /*me*/, uint32_t k_dk /*seed*/);
static uint32_t k_ab(type_(k_n) k_dq /*me*/);
static void k_ac(k_n* k_ds);
static type_(k_n) k_ad(type_(k_n) k_dt);
static type_(Array_<uint8_t>) k_ae(type_(k_n) k_dw);
static type_(k_n) k_af(type_(k_n) k_ec, type_(Array_<uint8_t>) k_ed, int64_t* k_ee);
static type_(k_n) k_ag(type_(k_n) k_eh /*me2*/, uint32_t k_ei /*seed*/);
using k_ej=void(*)(type_(k_n), uint32_t);
template<typename T_>static T_ k_ai(T_ k_ek /*me_*/, type_(Array_<int64_t>) k_el /*type*/, T_ k_em /*n*/);
static int64_t k_aj();
static int64_t k_ak();
static int64_t k_an();
template<typename T_,typename R_>static R_ k_bh(T_ k_eu /*me_*/, type_(Array_<int64_t>) k_ev /*type*/);
template<typename T_,typename R_>static R_ k_bi(T_ k_ew /*me_*/, type_(Array_<int64_t>) k_ex /*type*/);
static int64_t k_bo(int64_t k_ey /*me_*/);
static void k_bp(type_(Array_<char16_t>) k_ez /*str*/);
template<typename T_>static type_(Array_<char16_t>) k_bq(T_ k_fa /*me_*/, type_(Array_<int64_t>) k_fb /*type*/);
template<typename T_>static T_ k_dc(T_ k_fc /*me_*/, type_(Array_<int64_t>) k_fd /*type*/, T_ k_fe /*n*/);
template<typename T_>static T_ k_dd(T_ k_ff /*me_*/, type_(Array_<int64_t>) k_fg /*type*/, int64_t k_fh /*n*/);
template<typename T_>static T_ k_dp(T_ k_fk /*me_*/, type_(Array_<int64_t>) k_fl /*type*/, int64_t k_fm /*n*/);
static type_(Array_<char16_t>) k_eo();
static int64_t k_er(type_(Array_<char16_t>) k_gj /*me_*/, bool* k_gk /*success*/);
static char16_t k_fv();
template<typename T_>static T_ k_gi(T_ k_gz /*me_*/, type_(Array_<int64_t>) k_ha /*type*/, int64_t k_hb /*start*/, int64_t k_hc /*len*/);
static char16_t k_gp();
...
k_n::k_n(): k_o(), k_dl(), k_dm(), k_dn(), k_do(){ Y = 8; }
k_n::~k_n(){
k_ac(this);
}
// k_main_
...
int64_t k_am; // n
int64_t k_ao; // q
type_(Array_<int64_t>) k_ap; // x
type_(Array_<int64_t>) k_aq; // y
int64_t k_au;
int64_t k_aw; // nx
int64_t k_ax; // ny
int64_t k_ay; // minX
...
((k_ap)->At(k_au)) = ((k_an)());
((k_aq)->At(k_au)) = ((k_an)());
(k_aw) = (((k_ap)->At(k_au)) - ((k_aq)->At(k_au)));
(k_ax) = (((k_ap)->At(k_au)) + ((k_aq)->At(k_au)));
((k_ap)->At(k_au)) = (k_aw);
((k_aq)->At(k_au)) = (k_ax);
}
(k_ay) = ((k_ap)->At(0LL));
...
(k_ba) = ((k_aq)->At(0LL));
(k_bb) = ((k_aq)->At(0LL));
for(k_bf = (1LL), k_bg = ((k_am) - (1LL)); k_bf <= k_bg; k_bf += (1LL)){
(k_ay) = (k_bh<type_(Array_<int64_t>), int64_t>((new_(Array_<int64_t>)((k_ay), ((k_ap)->At(k_bf)))), (nullptr)));
(k_az) = (k_bi<type_(Array_<int64_t>), int64_t>((new_(Array_<int64_t>)((k_az), ((k_ap)->At(k_bf)))), (nullptr)));
(k_ba) = (k_bh<type_(Array_<int64_t>), int64_t>((new_(Array_<int64_t>)((k_ba), ((k_aq)->At(k_bf)))), (nullptr)));
(k_bb) = (k_bi<type_(Array_<int64_t>), int64_t>((new_(Array_<int64_t>)((k_bb), ((k_aq)->At(k_bf)))), (nullptr)));
}
for(k_bk = (1LL), k_bl = (k_ao); k_bk <= k_bl; k_bk += (1LL)){
(k_bm) = (((k_an)()) - (1LL));
(k_bn) = (0LL);
(k_bn) = (k_bi<type_(Array_<int64_t>), int64_t>((new_(Array_<int64_t>)((k_bn), ((k_bo)((((k_ap)->At(k_bm)) - (k_ay)))))), (nullptr)));
(k_bn) = (k_bi<type_(Array_<int64_t>), int64_t>((new_(Array_<int64_t>)((k_bn), ((k_bo)((((k_ap)->At(k_bm)) - (k_az)))))), (nullptr)));
(k_bn) = (k_bi<type_(Array_<int64_t>), int64_t>((new_(Array_<int64_t>)((k_bn), ((k_bo)((((k_aq)->At(k_bm)) - (k_ba)))))), (nullptr)));
(k_bn) = (k_bi<type_(Array_<int64_t>), int64_t>((new_(Array_<int64_t>)((k_bn), ((k_bo)((((k_aq)->At(k_bm)) - (k_bb)))))), (nullptr)));
(k_bp)((((newArrayBin_<char16_t>(0,""))->Cat(k_bq<int64_t>((k_bn), (nullptr))))->Cat(newArrayBin_<char16_t>(1,"\x0A\x00"))));
}
}
// _finVars
static void k_i(){
...
return as_<k_o>(classTable_, (k_bz), 0);
}
// _toBin
static type_(Array_<uint8_t>) k_t(type_(k_o) k_cb /*me*/){
type_(Array_<uint8_t>) k_cc;
(k_cc) = (newArrays_<type_(Array_<uint8_t>)>((0LL)));
return k_cc;
}
// _fromBin
static type_(k_o) k_u(type_(k_o) k_cd /*me*/, type_(Array_<uint8_t>) k_ce /*bin*/, int64_t* k_cf /*idx*/){
type_(k_o) k_cg;
type_(k_o) k_ch;
(k_cg) = (k_ch = new_(k_o)(), k_p(k_ch), k_ch);
return k_cg;
}
// toStr
static type_(Array_<char16_t>) k_v(type_(k_o) k_ci /*me*/){
...
uint64_t k_cs; // m
uint64_t k_ct; // r
(k_cr) = (static_cast<uint64_t>(((k_co) - (k_cn)) + (1LL)));
(k_cs) = ((0x0000000000000000ULL) - (((0x0000000000000000ULL) - (k_cr)) % (k_cr)));
(k_ct) = (0x0000000000000000ULL);
if((k_cs) == (0x0000000000000000ULL)){
(k_ct) = ((reinterpret_cast<k_cw>(classTable_[(k_cm)->Y + 9]))((k_cm)));
}
else{
do{
(k_ct) = ((reinterpret_cast<k_cw>(classTable_[(k_cm)->Y + 9]))((k_cm)));
...
return (static_cast<int64_t>((k_ct) % (k_cr))) + (k_cn);
}
// rndBit64
static uint64_t k_y(type_(k_n) k_cz /*me*/){
uint32_t k_da; // t
(k_da) = ((reinterpret_cast<k_db>(classTable_[(k_cz)->Y + 12]))((k_cz)));
return k_dc<uint64_t>((k_dd<uint64_t>((static_cast<uint64_t>(k_da)), (nullptr), (32LL))), (nullptr), (static_cast<uint64_t>((reinterpret_cast<k_db>(classTable_[(k_cz)->Y + 12]))((k_cz)))));
}
// rndFloat
static double k_z(type_(k_n) k_de /*me*/, double k_df /*min*/, double k_dg /*max*/){
return (((static_cast<double>((reinterpret_cast<k_cw>(classTable_[(k_de)->Y + 9]))((k_de)))) / (1.844674407370955e+19)) * ((k_dg) - (k_df))) + (k_df);
}
// setSeed
static void k_aa(type_(k_n) k_dj /*me*/, uint32_t k_dk /*seed*/){
((k_dj)->k_dl) = (0x075BCD15U);
((k_dj)->k_dm) = (0x159A55E5U);
((k_dj)->k_dn) = ((0x1F123BB5U) * (k_dk));
((k_dj)->k_do) = ((0x05491333U) * (k_dp<uint32_t>(((k_dj)->k_dn), (nullptr), (1LL))));
}
// xs128
static uint32_t k_ab(type_(k_n) k_dq /*me*/){
uint32_t k_dr; // t
(k_dr) = (k_ai<uint32_t>(((k_dq)->k_dl), (nullptr), (k_dd<uint32_t>(((k_dq)->k_dl), (nullptr), (11LL)))));
((k_dq)->k_dl) = ((k_dq)->k_dm);
((k_dq)->k_dm) = ((k_dq)->k_dn);
((k_dq)->k_dn) = ((k_dq)->k_do);
((k_dq)->k_do) = (k_ai<uint32_t>((k_ai<uint32_t>(((k_dq)->k_do), (nullptr), (k_dp<uint32_t>(((k_dq)->k_do), (nullptr), (19LL))))), (nullptr), (k_ai<uint32_t>((k_dr), (nullptr), (k_dp<uint32_t>((k_dr), (nullptr), (8LL)))))));
return (k_dq)->k_do;
}
// _dtor
static void k_ac(k_n* k_ds){
...
type_(k_n) k_du;
type_(k_n) k_dv;
(k_du) = (k_dv = new_(k_n)(), k_w(k_dv), k_dv);
((k_du)->k_dl) = ((k_dt)->k_dl);
((k_du)->k_dm) = ((k_dt)->k_dm);
((k_du)->k_dn) = ((k_dt)->k_dn);
((k_du)->k_do) = ((k_dt)->k_do);
return as_<k_n>(classTable_, (k_du), 8);
}
// _toBin
static type_(Array_<uint8_t>) k_ae(type_(k_n) k_dw){
type_(Array_<uint8_t>) k_dx;
(k_dx) = (newArrays_<type_(Array_<uint8_t>)>((0LL)));
(k_dx) = ((k_dx)->Cat(toBin_<uint32_t>()(((k_dw)->k_dl))));
(k_dx) = ((k_dx)->Cat(toBin_<uint32_t>()(((k_dw)->k_dm))));
(k_dx) = ((k_dx)->Cat(toBin_<uint32_t>()(((k_dw)->k_dn))));
(k_dx) = ((k_dx)->Cat(toBin_<uint32_t>()(((k_dw)->k_do))));
return k_dx;
}
// _fromBin
static type_(k_n) k_af(type_(k_n) k_ec, type_(Array_<uint8_t>) k_ed, int64_t* k_ee){
type_(k_n) k_ef;
type_(k_n) k_eg;
(k_ef) = (k_eg = new_(k_n)(), k_w(k_eg), k_eg);
((k_ef)->k_dl) = (fromBin_<uint32_t>()((k_ed), (*k_ee)));
((k_ef)->k_dm) = (fromBin_<uint32_t>()((k_ed), (*k_ee)));
((k_ef)->k_dn) = (fromBin_<uint32_t>()((k_ed), (*k_ee)));
((k_ef)->k_do) = (fromBin_<uint32_t>()((k_ed), (*k_ee)));
return k_ef;
}
// makeRnd
static type_(k_n) k_ag(type_(k_n) k_eh /*me2*/, uint32_t k_ei /*seed*/){
(reinterpret_cast<k_ej>(classTable_[(k_eh)->Y + 11]))((k_eh), (k_ei));
return k_eh;
}
// _xor
template<typename T_>static T_ k_ai(T_ k_ek /*me_*/, type_(Array_<int64_t>) k_el /*type*/, T_ k_em /*n*/){
...
if(cmp_(r_,k_eu->B[i_])>0)r_=k_eu->B[i_];
}
return r_;
}
// _max
...
auto r_=k_ew->B[0];
for(int64_t i_=1;i_<l_;i_++){
if(cmp_(r_,k_ew->B[i_])<0)r_=k_ew->B[i_];
}
return r_;
}
// _absInt
static int64_t k_bo(int64_t k_ey /*me_*/){
return ((k_ey) >= (0LL)) ? (k_ey) : (-(k_ey));
}
// print
static void k_bp(type_(Array_<char16_t>) k_ez /*str*/){
if(k_ez==nullptr){
...
#if defined(_WIN32)
fputws(reinterpret_cast<const wchar_t*>(k_ez->B),stdout);
#else
std::u16string s_=k_ez->B;
...
// inputStr
static type_(Array_<char16_t>) k_eo(){
type_(Array_<char16_t>) k_fp; // buf
int64_t k_fq; // ptr
char16_t k_fu; // c
(k_fp) = (newArrays_<type_(Array_<char16_t>)>((1024LL)));
(k_fq) = (0LL);
while(true){
(k_fu) = ((k_fv)());
if((k_fu) == (u'\uFFFF')){
...
if((k_fq) == ((k_fp)->Len())){
(k_fp) = ((k_fp)->Cat(newArrays_<type_(Array_<char16_t>)>(((k_fp)->Len()))));
}
((k_fp)->At(k_fq)) = (k_fu);
(k_fq) = ((k_fq) + (1LL));
}
return k_gi<type_(Array_<char16_t>)>((k_fp), (nullptr), (0LL), (k_fq));
}
// _toInt
static int64_t k_er(type_(Array_<char16_t>) k_gj /*me_*/, bool* k_gk /*success*/){
std::u16string u_=k_gj->B;
...
while(true){
(k_go) = ((k_gp)());
for(k_gt = (0LL), k_gu = (((k_al)->Len()) - (1LL)); k_gt <= k_gu; k_gt += (1LL)){
if(((k_go) == ((k_al)->At(k_gt))) || (((k_go) == (u'\u000D')) && (((k_al)->At(k_gt)) == (u'\u000A')))){
return u'\u0000';
}
}
if((k_go) == (u'\u000D')){
