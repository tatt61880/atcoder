...
static int64_t k_ak();
static int64_t k_an();
static type_(Array_<bool>) k_aq(int64_t k_er /*n*/);
static void k_bm(type_(Array_<char16_t>) k_fg /*str*/);
template<typename T_>static type_(Array_<char16_t>) k_bn(T_ k_fh /*me_*/, type_(Array_<int64_t>) k_fi /*type*/);
template<typename T_>static T_ k_cz(T_ k_fj /*me_*/, type_(Array_<int64_t>) k_fk /*type*/, T_ k_fl /*n*/);
template<typename T_>static T_ k_da(T_ k_fm /*me_*/, type_(Array_<int64_t>) k_fn /*type*/, int64_t k_fo /*n*/);
template<typename T_>static T_ k_dm(T_ k_fr /*me_*/, type_(Array_<int64_t>) k_fs /*type*/, int64_t k_ft /*n*/);
static type_(Array_<char16_t>) k_el();
static int64_t k_eo(type_(Array_<char16_t>) k_gq /*me_*/, bool* k_gr /*success*/);
template<typename T_>static T_ k_eu(T_ k_gs /*me_*/, type_(Array_<int64_t>) k_gt /*type*/, int64_t k_gu /*len*/);
static char16_t k_gc();
template<typename T_>static T_ k_gp(T_ k_hl /*me_*/, type_(Array_<int64_t>) k_hm /*type*/, int64_t k_hn /*start*/, int64_t k_ho /*len*/);
static char16_t k_hb();
...
int64_t k_am; // n
int64_t k_ao; // k
type_(Array_<bool>) k_ap; // isPrime
type_(Array_<int64_t>) k_ar; // a
int64_t k_av;
int64_t k_az; // j
int64_t k_bd; // ans
int64_t k_bh;
...
(k_ao) = ((k_an)());
(k_ap) = ((k_aq)((k_am)));
(k_ar) = (newArrays_<type_(Array_<int64_t>)>(((k_am) + (1LL))));
for(k_av = (2LL), k_aw = (k_am); k_av <= k_aw; k_av += (1LL)){
if((k_ap)->At(k_av)){
(k_az) = (k_av);
while((k_az) <= (k_am)){
((k_ar)->At(k_az)) = (((k_ar)->At(k_az)) + (1LL));
(k_az) = ((k_az) + (k_av));
}
}
}
(k_bd) = (0LL);
for(k_bh = (2LL), k_bi = (k_am); k_bh <= k_bi; k_bh += (1LL)){
if(((k_ar)->At(k_bh)) >= (k_ao)){
(k_bd) = ((k_bd) + (1LL));
}
}
(k_bm)((((newArrayBin_<char16_t>(0,""))->Cat(k_bn<int64_t>((k_bd), (nullptr))))->Cat(newArrayBin_<char16_t>(1,"\x0A\x00"))));
}
// _finVars
static void k_i(){
(k_al) = (nullptr);
...
// sieve
static type_(Array_<bool>) k_aq(int64_t k_er /*n*/){
type_(Array_<bool>) k_ev; // isPrime
int64_t k_ez;
int64_t k_fd; // j
int64_t k_fa;
if((k_er) < (2LL)){
return k_eu<type_(Array_<bool>)>((new_(Array_<bool>)((false))), (nullptr), ((k_er) + (1LL)));
}
(k_ev) = (k_eu<type_(Array_<bool>)>((new_(Array_<bool>)((true))), (nullptr), ((k_er) + (1LL))));
((k_ev)->At(0LL)) = (false);
((k_ev)->At(1LL)) = (false);
for(k_ez = (2LL), k_fa = (k_er); k_ez <= k_fa; k_ez += (1LL)){
if((k_ev)->At(k_ez)){
(k_fd) = ((k_ez) * (k_ez));
while((k_fd) <= (k_er)){
((k_ev)->At(k_fd)) = (false);
...
// inputStr
static type_(Array_<char16_t>) k_el(){
type_(Array_<char16_t>) k_fw; // buf
int64_t k_fx; // ptr
char16_t k_gb; // c
(k_fw) = (newArrays_<type_(Array_<char16_t>)>((1024LL)));
(k_fx) = (0LL);
while(true){
(k_gb) = ((k_gc)());
if((k_gb) == (u'\uFFFF')){
...
if((k_fx) == ((k_fw)->Len())){
(k_fw) = ((k_fw)->Cat(newArrays_<type_(Array_<char16_t>)>(((k_fw)->Len()))));
}
((k_fw)->At(k_fx)) = (k_gb);
(k_fx) = ((k_fx) + (1LL));
}
return k_gp<type_(Array_<char16_t>)>((k_fw), (nullptr), (0LL), (k_fx));
}
// _toInt
static int64_t k_eo(type_(Array_<char16_t>) k_gq /*me_*/, bool* k_gr /*success*/){
std::u16string u_=k_gq->B;
...
while(true){
(k_ha) = ((k_hb)());
for(k_hf = (0LL), k_hg = (((k_al)->Len()) - (1LL)); k_hf <= k_hg; k_hf += (1LL)){
if(((k_ha) == ((k_al)->At(k_hf))) || (((k_ha) == (u'\u000D')) && (((k_al)->At(k_hf)) == (u'\u000A')))){
return u'\u0000';
}
}
if((k_ha) == (u'\u000D')){
