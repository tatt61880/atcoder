...
#endif
//{{{ This C++ code is transpiled from Kuin code by Kuin Programming Language v.2021.4.17
#if defined(_WIN32)
...
using k_cl=uint64_t(*)(type_(k_n));
static uint64_t k_y(type_(k_n) k_co /*me*/);
using k_cq=uint32_t(*)(type_(k_n));
static double k_z(type_(k_n) k_ct /*me*/, double k_cu /*min*/, double k_cv /*max*/);
static void k_aa(type_(k_n) k_cy /*me*/, uint32_t k_cz /*seed*/);
static uint32_t k_ab(type_(k_n) k_df /*me*/);
static void k_ac(k_n* k_dh);
static type_(k_n) k_ad(type_(k_n) k_di);
static type_(Array_<uint8_t>) k_ae(type_(k_n) k_dl);
static type_(k_n) k_af(type_(k_n) k_dr, type_(Array_<uint8_t>) k_ds, int64_t* k_dt);
static type_(k_n) k_ag(type_(k_n) k_dw /*me2*/, uint32_t k_dx /*seed*/);
using k_dy=void(*)(type_(k_n), uint32_t);
template<typename T_>static T_ k_ai(T_ k_dz /*me_*/, type_(Array_<int64_t>) k_ea /*type*/, T_ k_eb /*n*/);
static int64_t k_aj();
static int64_t k_ak();
static int64_t k_an();
template<typename T_,typename C_>static void k_bb(T_ k_ej /*me_*/, type_(Array_<int64_t>) k_ek /*type*/, C_ k_el /*item*/);
static int64_t k_bd(type_(Array_<type_(List_<int64_t>)>) k_em /*graph*/);
static void k_be(type_(Array_<char16_t>) k_ep /*str*/);
template<typename T_>static type_(Array_<char16_t>) k_bf(T_ k_eq /*me_*/, type_(Array_<int64_t>) k_er /*type*/);
template<typename T_>static T_ k_cr(T_ k_es /*me_*/, type_(Array_<int64_t>) k_et /*type*/, T_ k_eu /*n*/);
template<typename T_>static T_ k_cs(T_ k_ev /*me_*/, type_(Array_<int64_t>) k_ew /*type*/, int64_t k_ex /*n*/);
template<typename T_>static T_ k_de(T_ k_fa /*me_*/, type_(Array_<int64_t>) k_fb /*type*/, int64_t k_fc /*n*/);
static type_(Array_<char16_t>) k_ed();
static int64_t k_eg(type_(Array_<char16_t>) k_fz /*me_*/, bool* k_ga /*success*/);
static int64_t k_eo(type_(Array_<type_(List_<int64_t>)>) k_gb /*graph*/, int64_t* k_gc /*pos*/, int64_t k_gd /*p*/);
static char16_t k_fl();
template<typename T_>static T_ k_fy(T_ k_hf /*me_*/, type_(Array_<int64_t>) k_hg /*type*/, int64_t k_hh /*start*/, int64_t k_hi /*len*/);
template<typename T_>static void k_gg(T_ k_hj /*me_*/, type_(Array_<int64_t>) k_hk /*type*/);
template<typename T_>static bool k_gi(T_ k_hl /*me_*/, type_(Array_<int64_t>) k_hm /*type*/);
template<typename T_,typename R_>static R_ k_gk(T_ k_hn /*me_*/, type_(Array_<int64_t>) k_ho /*type*/);
template<typename T_>static void k_gq(T_ k_hr /*me_*/, type_(Array_<int64_t>) k_hs /*type*/);
...
k_n::k_n(): k_o(), k_da(), k_db(), k_dc(), k_dd(){ Y = 8; }
k_n::~k_n(){
k_ac(this);
}
// k_main_
...
static void k_g(){
int64_t k_am; // n
type_(Array_<type_(List_<int64_t>)>) k_ao; // graph
int64_t k_as;
int64_t k_ax;
int64_t k_az; // a
int64_t k_ba; // b
...
int64_t k_ay;
(k_am) = ((k_an)());
(k_ao) = (newArrays_<type_(Array_<type_(List_<int64_t>)>)>((k_am)));
for(k_as = (0LL), k_at = ((k_am) - (1LL)); k_as <= k_at; k_as += (1LL)){
((k_ao)->At(k_as)) = (new_(List_<int64_t>)());
}
for(k_ax = (0LL), k_ay = ((k_am) - (2LL)); k_ax <= k_ay; k_ax += (1LL)){
(k_az) = (((k_an)()) - (1LL));
(k_ba) = (((k_an)()) - (1LL));
k_bb<type_(List_<int64_t>), int64_t>(((k_ao)->At(k_az)), (nullptr), (k_ba));
k_bb<type_(List_<int64_t>), int64_t>(((k_ao)->At(k_ba)), (nullptr), (k_az));
}
(k_bc) = (((k_bd)((k_ao))) + (1LL));
(k_be)((((newArrayBin_<char16_t>(0,""))->Cat(k_bf<int64_t>((k_bc), (nullptr))))->Cat(newArrayBin_<char16_t>(1,"\x0A\x00"))));
}
// _finVars
static void k_i(){
(k_al) = (nullptr);
...
uint64_t k_ch; // m
uint64_t k_ci; // r
(k_cg) = (static_cast<uint64_t>(((k_cd) - (k_cc)) + (1LL)));
(k_ch) = ((0x0000000000000000ULL) - (((0x0000000000000000ULL) - (k_cg)) % (k_cg)));
(k_ci) = (0x0000000000000000ULL);
if((k_ch) == (0x0000000000000000ULL)){
(k_ci) = ((reinterpret_cast<k_cl>(classTable_[(k_cb)->Y + 9]))((k_cb)));
}
else{
do{
(k_ci) = ((reinterpret_cast<k_cl>(classTable_[(k_cb)->Y + 9]))((k_cb)));
...
return (static_cast<int64_t>((k_ci) % (k_cg))) + (k_cc);
}
// rndBit64
static uint64_t k_y(type_(k_n) k_co /*me*/){
uint32_t k_cp; // t
(k_cp) = ((reinterpret_cast<k_cq>(classTable_[(k_co)->Y + 12]))((k_co)));
return k_cr<uint64_t>((k_cs<uint64_t>((static_cast<uint64_t>(k_cp)), (nullptr), (32LL))), (nullptr), (static_cast<uint64_t>((reinterpret_cast<k_cq>(classTable_[(k_co)->Y + 12]))((k_co)))));
}
// rndFloat
static double k_z(type_(k_n) k_ct /*me*/, double k_cu /*min*/, double k_cv /*max*/){
return (((static_cast<double>((reinterpret_cast<k_cl>(classTable_[(k_ct)->Y + 9]))((k_ct)))) / (1.844674407370955e+19)) * ((k_cv) - (k_cu))) + (k_cu);
}
// setSeed
static void k_aa(type_(k_n) k_cy /*me*/, uint32_t k_cz /*seed*/){
((k_cy)->k_da) = (0x075BCD15U);
((k_cy)->k_db) = (0x159A55E5U);
((k_cy)->k_dc) = ((0x1F123BB5U) * (k_cz));
((k_cy)->k_dd) = ((0x05491333U) * (k_de<uint32_t>(((k_cy)->k_dc), (nullptr), (1LL))));
}
// xs128
static uint32_t k_ab(type_(k_n) k_df /*me*/){
uint32_t k_dg; // t
(k_dg) = (k_ai<uint32_t>(((k_df)->k_da), (nullptr), (k_cs<uint32_t>(((k_df)->k_da), (nullptr), (11LL)))));
((k_df)->k_da) = ((k_df)->k_db);
((k_df)->k_db) = ((k_df)->k_dc);
((k_df)->k_dc) = ((k_df)->k_dd);
((k_df)->k_dd) = (k_ai<uint32_t>((k_ai<uint32_t>(((k_df)->k_dd), (nullptr), (k_de<uint32_t>(((k_df)->k_dd), (nullptr), (19LL))))), (nullptr), (k_ai<uint32_t>((k_dg), (nullptr), (k_de<uint32_t>((k_dg), (nullptr), (8LL)))))));
return (k_df)->k_dd;
}
// _dtor
static void k_ac(k_n* k_dh){
...
((k_dj)->k_dc) = ((k_di)->k_dc);
((k_dj)->k_dd) = ((k_di)->k_dd);
return as_<k_n>(classTable_, (k_dj), 8);
}
// _toBin
static type_(Array_<uint8_t>) k_ae(type_(k_n) k_dl){
type_(Array_<uint8_t>) k_dm;
(k_dm) = (newArrays_<type_(Array_<uint8_t>)>((0LL)));
(k_dm) = ((k_dm)->Cat(toBin_<uint32_t>()(((k_dl)->k_da))));
(k_dm) = ((k_dm)->Cat(toBin_<uint32_t>()(((k_dl)->k_db))));
(k_dm) = ((k_dm)->Cat(toBin_<uint32_t>()(((k_dl)->k_dc))));
(k_dm) = ((k_dm)->Cat(toBin_<uint32_t>()(((k_dl)->k_dd))));
return k_dm;
}
// _fromBin
static type_(k_n) k_af(type_(k_n) k_dr, type_(Array_<uint8_t>) k_ds, int64_t* k_dt){
...
((k_du)->k_da) = (fromBin_<uint32_t>()((k_ds), (*k_dt)));
((k_du)->k_db) = (fromBin_<uint32_t>()((k_ds), (*k_dt)));
((k_du)->k_dc) = (fromBin_<uint32_t>()((k_ds), (*k_dt)));
((k_du)->k_dd) = (fromBin_<uint32_t>()((k_ds), (*k_dt)));
return k_du;
}
// makeRnd
static type_(k_n) k_ag(type_(k_n) k_dw /*me2*/, uint32_t k_dx /*seed*/){
(reinterpret_cast<k_dy>(classTable_[(k_dw)->Y + 11]))((k_dw), (k_dx));
return k_dw;
}
// _xor
template<typename T_>static T_ k_ai(T_ k_dz /*me_*/, type_(Array_<int64_t>) k_ea /*type*/, T_ k_eb /*n*/){
...
if((k_fg) == ((k_ff)->Len())){
(k_ff) = ((k_ff)->Cat(newArrays_<type_(Array_<char16_t>)>(((k_ff)->Len()))));
}
((k_ff)->At(k_fg)) = (k_fk);
(k_fg) = ((k_fg) + (1LL));
}
...
(k_ge) = (*k_gc);
(k_gf) = (0LL);
k_gg<type_(List_<int64_t>)>(((k_gb)->At(k_ge)), (nullptr));
while(!(k_gi<type_(List_<int64_t>)>(((k_gb)->At(k_ge)), (nullptr)))){
(k_gj) = (k_gk<type_(List_<int64_t>), int64_t>(((k_gb)->At(k_ge)), (nullptr)));
if((k_gj) != (k_gd)){
(k_gn) = (((k_eo)((k_gb), (&k_gj), (k_ge))) + (1LL));
if((k_gn) > (k_gf)){
(k_gf) = (k_gn);
(*k_gc) = (k_gj);
}
}
k_gq<type_(List_<int64_t>)>(((k_gb)->At(k_ge)), (nullptr));
}
return k_gf;
}
