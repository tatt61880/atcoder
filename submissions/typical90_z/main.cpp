...
static int64_t k_ak();
static int64_t k_an();
template<typename T_,typename C_>static void k_bb(T_ k_ex /*me_*/, type_(Array_<int64_t>) k_ey /*type*/, C_ k_ez /*item*/);
static void k_be(type_(Array_<type_(List_<int64_t>)>) k_fa /*graph*/, int64_t k_fb /*u*/, int64_t k_fc /*parent*/, bool k_fd /*f*/);
template<typename T_>static void k_bj(T_ k_fm /*me_*/, type_(Array_<int64_t>) k_fn /*type*/);
template<typename T_,typename R_>static R_ k_bq(T_ k_fo /*me_*/, type_(Array_<int64_t>) k_fp /*type*/);
static void k_br(type_(Array_<char16_t>) k_fs /*str*/);
template<typename T_>static type_(Array_<char16_t>) k_bs(T_ k_ft /*me_*/, type_(Array_<int64_t>) k_fu /*type*/);
template<typename T_>static void k_bt(T_ k_fv /*me_*/, type_(Array_<int64_t>) k_fw /*type*/);
template<typename T_>static T_ k_df(T_ k_fx /*me_*/, type_(Array_<int64_t>) k_fy /*type*/, T_ k_fz /*n*/);
template<typename T_>static T_ k_dg(T_ k_ga /*me_*/, type_(Array_<int64_t>) k_gb /*type*/, int64_t k_gc /*n*/);
template<typename T_>static T_ k_ds(T_ k_gf /*me_*/, type_(Array_<int64_t>) k_gg /*type*/, int64_t k_gh /*n*/);
static type_(Array_<char16_t>) k_er();
static int64_t k_eu(type_(Array_<char16_t>) k_he /*me_*/, bool* k_hf /*success*/);
template<typename T_>static bool k_fi(T_ k_hg /*me_*/, type_(Array_<int64_t>) k_hh /*type*/);
static char16_t k_gq();
template<typename T_>static T_ k_hd(T_ k_hw /*me_*/, type_(Array_<int64_t>) k_hx /*type*/, int64_t k_hy /*start*/, int64_t k_hz /*len*/);
...
static type_(List_<int64_t>) k_bc;
static type_(List_<int64_t>) k_bd;
k_o::k_o(): Class_(){ Y = 0; }
k_o::~k_o(){
k_q(this);
}
...
int64_t k_az; // a
int64_t k_ba; // b
type_(List_<int64_t>) k_bf; // majority
int64_t k_bn;
int64_t k_bp; // ans
int64_t k_at;
int64_t k_ay;
...
(k_bc) = (new_(List_<int64_t>)());
(k_bd) = (new_(List_<int64_t>)());
(k_be)((k_ao), (0LL), (0LL), (true));
(k_bf) = (nullptr);
if(((k_bc)->Len()) >= ((k_am) / (2LL))){
(k_bf) = (k_bc);
}
else{
(k_bf) = (k_bd);
}
k_bj<type_(List_<int64_t>)>((k_bf), (nullptr));
for(k_bn = (1LL), k_bo = ((k_am) / (2LL)); k_bn <= k_bo; k_bn += (1LL)){
(k_bp) = ((k_bq<type_(List_<int64_t>), int64_t>((k_bf), (nullptr))) + (1LL));
(k_br)(((((newArrayBin_<char16_t>(0,""))->Cat(k_bs<int64_t>((k_bp), (nullptr))))->Cat(newArrayBin_<char16_t>(0,"")))->Cat(((k_bn) == ((k_am) / (2LL))) ? (newArrayBin_<char16_t>(1,"\x0A\x00")) : (newArrayBin_<char16_t>(1,"\x20\x00")))));
k_bt<type_(List_<int64_t>)>((k_bf), (nullptr));
}
}
// _finVars
static void k_i(){
...
// _addList
template<typename T_,typename C_>static void k_bb(T_ k_ex /*me_*/, type_(Array_<int64_t>) k_ey /*type*/, C_ k_ez /*item*/){
k_ex->B.push_back(k_ez);
}
// dfs
static void k_be(type_(Array_<type_(List_<int64_t>)>) k_fa /*graph*/, int64_t k_fb /*u*/, int64_t k_fc /*parent*/, bool k_fd /*f*/){
int64_t k_fj; // x
if(k_fd){
k_bb<type_(List_<int64_t>), int64_t>((k_bc), (nullptr), (k_fb));
}
else{
k_bb<type_(List_<int64_t>), int64_t>((k_bd), (nullptr), (k_fb));
}
k_bj<type_(List_<int64_t>)>(((k_fa)->At(k_fb)), (nullptr));
while(!(k_fi<type_(List_<int64_t>)>(((k_fa)->At(k_fb)), (nullptr)))){
(k_fj) = (k_bq<type_(List_<int64_t>), int64_t>(((k_fa)->At(k_fb)), (nullptr)));
if((k_fj) != (k_fc)){
(k_be)((k_fa), (k_fj), (k_fb), (!(k_fd)));
}
k_bt<type_(List_<int64_t>)>(((k_fa)->At(k_fb)), (nullptr));
}
}
// _head
template<typename T_>static void k_bj(T_ k_fm /*me_*/, type_(Array_<int64_t>) k_fn /*type*/){
...
// inputStr
static type_(Array_<char16_t>) k_er(){
type_(Array_<char16_t>) k_gk; // buf
int64_t k_gl; // ptr
char16_t k_gp; // c
(k_gk) = (newArrays_<type_(Array_<char16_t>)>((1024LL)));
(k_gl) = (0LL);
while(true){
(k_gp) = ((k_gq)());
if((k_gp) == (u'\uFFFF')){
...
if((k_gl) == ((k_gk)->Len())){
(k_gk) = ((k_gk)->Cat(newArrays_<type_(Array_<char16_t>)>(((k_gk)->Len()))));
}
((k_gk)->At(k_gl)) = (k_gp);
(k_gl) = ((k_gl) + (1LL));
}
return k_hd<type_(Array_<char16_t>)>((k_gk), (nullptr), (0LL), (k_gl));
}
// _toInt
static int64_t k_eu(type_(Array_<char16_t>) k_he /*me_*/, bool* k_hf /*success*/){
std::u16string u_=k_he->B;
...
while(true){
(k_hl) = ((k_hm)());
for(k_hq = (0LL), k_hr = (((k_al)->Len()) - (1LL)); k_hq <= k_hr; k_hq += (1LL)){
if(((k_hl) == ((k_al)->At(k_hq))) || (((k_hl) == (u'\u000D')) && (((k_al)->At(k_hq)) == (u'\u000A')))){
return u'\u0000';
}
}
if((k_hl) == (u'\u000D')){
