...
static void k_i();
static void k_j();
static void k_l(int64_t k_ay /*excpt*/);
static void k_p(type_(k_o) k_bb /*me*/);
static void k_q(k_o* k_bc);
static int64_t k_r(type_(k_o) k_bd /*me*/, type_(k_o) k_be /*t*/);
static type_(k_o) k_s(type_(k_o) k_bf /*me*/);
static type_(Array_<uint8_t>) k_t(type_(k_o) k_bi /*me*/);
static type_(k_o) k_u(type_(k_o) k_bk /*me*/, type_(Array_<uint8_t>) k_bl /*bin*/, int64_t* k_bm /*idx*/);
static type_(Array_<char16_t>) k_v(type_(k_o) k_bp /*me*/);
static void k_w(type_(k_n) k_bq /*me*/);
using k_bs=void(*)(type_(k_o));
static int64_t k_x(type_(k_n) k_bt /*me*/, int64_t k_bu /*min*/, int64_t k_bv /*max*/);
using k_cd=uint64_t(*)(type_(k_n));
static uint64_t k_y(type_(k_n) k_cg /*me*/);
using k_ci=uint32_t(*)(type_(k_n));
static double k_z(type_(k_n) k_cl /*me*/, double k_cm /*min*/, double k_cn /*max*/);
...
static int64_t k_ak();
static int64_t k_an();
static int64_t k_av(type_(Array_<int64_t>) k_eb /*a*/);
static void k_aw(type_(Array_<char16_t>) k_en /*str*/);
template<typename T_>static type_(Array_<char16_t>) k_ax(T_ k_eo /*me_*/, type_(Array_<int64_t>) k_ep /*type*/);
template<typename T_>static T_ k_cj(T_ k_eq /*me_*/, type_(Array_<int64_t>) k_er /*type*/, T_ k_es /*n*/);
template<typename T_>static T_ k_ck(T_ k_et /*me_*/, type_(Array_<int64_t>) k_eu /*type*/, int64_t k_ev /*n*/);
template<typename T_>static T_ k_cw(T_ k_ey /*me_*/, type_(Array_<int64_t>) k_ez /*type*/, int64_t k_fa /*n*/);
static type_(Array_<char16_t>) k_dv();
static int64_t k_dy(type_(Array_<char16_t>) k_fx /*me_*/, bool* k_fy /*success*/);
static type_(Array_<int64_t>) k_ed(type_(Array_<int64_t>) k_fz /*a*/);
template<typename T_>static void k_ee(T_ k_gl /*me_*/, type_(Array_<int64_t>) k_gm /*type*/);
template<typename T_,typename R_>static R_ k_em(T_ k_gn /*me_*/, type_(Array_<int64_t>) k_go /*type*/);
static char16_t k_fj();
template<typename T_>static T_ k_fw(T_ k_hd /*me_*/, type_(Array_<int64_t>) k_he /*type*/, int64_t k_hf /*start*/, int64_t k_hg /*len*/);
template<typename T_>static T_ k_gc(T_ k_hh /*me_*/, type_(Array_<int64_t>) k_hi /*type*/, int64_t k_hj /*len*/);
static int64_t k_gk(type_(Array_<int64_t>) k_hm /*arr*/, int64_t k_hn /*val*/);
static char16_t k_gt();
static int64_t k_hr(int64_t k_hw /*me_*/);
...
(k_au) = ((k_av)((k_ao)));
(k_aw)((((newArrayBin_<char16_t>(0,""))->Cat(k_ax<int64_t>((k_au), (nullptr))))->Cat(newArrayBin_<char16_t>(1,"\x0A\x00"))));
}
// _finVars
static void k_i(){
(k_al) = (nullptr);
...
return as_<k_o>(classTable_, (k_bg), 0);
}
// _toBin
static type_(Array_<uint8_t>) k_t(type_(k_o) k_bi /*me*/){
type_(Array_<uint8_t>) k_bj;
(k_bj) = (newArrays_<type_(Array_<uint8_t>)>((0LL)));
return k_bj;
}
// _fromBin
static type_(k_o) k_u(type_(k_o) k_bk /*me*/, type_(Array_<uint8_t>) k_bl /*bin*/, int64_t* k_bm /*idx*/){
...
uint64_t k_bz; // m
uint64_t k_ca; // r
(k_by) = (static_cast<uint64_t>(((k_bv) - (k_bu)) + (1LL)));
(k_bz) = ((0x0000000000000000ULL) - (((0x0000000000000000ULL) - (k_by)) % (k_by)));
(k_ca) = (0x0000000000000000ULL);
if((k_bz) == (0x0000000000000000ULL)){
(k_ca) = ((reinterpret_cast<k_cd>(classTable_[(k_bt)->Y + 9]))((k_bt)));
}
else{
do{
(k_ca) = ((reinterpret_cast<k_cd>(classTable_[(k_bt)->Y + 9]))((k_bt)));
...
return (static_cast<int64_t>((k_ca) % (k_by))) + (k_bu);
}
// rndBit64
static uint64_t k_y(type_(k_n) k_cg /*me*/){
uint32_t k_ch; // t
(k_ch) = ((reinterpret_cast<k_ci>(classTable_[(k_cg)->Y + 12]))((k_cg)));
return k_cj<uint64_t>((k_ck<uint64_t>((static_cast<uint64_t>(k_ch)), (nullptr), (32LL))), (nullptr), (static_cast<uint64_t>((reinterpret_cast<k_ci>(classTable_[(k_cg)->Y + 12]))((k_cg)))));
}
// rndFloat
static double k_z(type_(k_n) k_cl /*me*/, double k_cm /*min*/, double k_cn /*max*/){
return (((static_cast<double>((reinterpret_cast<k_cd>(classTable_[(k_cl)->Y + 9]))((k_cl)))) / (1.844674407370955e+19)) * ((k_cn) - (k_cm))) + (k_cm);
}
// setSeed
static void k_aa(type_(k_n) k_cq /*me*/, uint32_t k_cr /*seed*/){
...
static uint32_t k_ab(type_(k_n) k_cx /*me*/){
uint32_t k_cy; // t
(k_cy) = (k_ai<uint32_t>(((k_cx)->k_cs), (nullptr), (k_ck<uint32_t>(((k_cx)->k_cs), (nullptr), (11LL)))));
((k_cx)->k_cs) = ((k_cx)->k_ct);
((k_cx)->k_ct) = ((k_cx)->k_cu);
((k_cx)->k_cu) = ((k_cx)->k_cv);
...
// solve
static int64_t k_av(type_(Array_<int64_t>) k_eb /*a*/){
type_(Array_<int64_t>) k_ec; // lis
type_(Array_<int64_t>) k_ef; // revLis
int64_t k_eg; // res
int64_t k_ek;
int64_t k_el;
(k_ec) = ((k_ed)((k_eb)));
k_ee<type_(Array_<int64_t>)>((k_eb), (nullptr));
(k_ef) = ((k_ed)((k_eb)));
k_ee<type_(Array_<int64_t>)>((k_ef), (nullptr));
k_ee<type_(Array_<int64_t>)>((k_eb), (nullptr));
(k_eg) = (0LL);
for(k_ek = (0LL), k_el = (((k_eb)->Len()) - (1LL)); k_ek <= k_el; k_ek += (1LL)){
(k_eg) = (k_em<type_(Array_<int64_t>), int64_t>((new_(Array_<int64_t>)((k_eg), ((((k_ec)->At(k_ek)) + ((k_ef)->At(k_ek))) - (1LL)))), (nullptr)));
}
return k_eg;
}
// print
...
#if defined(_WIN32)
fputws(reinterpret_cast<const wchar_t*>(k_en->B),stdout);
#else
std::u16string s_=k_en->B;
...
static type_(Array_<int64_t>) k_ed(type_(Array_<int64_t>) k_fz /*a*/){
int64_t k_ga; // n
type_(Array_<int64_t>) k_gb; // buff
type_(Array_<int64_t>) k_gd; // lisArr
int64_t k_gh;
int64_t k_gj; // idx
int64_t k_gi;
(k_ga) = ((k_fz)->Len());
(k_gb) = (k_gc<type_(Array_<int64_t>)>((new_(Array_<int64_t>)((9223372036854775807LL))), (nullptr), ((k_ga) + (1LL))));
(k_gd) = (newArrays_<type_(Array_<int64_t>)>((k_ga)));
for(k_gh = (0LL), k_gi = ((k_ga) - (1LL)); k_gh <= k_gi; k_gh += (1LL)){
(k_gj) = ((k_gk)((k_gb), ((k_fz)->At(k_gh))));
((k_gb)->At(k_gj)) = ((k_fz)->At(k_gh));
((k_gd)->At(k_gh)) = ((k_gk)((k_gb), (9223372036854775807LL)));
}
return k_gd;
}
// _reverse
...
if(cmp_(r_,k_gn->B[i_])<0)r_=k_gn->B[i_];
}
return r_;
}
...
(k_ho) = (-1LL);
(k_hp) = (((k_hm)->Len()) - (1LL));
while(((k_hr)(((k_ho) - (k_hp)))) > (1LL)){
(k_hs) = (((k_ho) + (k_hp)) / (2LL));
if((k_hn) > ((k_hm)->At(k_hs))){
(k_ho) = (k_hs);
}
else{
(k_hp) = (k_hs);
