...
static void k_i();
static void k_j();
static void k_l(int64_t k_cd /*excpt*/);
static void k_p(type_(k_o) k_cg /*me*/);
static void k_q(k_o* k_ch);
static int64_t k_r(type_(k_o) k_ci /*me*/, type_(k_o) k_cj /*t*/);
static type_(k_o) k_s(type_(k_o) k_ck /*me*/);
static type_(Array_<uint8_t>) k_t(type_(k_o) k_cn /*me*/);
static type_(k_o) k_u(type_(k_o) k_cp /*me*/, type_(Array_<uint8_t>) k_cq /*bin*/, int64_t* k_cr /*idx*/);
static type_(Array_<char16_t>) k_v(type_(k_o) k_cu /*me*/);
static void k_w(type_(k_n) k_cv /*me*/);
using k_cx=void(*)(type_(k_o));
static int64_t k_x(type_(k_n) k_cy /*me*/, int64_t k_cz /*min*/, int64_t k_da /*max*/);
using k_di=uint64_t(*)(type_(k_n));
static uint64_t k_y(type_(k_n) k_dl /*me*/);
using k_dn=uint32_t(*)(type_(k_n));
static double k_z(type_(k_n) k_dq /*me*/, double k_dr /*min*/, double k_ds /*max*/);
static void k_aa(type_(k_n) k_dv /*me*/, uint32_t k_dw /*seed*/);
static uint32_t k_ab(type_(k_n) k_ec /*me*/);
static void k_ac(k_n* k_ee);
static type_(k_n) k_ad(type_(k_n) k_ef);
static type_(Array_<uint8_t>) k_ae(type_(k_n) k_ei);
static type_(k_n) k_af(type_(k_n) k_eo, type_(Array_<uint8_t>) k_ep, int64_t* k_eq);
static type_(k_n) k_ag(type_(k_n) k_et /*me2*/, uint32_t k_eu /*seed*/);
using k_ev=void(*)(type_(k_n), uint32_t);
template<typename T_>static T_ k_ai(T_ k_ew /*me_*/, type_(Array_<int64_t>) k_ex /*type*/, T_ k_ey /*n*/);
static int64_t k_aj();
static int64_t k_ak();
static int64_t k_an();
template<typename T_,typename C_>static void k_bb(T_ k_fg /*me_*/, type_(Array_<int64_t>) k_fh /*type*/, C_ k_fi /*item*/);
static void k_bl(type_(Array_<type_(List_<int64_t>)>) k_fj /*ab*/, type_(Array_<bool>) k_fk /*used*/, int64_t k_fl /*u*/, type_(List_<int64_t>) k_fm /*ordersList*/);
template<typename T_,typename C_>static type_(Array_<C_>) k_bn(T_ k_fy /*me_*/, type_(Array_<int64_t>) k_fz /*type*/);
template<typename T_>static void k_bo(T_ k_ga /*me_*/, type_(Array_<int64_t>) k_gb /*type*/);
static int64_t k_bz(type_(Array_<type_(List_<int64_t>)>) k_gc /*ba*/, type_(Array_<bool>) k_gd /*used*/, int64_t k_ge /*u*/);
static void k_cb(type_(Array_<char16_t>) k_gp /*str*/);
template<typename T_>static type_(Array_<char16_t>) k_cc(T_ k_gq /*me_*/, type_(Array_<int64_t>) k_gr /*type*/);
template<typename T_>static T_ k_do(T_ k_gs /*me_*/, type_(Array_<int64_t>) k_gt /*type*/, T_ k_gu /*n*/);
template<typename T_>static T_ k_dp(T_ k_gv /*me_*/, type_(Array_<int64_t>) k_gw /*type*/, int64_t k_gx /*n*/);
template<typename T_>static T_ k_eb(T_ k_ha /*me_*/, type_(Array_<int64_t>) k_hb /*type*/, int64_t k_hc /*n*/);
static type_(Array_<char16_t>) k_fa();
static int64_t k_fd(type_(Array_<char16_t>) k_hz /*me_*/, bool* k_ia /*success*/);
template<typename T_>static void k_fn(T_ k_ib /*me_*/, type_(Array_<int64_t>) k_ic /*type*/);
template<typename T_,typename R_>static R_ k_fu(T_ k_id /*me_*/, type_(Array_<int64_t>) k_ie /*type*/);
template<typename T_>static void k_fx(T_ k_ih /*me_*/, type_(Array_<int64_t>) k_ii /*type*/);
static char16_t k_hl();
template<typename T_>static T_ k_hy(T_ k_ix /*me_*/, type_(Array_<int64_t>) k_iy /*type*/, int64_t k_iz /*start*/, int64_t k_ja /*len*/);
static char16_t k_in();
...
k_n::k_n(): k_o(), k_dx(), k_dy(), k_dz(), k_ea(){ Y = 8; }
k_n::~k_n(){
k_ac(this);
}
// k_main_
...
int64_t k_am; // n
int64_t k_ao; // m
type_(Array_<type_(List_<int64_t>)>) k_ap; // ab
type_(Array_<type_(List_<int64_t>)>) k_aq; // ba
int64_t k_au;
int64_t k_ax;
int64_t k_az; // a
int64_t k_ba; // b
type_(Array_<bool>) k_bc; // used1
type_(List_<int64_t>) k_bd; // ordersList
int64_t k_bh;
type_(Array_<int64_t>) k_bm; // orders
type_(Array_<bool>) k_bp; // used2
int64_t k_bq; // ans
int64_t k_bu;
int64_t k_by; // res
int64_t k_av;
...
(k_am) = ((k_an)());
(k_ao) = ((k_an)());
(k_ap) = (newArrays_<type_(Array_<type_(List_<int64_t>)>)>((k_am)));
(k_aq) = (newArrays_<type_(Array_<type_(List_<int64_t>)>)>((k_am)));
for(k_au = (0LL), k_av = ((k_am) - (1LL)); k_au <= k_av; k_au += (1LL)){
((k_ap)->At(k_au)) = (new_(List_<int64_t>)());
((k_aq)->At(k_au)) = (new_(List_<int64_t>)());
}
for(k_ax = (1LL), k_ay = (k_ao); k_ax <= k_ay; k_ax += (1LL)){
(k_az) = (((k_an)()) - (1LL));
(k_ba) = (((k_an)()) - (1LL));
k_bb<type_(List_<int64_t>), int64_t>(((k_ap)->At(k_az)), (nullptr), (k_ba));
k_bb<type_(List_<int64_t>), int64_t>(((k_aq)->At(k_ba)), (nullptr), (k_az));
}
(k_bc) = (newArrays_<type_(Array_<bool>)>((k_am)));
(k_bd) = (new_(List_<int64_t>)());
for(k_bh = (0LL), k_bi = ((k_am) - (1LL)); k_bh <= k_bi; k_bh += (1LL)){
if(!((k_bc)->At(k_bh))){
(k_bl)((k_ap), (k_bc), (k_bh), (k_bd));
}
}
(k_bm) = (k_bn<type_(List_<int64_t>), int64_t>((k_bd), (nullptr)));
k_bo<type_(Array_<int64_t>)>((k_bm), (nullptr));
(k_bp) = (newArrays_<type_(Array_<bool>)>((k_am)));
(k_bq) = (0LL);
for(k_bu = (0LL), k_bv = ((k_am) - (1LL)); k_bu <= k_bv; k_bu += (1LL)){
if(!((k_bp)->At((k_bm)->At(k_bu)))){
(k_by) = ((k_bz)((k_aq), (k_bp), ((k_bm)->At(k_bu))));
(k_bq) = ((k_bq) + (((k_by) * ((k_by) - (1LL))) / (2LL)));
}
}
(k_cb)((((newArrayBin_<char16_t>(0,""))->Cat(k_cc<int64_t>((k_bq), (nullptr))))->Cat(newArrayBin_<char16_t>(1,"\x0A\x00"))));
}
// _finVars
static void k_i(){
(k_al) = (nullptr);
...
return as_<k_o>(classTable_, (k_cl), 0);
}
// _toBin
static type_(Array_<uint8_t>) k_t(type_(k_o) k_cn /*me*/){
type_(Array_<uint8_t>) k_co;
(k_co) = (newArrays_<type_(Array_<uint8_t>)>((0LL)));
return k_co;
}
// _fromBin
static type_(k_o) k_u(type_(k_o) k_cp /*me*/, type_(Array_<uint8_t>) k_cq /*bin*/, int64_t* k_cr /*idx*/){
...
uint64_t k_de; // m
uint64_t k_df; // r
(k_dd) = (static_cast<uint64_t>(((k_da) - (k_cz)) + (1LL)));
(k_de) = ((0x0000000000000000ULL) - (((0x0000000000000000ULL) - (k_dd)) % (k_dd)));
(k_df) = (0x0000000000000000ULL);
if((k_de) == (0x0000000000000000ULL)){
(k_df) = ((reinterpret_cast<k_di>(classTable_[(k_cy)->Y + 9]))((k_cy)));
}
else{
do{
(k_df) = ((reinterpret_cast<k_di>(classTable_[(k_cy)->Y + 9]))((k_cy)));
...
return (static_cast<int64_t>((k_df) % (k_dd))) + (k_cz);
}
// rndBit64
static uint64_t k_y(type_(k_n) k_dl /*me*/){
uint32_t k_dm; // t
(k_dm) = ((reinterpret_cast<k_dn>(classTable_[(k_dl)->Y + 12]))((k_dl)));
return k_do<uint64_t>((k_dp<uint64_t>((static_cast<uint64_t>(k_dm)), (nullptr), (32LL))), (nullptr), (static_cast<uint64_t>((reinterpret_cast<k_dn>(classTable_[(k_dl)->Y + 12]))((k_dl)))));
}
// rndFloat
static double k_z(type_(k_n) k_dq /*me*/, double k_dr /*min*/, double k_ds /*max*/){
return (((static_cast<double>((reinterpret_cast<k_di>(classTable_[(k_dq)->Y + 9]))((k_dq)))) / (1.844674407370955e+19)) * ((k_ds) - (k_dr))) + (k_dr);
}
// setSeed
static void k_aa(type_(k_n) k_dv /*me*/, uint32_t k_dw /*seed*/){
((k_dv)->k_dx) = (0x075BCD15U);
((k_dv)->k_dy) = (0x159A55E5U);
((k_dv)->k_dz) = ((0x1F123BB5U) * (k_dw));
((k_dv)->k_ea) = ((0x05491333U) * (k_eb<uint32_t>(((k_dv)->k_dz), (nullptr), (1LL))));
}
// xs128
static uint32_t k_ab(type_(k_n) k_ec /*me*/){
uint32_t k_ed; // t
(k_ed) = (k_ai<uint32_t>(((k_ec)->k_dx), (nullptr), (k_dp<uint32_t>(((k_ec)->k_dx), (nullptr), (11LL)))));
((k_ec)->k_dx) = ((k_ec)->k_dy);
((k_ec)->k_dy) = ((k_ec)->k_dz);
((k_ec)->k_dz) = ((k_ec)->k_ea);
((k_ec)->k_ea) = (k_ai<uint32_t>((k_ai<uint32_t>(((k_ec)->k_ea), (nullptr), (k_eb<uint32_t>(((k_ec)->k_ea), (nullptr), (19LL))))), (nullptr), (k_ai<uint32_t>((k_ed), (nullptr), (k_eb<uint32_t>((k_ed), (nullptr), (8LL)))))));
return (k_ec)->k_ea;
}
// _dtor
static void k_ac(k_n* k_ee){
...
type_(k_n) k_eg;
type_(k_n) k_eh;
(k_eg) = (k_eh = new_(k_n)(), k_w(k_eh), k_eh);
((k_eg)->k_dx) = ((k_ef)->k_dx);
((k_eg)->k_dy) = ((k_ef)->k_dy);
((k_eg)->k_dz) = ((k_ef)->k_dz);
((k_eg)->k_ea) = ((k_ef)->k_ea);
return as_<k_n>(classTable_, (k_eg), 8);
}
// _toBin
static type_(Array_<uint8_t>) k_ae(type_(k_n) k_ei){
type_(Array_<uint8_t>) k_ej;
(k_ej) = (newArrays_<type_(Array_<uint8_t>)>((0LL)));
(k_ej) = ((k_ej)->Cat(toBin_<uint32_t>()(((k_ei)->k_dx))));
(k_ej) = ((k_ej)->Cat(toBin_<uint32_t>()(((k_ei)->k_dy))));
(k_ej) = ((k_ej)->Cat(toBin_<uint32_t>()(((k_ei)->k_dz))));
(k_ej) = ((k_ej)->Cat(toBin_<uint32_t>()(((k_ei)->k_ea))));
return k_ej;
}
// _fromBin
static type_(k_n) k_af(type_(k_n) k_eo, type_(Array_<uint8_t>) k_ep, int64_t* k_eq){
type_(k_n) k_er;
type_(k_n) k_es;
(k_er) = (k_es = new_(k_n)(), k_w(k_es), k_es);
((k_er)->k_dx) = (fromBin_<uint32_t>()((k_ep), (*k_eq)));
((k_er)->k_dy) = (fromBin_<uint32_t>()((k_ep), (*k_eq)));
((k_er)->k_dz) = (fromBin_<uint32_t>()((k_ep), (*k_eq)));
((k_er)->k_ea) = (fromBin_<uint32_t>()((k_ep), (*k_eq)));
return k_er;
}
// makeRnd
static type_(k_n) k_ag(type_(k_n) k_et /*me2*/, uint32_t k_eu /*seed*/){
(reinterpret_cast<k_ev>(classTable_[(k_et)->Y + 11]))((k_et), (k_eu));
return k_et;
}
// _xor
template<typename T_>static T_ k_ai(T_ k_ew /*me_*/, type_(Array_<int64_t>) k_ex /*type*/, T_ k_ey /*n*/){
...
int64_t k_fs;
((k_fk)->At(k_fl)) = (true);
k_fn<type_(List_<int64_t>)>(((k_fj)->At(k_fl)), (nullptr));
for(k_fr = (0LL), k_fs = ((((k_fj)->At(k_fl))->Len()) - (1LL)); k_fr <= k_fs; k_fr += (1LL)){
(k_ft) = (k_fu<type_(List_<int64_t>), int64_t>(((k_fj)->At(k_fl)), (nullptr)));
if(!((k_fk)->At(k_ft))){
(k_bl)((k_fj), (k_fk), (k_ft), (k_fm));
}
k_fx<type_(List_<int64_t>)>(((k_fj)->At(k_fl)), (nullptr));
}
k_bb<type_(List_<int64_t>), int64_t>((k_fm), (nullptr), (k_fl));
}
// _toArray
template<typename T_,typename C_>static type_(Array_<C_>) k_bn(T_ k_fy /*me_*/, type_(Array_<int64_t>) k_fz /*type*/){
return toArray_<C_>(k_fy);}
// _reverse
template<typename T_>static void k_bo(T_ k_ga /*me_*/, type_(Array_<int64_t>) k_gb /*type*/){
reverse_(k_ga);
}
...
(k_gf) = (1LL);
((k_gd)->At(k_ge)) = (true);
k_fn<type_(List_<int64_t>)>(((k_gc)->At(k_ge)), (nullptr));
for(k_gj = (0LL), k_gk = ((((k_gc)->At(k_ge))->Len()) - (1LL)); k_gj <= k_gk; k_gj += (1LL)){
(k_gl) = (k_fu<type_(List_<int64_t>), int64_t>(((k_gc)->At(k_ge)), (nullptr)));
if(!((k_gd)->At(k_gl))){
(k_gf) = ((k_gf) + ((k_bz)((k_gc), (k_gd), (k_gl))));
}
k_fx<type_(List_<int64_t>)>(((k_gc)->At(k_ge)), (nullptr));
}
return k_gf;
}
// print
...
#if defined(_WIN32)
fputws(reinterpret_cast<const wchar_t*>(k_gp->B),stdout);
#else
std::u16string s_=k_gp->B;
...
// inputStr
static type_(Array_<char16_t>) k_fa(){
type_(Array_<char16_t>) k_hf; // buf
int64_t k_hg; // ptr
char16_t k_hk; // c
(k_hf) = (newArrays_<type_(Array_<char16_t>)>((1024LL)));
(k_hg) = (0LL);
while(true){
(k_hk) = ((k_hl)());
if((k_hk) == (u'\uFFFF')){
...
if((k_hg) == ((k_hf)->Len())){
(k_hf) = ((k_hf)->Cat(newArrays_<type_(Array_<char16_t>)>(((k_hf)->Len()))));
}
((k_hf)->At(k_hg)) = (k_hk);
(k_hg) = ((k_hg) + (1LL));
}
return k_hy<type_(Array_<char16_t>)>((k_hf), (nullptr), (0LL), (k_hg));
}
// _toInt
static int64_t k_fd(type_(Array_<char16_t>) k_hz /*me_*/, bool* k_ia /*success*/){
std::u16string u_=k_hz->B;
...
while(true){
(k_im) = ((k_in)());
for(k_ir = (0LL), k_is = (((k_al)->Len()) - (1LL)); k_ir <= k_is; k_ir += (1LL)){
if(((k_im) == ((k_al)->At(k_ir))) || (((k_im) == (u'\u000D')) && (((k_al)->At(k_ir)) == (u'\u000A')))){
return u'\u0000';
}
}
if((k_im) == (u'\u000D')){
