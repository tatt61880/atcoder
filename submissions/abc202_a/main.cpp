...
static void k_i();
static void k_j();
static void k_l(int64_t k_at /*excpt*/);
static void k_p(type_(k_o) k_aw /*me*/);
static void k_q(k_o* k_ax);
static int64_t k_r(type_(k_o) k_ay /*me*/, type_(k_o) k_az /*t*/);
static type_(k_o) k_s(type_(k_o) k_ba /*me*/);
static type_(Array_<uint8_t>) k_t(type_(k_o) k_bd /*me*/);
static type_(k_o) k_u(type_(k_o) k_bf /*me*/, type_(Array_<uint8_t>) k_bg /*bin*/, int64_t* k_bh /*idx*/);
static type_(Array_<char16_t>) k_v(type_(k_o) k_bk /*me*/);
static void k_w(type_(k_n) k_bl /*me*/);
using k_bn=void(*)(type_(k_o));
static int64_t k_x(type_(k_n) k_bo /*me*/, int64_t k_bp /*min*/, int64_t k_bq /*max*/);
...
static void k_aa(type_(k_n) k_cl /*me*/, uint32_t k_cm /*seed*/);
static uint32_t k_ab(type_(k_n) k_cs /*me*/);
static void k_ac(k_n* k_cu);
static type_(k_n) k_ad(type_(k_n) k_cv);
static type_(Array_<uint8_t>) k_ae(type_(k_n) k_cy);
static type_(k_n) k_af(type_(k_n) k_de, type_(Array_<uint8_t>) k_df, int64_t* k_dg);
static type_(k_n) k_ag(type_(k_n) k_dj /*me2*/, uint32_t k_dk /*seed*/);
using k_dl=void(*)(type_(k_n), uint32_t);
template<typename T_>static T_ k_ai(T_ k_dm /*me_*/, type_(Array_<int64_t>) k_dn /*type*/, T_ k_do /*n*/);
static int64_t k_aj();
static int64_t k_ak();
static int64_t k_an();
static void k_ar(type_(Array_<char16_t>) k_dw /*str*/);
template<typename T_>static type_(Array_<char16_t>) k_as(T_ k_dx /*me_*/, type_(Array_<int64_t>) k_dy /*type*/);
template<typename T_>static T_ k_ce(T_ k_dz /*me_*/, type_(Array_<int64_t>) k_ea /*type*/, T_ k_eb /*n*/);
template<typename T_>static T_ k_cf(T_ k_ec /*me_*/, type_(Array_<int64_t>) k_ed /*type*/, int64_t k_ee /*n*/);
template<typename T_>static T_ k_cr(T_ k_eh /*me_*/, type_(Array_<int64_t>) k_ei /*type*/, int64_t k_ej /*n*/);
static type_(Array_<char16_t>) k_dq();
static int64_t k_dt(type_(Array_<char16_t>) k_fg /*me_*/, bool* k_fh /*success*/);
static char16_t k_es();
template<typename T_>static T_ k_ff(T_ k_fw /*me_*/, type_(Array_<int64_t>) k_fx /*type*/, int64_t k_fy /*start*/, int64_t k_fz /*len*/);
...
k_n::k_n(): k_o(), k_cn(), k_co(), k_cp(), k_cq(){ Y = 8; }
k_n::~k_n(){
k_ac(this);
}
// k_main_
...
(k_ao) = ((k_an)());
(k_ap) = ((k_an)());
(k_aq) = ((((21LL) - (k_am)) - (k_ao)) - (k_ap));
(k_ar)((((newArrayBin_<char16_t>(0,""))->Cat(k_as<int64_t>((k_aq), (nullptr))))->Cat(newArrayBin_<char16_t>(1,"\x0A\x00"))));
}
// _finVars
static void k_i(){
(k_al) = (nullptr);
...
type_(k_o) k_bb;
type_(k_o) k_bc;
(k_bb) = (k_bc = new_(k_o)(), k_p(k_bc), k_bc);
return as_<k_o>(classTable_, (k_bb), 0);
}
// _toBin
static type_(Array_<uint8_t>) k_t(type_(k_o) k_bd /*me*/){
type_(Array_<uint8_t>) k_be;
(k_be) = (newArrays_<type_(Array_<uint8_t>)>((0LL)));
return k_be;
}
// _fromBin
static type_(k_o) k_u(type_(k_o) k_bf /*me*/, type_(Array_<uint8_t>) k_bg /*bin*/, int64_t* k_bh /*idx*/){
type_(k_o) k_bi;
type_(k_o) k_bj;
(k_bi) = (k_bj = new_(k_o)(), k_p(k_bj), k_bj);
return k_bi;
}
// toStr
static type_(Array_<char16_t>) k_v(type_(k_o) k_bk /*me*/){
...
return (static_cast<int64_t>((k_bv) % (k_bt))) + (k_bp);
}
// rndBit64
static uint64_t k_y(type_(k_n) k_cb /*me*/){
uint32_t k_cc; // t
...
// setSeed
static void k_aa(type_(k_n) k_cl /*me*/, uint32_t k_cm /*seed*/){
((k_cl)->k_cn) = (0x075BCD15U);
((k_cl)->k_co) = (0x159A55E5U);
((k_cl)->k_cp) = ((0x1F123BB5U) * (k_cm));
((k_cl)->k_cq) = ((0x05491333U) * (k_cr<uint32_t>(((k_cl)->k_cp), (nullptr), (1LL))));
}
// xs128
static uint32_t k_ab(type_(k_n) k_cs /*me*/){
uint32_t k_ct; // t
(k_ct) = (k_ai<uint32_t>(((k_cs)->k_cn), (nullptr), (k_cf<uint32_t>(((k_cs)->k_cn), (nullptr), (11LL)))));
((k_cs)->k_cn) = ((k_cs)->k_co);
((k_cs)->k_co) = ((k_cs)->k_cp);
((k_cs)->k_cp) = ((k_cs)->k_cq);
((k_cs)->k_cq) = (k_ai<uint32_t>((k_ai<uint32_t>(((k_cs)->k_cq), (nullptr), (k_cr<uint32_t>(((k_cs)->k_cq), (nullptr), (19LL))))), (nullptr), (k_ai<uint32_t>((k_ct), (nullptr), (k_cr<uint32_t>((k_ct), (nullptr), (8LL)))))));
return (k_cs)->k_cq;
}
// _dtor
static void k_ac(k_n* k_cu){
...
type_(k_n) k_cw;
type_(k_n) k_cx;
(k_cw) = (k_cx = new_(k_n)(), k_w(k_cx), k_cx);
((k_cw)->k_cn) = ((k_cv)->k_cn);
((k_cw)->k_co) = ((k_cv)->k_co);
((k_cw)->k_cp) = ((k_cv)->k_cp);
((k_cw)->k_cq) = ((k_cv)->k_cq);
return as_<k_n>(classTable_, (k_cw), 8);
}
// _toBin
static type_(Array_<uint8_t>) k_ae(type_(k_n) k_cy){
type_(Array_<uint8_t>) k_cz;
(k_cz) = (newArrays_<type_(Array_<uint8_t>)>((0LL)));
(k_cz) = ((k_cz)->Cat(toBin_<uint32_t>()(((k_cy)->k_cn))));
(k_cz) = ((k_cz)->Cat(toBin_<uint32_t>()(((k_cy)->k_co))));
(k_cz) = ((k_cz)->Cat(toBin_<uint32_t>()(((k_cy)->k_cp))));
(k_cz) = ((k_cz)->Cat(toBin_<uint32_t>()(((k_cy)->k_cq))));
return k_cz;
}
// _fromBin
static type_(k_n) k_af(type_(k_n) k_de, type_(Array_<uint8_t>) k_df, int64_t* k_dg){
...
((k_dh)->k_cn) = (fromBin_<uint32_t>()((k_df), (*k_dg)));
((k_dh)->k_co) = (fromBin_<uint32_t>()((k_df), (*k_dg)));
((k_dh)->k_cp) = (fromBin_<uint32_t>()((k_df), (*k_dg)));
((k_dh)->k_cq) = (fromBin_<uint32_t>()((k_df), (*k_dg)));
return k_dh;
}
// makeRnd
static type_(k_n) k_ag(type_(k_n) k_dj /*me2*/, uint32_t k_dk /*seed*/){
(reinterpret_cast<k_dl>(classTable_[(k_dj)->Y + 11]))((k_dj), (k_dk));
return k_dj;
}
// _xor
template<typename T_>static T_ k_ai(T_ k_dm /*me_*/, type_(Array_<int64_t>) k_dn /*type*/, T_ k_do /*n*/){
...
#if defined(_WIN32)
fputws(reinterpret_cast<const wchar_t*>(k_dw->B),stdout);
#else
std::u16string s_=k_dw->B;
...
// inputStr
static type_(Array_<char16_t>) k_dq(){
type_(Array_<char16_t>) k_em; // buf
int64_t k_en; // ptr
char16_t k_er; // c
(k_em) = (newArrays_<type_(Array_<char16_t>)>((1024LL)));
(k_en) = (0LL);
while(true){
(k_er) = ((k_es)());
if((k_er) == (u'\uFFFF')){
...
if((k_en) == ((k_em)->Len())){
(k_em) = ((k_em)->Cat(newArrays_<type_(Array_<char16_t>)>(((k_em)->Len()))));
}
((k_em)->At(k_en)) = (k_er);
(k_en) = ((k_en) + (1LL));
}
return k_ff<type_(Array_<char16_t>)>((k_em), (nullptr), (0LL), (k_en));
}
// _toInt
static int64_t k_dt(type_(Array_<char16_t>) k_fg /*me_*/, bool* k_fh /*success*/){
std::u16string u_=k_fg->B;
...
while(true){
(k_fl) = ((k_fm)());
for(k_fq = (0LL), k_fr = (((k_al)->Len()) - (1LL)); k_fq <= k_fr; k_fq += (1LL)){
if(((k_fl) == ((k_al)->At(k_fq))) || (((k_fl) == (u'\u000D')) && (((k_al)->At(k_fq)) == (u'\u000A')))){
return u'\u0000';
}
}
if((k_fl) == (u'\u000D')){
