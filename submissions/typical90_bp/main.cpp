...
static void k_f();
static void k_g();
using k_ba=type_(k_ar)(*)(type_(k_ar), int64_t);
using k_bo=void(*)(type_(k_ar), int64_t, int64_t);
using k_bz=int64_t(*)(type_(k_ar), int64_t, int64_t);
static void k_i();
static void k_j();
static void k_l(int64_t k_dc /*excpt*/);
static void k_p(type_(k_o) k_df /*me*/);
static void k_q(k_o* k_dg);
static int64_t k_r(type_(k_o) k_dh /*me*/, type_(k_o) k_di /*t*/);
static type_(k_o) k_s(type_(k_o) k_dj /*me*/);
static type_(Array_<uint8_t>) k_t(type_(k_o) k_dm /*me*/);
static type_(k_o) k_u(type_(k_o) k_do /*me*/, type_(Array_<uint8_t>) k_dp /*bin*/, int64_t* k_dq /*idx*/);
static type_(Array_<char16_t>) k_v(type_(k_o) k_dt /*me*/);
static void k_w(type_(k_n) k_du /*me*/);
using k_dw=void(*)(type_(k_o));
static int64_t k_x(type_(k_n) k_dx /*me*/, int64_t k_dy /*min*/, int64_t k_dz /*max*/);
using k_eh=uint64_t(*)(type_(k_n));
static uint64_t k_y(type_(k_n) k_ek /*me*/);
using k_em=uint32_t(*)(type_(k_n));
static double k_z(type_(k_n) k_ep /*me*/, double k_eq /*min*/, double k_er /*max*/);
static void k_aa(type_(k_n) k_eu /*me*/, uint32_t k_ev /*seed*/);
static uint32_t k_ab(type_(k_n) k_fb /*me*/);
static void k_ac(k_n* k_fd);
static type_(k_n) k_ad(type_(k_n) k_fe);
static type_(Array_<uint8_t>) k_ae(type_(k_n) k_fh);
static type_(k_n) k_af(type_(k_n) k_fn, type_(Array_<uint8_t>) k_fo, int64_t* k_fp);
static type_(k_n) k_ag(type_(k_n) k_fs /*me2*/, uint32_t k_ft /*seed*/);
using k_fu=void(*)(type_(k_n), uint32_t);
template<typename T_>static T_ k_ai(T_ k_fv /*me_*/, type_(Array_<int64_t>) k_fw /*type*/, T_ k_fx /*n*/);
static int64_t k_aj();
static int64_t k_ak();
static int64_t k_an();
static type_(k_ar) k_as(type_(k_ar) k_gf /*me*/, int64_t k_gg /*n*/);
static void k_at(type_(k_ar) k_gj /*me*/, int64_t k_gk /*i*/, int64_t k_gl /*v*/);
static int64_t k_au(type_(k_ar) k_gs /*me*/, int64_t k_gt /*i*/);
static int64_t k_av(type_(k_ar) k_gz /*me*/, int64_t k_ha /*l*/, int64_t k_hb /*r*/);
using k_hc=int64_t(*)(type_(k_ar), int64_t);
static void k_aw(k_ar* k_hd);
static type_(k_ar) k_ax(type_(k_ar) k_he);
static type_(Array_<uint8_t>) k_ay(type_(k_ar) k_hg);
static type_(k_ar) k_az(type_(k_ar) k_hk, type_(Array_<uint8_t>) k_hl, int64_t* k_hm);
static void k_bv(type_(Array_<char16_t>) k_ho /*str*/);
template<typename T_>static type_(Array_<char16_t>) k_bw(T_ k_hp /*me_*/, type_(Array_<int64_t>) k_hq /*type*/);
template<typename T_>static T_ k_en(T_ k_hr /*me_*/, type_(Array_<int64_t>) k_hs /*type*/, T_ k_ht /*n*/);
template<typename T_>static T_ k_eo(T_ k_hu /*me_*/, type_(Array_<int64_t>) k_hv /*type*/, int64_t k_hw /*n*/);
template<typename T_>static T_ k_fa(T_ k_hz /*me_*/, type_(Array_<int64_t>) k_ia /*type*/, int64_t k_ib /*n*/);
static type_(Array_<char16_t>) k_fz();
static int64_t k_gc(type_(Array_<char16_t>) k_iy /*me_*/, bool* k_iz /*success*/);
template<typename T_>static T_ k_gr(T_ k_ja /*me_*/, type_(Array_<int64_t>) k_jb /*type*/, T_ k_jc /*n*/);
static char16_t k_ik();
template<typename T_>static T_ k_ix(T_ k_jr /*me_*/, type_(Array_<int64_t>) k_js /*type*/, int64_t k_jt /*start*/, int64_t k_ju /*len*/);
static char16_t k_jh();
...
virtual ~k_ar();
int64_t k_gh; // n
type_(Array_<int64_t>) k_gi; // data
};
...
k_n::k_n(): k_o(), k_ew(), k_ex(), k_ey(), k_ez(){ Y = 8; }
k_n::~k_n(){
k_ac(this);
}
k_ar::k_ar(): k_o(), k_gh(), k_gi(){ Y = 21; }
k_ar::~k_ar(){
k_aw(this);
}
// k_main_
...
int64_t k_am; // n
int64_t k_ao; // q
type_(Array_<bool>) k_ap; // f
type_(k_ar) k_aq; // bitFlag
type_(k_ar) k_bb; // bitEven
type_(k_ar) k_bc; // bitOdd
int64_t k_be;
int64_t k_bg; // t
int64_t k_bh; // x
int64_t k_bi; // y
...
(k_am) = ((k_an)());
(k_ao) = ((k_an)());
(k_ap) = (newArrays_<type_(Array_<bool>)>((k_am)));
(k_aq) = ((reinterpret_cast<k_ba>(classTable_[(new_(k_ar)())->Y + 8]))((new_(k_ar)()), (k_am)));
(k_bb) = ((reinterpret_cast<k_ba>(classTable_[(new_(k_ar)())->Y + 8]))((new_(k_ar)()), (k_am)));
(k_bc) = ((reinterpret_cast<k_ba>(classTable_[(new_(k_ar)())->Y + 8]))((new_(k_ar)()), (k_am)));
for(k_be = (1LL), k_bf = (k_ao); k_be <= k_bf; k_be += (1LL)){
(k_bg) = ((k_an)());
(k_bh) = (((k_an)()) - (1LL));
(k_bi) = (((k_an)()) - (1LL));
(k_bj) = ((k_an)());
...
if(!((k_ap)->At(k_bh))){
((k_ap)->At(k_bh)) = (true);
(reinterpret_cast<k_bo>(classTable_[(k_aq)->Y + 9]))((k_aq), (k_bh), (1LL));
if(((k_bh) % (2LL)) == (0LL)){
(reinterpret_cast<k_bo>(classTable_[(k_bb)->Y + 9]))((k_bb), (k_bh), (k_bj));
}
else{
(reinterpret_cast<k_bo>(classTable_[(k_bc)->Y + 9]))((k_bc), (k_bh), (k_bj));
}
}
}
else{
if((k_bh) == (k_bi)){
(k_bv)((((newArrayBin_<char16_t>(0,""))->Cat(k_bw<int64_t>((k_bj), (nullptr))))->Cat(newArrayBin_<char16_t>(1,"\x0A\x00"))));
}
else if((k_bh) < (k_bi)){
if(((reinterpret_cast<k_bz>(classTable_[(k_aq)->Y + 11]))((k_aq), (k_bh), (k_bi))) == ((k_bi) - (k_bh))){
(k_cb) = (0LL);
if(((k_bi) % (2LL)) == (1LL)){
(k_cb) = ((k_cb) + (((reinterpret_cast<k_bz>(classTable_[(k_bb)->Y + 11]))((k_bb), (k_bh), (k_bi))) - ((reinterpret_cast<k_bz>(classTable_[(k_bc)->Y + 11]))((k_bc), (k_bh), (k_bi)))));
}
else{
(k_cb) = ((k_cb) - (((reinterpret_cast<k_bz>(classTable_[(k_bb)->Y + 11]))((k_bb), (k_bh), (k_bi))) - ((reinterpret_cast<k_bz>(classTable_[(k_bc)->Y + 11]))((k_bc), (k_bh), (k_bi)))));
}
if((((k_bi) - (k_bh)) % (2LL)) == (0LL)){
(k_cb) = ((k_cb) + (k_bj));
}
else{
(k_cb) = ((k_cb) - (k_bj));
}
(k_bv)((((newArrayBin_<char16_t>(0,""))->Cat(k_bw<int64_t>((k_cb), (nullptr))))->Cat(newArrayBin_<char16_t>(1,"\x0A\x00"))));
}
else{
(k_bv)((newArrayBin_<char16_t>(10,"\x41\x00\x6D\x00\x62\x00\x69\x00\x67\x00\x75\x00\x6F\x00\x75\x00\x73\x00\x0A\x00")));
}
}
else{
if(((reinterpret_cast<k_bz>(classTable_[(k_aq)->Y + 11]))((k_aq), (k_bi), (k_bh))) == ((k_bh) - (k_bi))){
(k_cq) = (0LL);
if(((k_bi) % (2LL)) == (1LL)){
(k_cq) = ((k_cq) + (((reinterpret_cast<k_bz>(classTable_[(k_bb)->Y + 11]))((k_bb), (k_bh), (k_bi))) - ((reinterpret_cast<k_bz>(classTable_[(k_bc)->Y + 11]))((k_bc), (k_bh), (k_bi)))));
}
else{
(k_cq) = ((k_cq) - (((reinterpret_cast<k_bz>(classTable_[(k_bb)->Y + 11]))((k_bb), (k_bh), (k_bi))) - ((reinterpret_cast<k_bz>(classTable_[(k_bc)->Y + 11]))((k_bc), (k_bh), (k_bi)))));
}
...
(k_bv)((((newArrayBin_<char16_t>(0,""))->Cat(k_bw<int64_t>((k_cq), (nullptr))))->Cat(newArrayBin_<char16_t>(1,"\x0A\x00"))));
}
else{
(k_bv)((newArrayBin_<char16_t>(10,"\x41\x00\x6D\x00\x62\x00\x69\x00\x67\x00\x75\x00\x6F\x00\x75\x00\x73\x00\x0A\x00")));
}
...
type_(k_o) k_dk;
type_(k_o) k_dl;
(k_dk) = (k_dl = new_(k_o)(), k_p(k_dl), k_dl);
return as_<k_o>(classTable_, (k_dk), 0);
}
// _toBin
static type_(Array_<uint8_t>) k_t(type_(k_o) k_dm /*me*/){
...
type_(k_o) k_dr;
type_(k_o) k_ds;
(k_dr) = (k_ds = new_(k_o)(), k_p(k_ds), k_ds);
return k_dr;
}
// toStr
static type_(Array_<char16_t>) k_v(type_(k_o) k_dt /*me*/){
...
uint64_t k_ed; // m
uint64_t k_ee; // r
(k_ec) = (static_cast<uint64_t>(((k_dz) - (k_dy)) + (1LL)));
(k_ed) = ((0x0000000000000000ULL) - (((0x0000000000000000ULL) - (k_ec)) % (k_ec)));
(k_ee) = (0x0000000000000000ULL);
if((k_ed) == (0x0000000000000000ULL)){
(k_ee) = ((reinterpret_cast<k_eh>(classTable_[(k_dx)->Y + 9]))((k_dx)));
}
else{
do{
(k_ee) = ((reinterpret_cast<k_eh>(classTable_[(k_dx)->Y + 9]))((k_dx)));
...
return (static_cast<int64_t>((k_ee) % (k_ec))) + (k_dy);
}
// rndBit64
static uint64_t k_y(type_(k_n) k_ek /*me*/){
uint32_t k_el; // t
(k_el) = ((reinterpret_cast<k_em>(classTable_[(k_ek)->Y + 12]))((k_ek)));
return k_en<uint64_t>((k_eo<uint64_t>((static_cast<uint64_t>(k_el)), (nullptr), (32LL))), (nullptr), (static_cast<uint64_t>((reinterpret_cast<k_em>(classTable_[(k_ek)->Y + 12]))((k_ek)))));
}
// rndFloat
static double k_z(type_(k_n) k_ep /*me*/, double k_eq /*min*/, double k_er /*max*/){
return (((static_cast<double>((reinterpret_cast<k_eh>(classTable_[(k_ep)->Y + 9]))((k_ep)))) / (1.844674407370955e+19)) * ((k_er) - (k_eq))) + (k_eq);
}
// setSeed
static void k_aa(type_(k_n) k_eu /*me*/, uint32_t k_ev /*seed*/){
((k_eu)->k_ew) = (0x075BCD15U);
((k_eu)->k_ex) = (0x159A55E5U);
((k_eu)->k_ey) = ((0x1F123BB5U) * (k_ev));
((k_eu)->k_ez) = ((0x05491333U) * (k_fa<uint32_t>(((k_eu)->k_ey), (nullptr), (1LL))));
}
// xs128
static uint32_t k_ab(type_(k_n) k_fb /*me*/){
uint32_t k_fc; // t
(k_fc) = (k_ai<uint32_t>(((k_fb)->k_ew), (nullptr), (k_eo<uint32_t>(((k_fb)->k_ew), (nullptr), (11LL)))));
((k_fb)->k_ew) = ((k_fb)->k_ex);
((k_fb)->k_ex) = ((k_fb)->k_ey);
((k_fb)->k_ey) = ((k_fb)->k_ez);
((k_fb)->k_ez) = (k_ai<uint32_t>((k_ai<uint32_t>(((k_fb)->k_ez), (nullptr), (k_fa<uint32_t>(((k_fb)->k_ez), (nullptr), (19LL))))), (nullptr), (k_ai<uint32_t>((k_fc), (nullptr), (k_fa<uint32_t>((k_fc), (nullptr), (8LL)))))));
return (k_fb)->k_ez;
}
// _dtor
static void k_ac(k_n* k_fd){
...
type_(k_n) k_ff;
type_(k_n) k_fg;
(k_ff) = (k_fg = new_(k_n)(), k_w(k_fg), k_fg);
((k_ff)->k_ew) = ((k_fe)->k_ew);
((k_ff)->k_ex) = ((k_fe)->k_ex);
((k_ff)->k_ey) = ((k_fe)->k_ey);
((k_ff)->k_ez) = ((k_fe)->k_ez);
return as_<k_n>(classTable_, (k_ff), 8);
}
// _toBin
static type_(Array_<uint8_t>) k_ae(type_(k_n) k_fh){
type_(Array_<uint8_t>) k_fi;
(k_fi) = (newArrays_<type_(Array_<uint8_t>)>((0LL)));
(k_fi) = ((k_fi)->Cat(toBin_<uint32_t>()(((k_fh)->k_ew))));
(k_fi) = ((k_fi)->Cat(toBin_<uint32_t>()(((k_fh)->k_ex))));
(k_fi) = ((k_fi)->Cat(toBin_<uint32_t>()(((k_fh)->k_ey))));
(k_fi) = ((k_fi)->Cat(toBin_<uint32_t>()(((k_fh)->k_ez))));
return k_fi;
}
// _fromBin
static type_(k_n) k_af(type_(k_n) k_fn, type_(Array_<uint8_t>) k_fo, int64_t* k_fp){
type_(k_n) k_fq;
type_(k_n) k_fr;
(k_fq) = (k_fr = new_(k_n)(), k_w(k_fr), k_fr);
((k_fq)->k_ew) = (fromBin_<uint32_t>()((k_fo), (*k_fp)));
((k_fq)->k_ex) = (fromBin_<uint32_t>()((k_fo), (*k_fp)));
((k_fq)->k_ey) = (fromBin_<uint32_t>()((k_fo), (*k_fp)));
((k_fq)->k_ez) = (fromBin_<uint32_t>()((k_fo), (*k_fp)));
return k_fq;
}
// makeRnd
static type_(k_n) k_ag(type_(k_n) k_fs /*me2*/, uint32_t k_ft /*seed*/){
(reinterpret_cast<k_fu>(classTable_[(k_fs)->Y + 11]))((k_fs), (k_ft));
return k_fs;
}
// _xor
template<typename T_>static T_ k_ai(T_ k_fv /*me_*/, type_(Array_<int64_t>) k_fw /*type*/, T_ k_fx /*n*/){
...
static type_(k_ar) k_as(type_(k_ar) k_gf /*me*/, int64_t k_gg /*n*/){
((k_gf)->k_gh) = (k_gg);
((k_gf)->k_gi) = (newArrays_<type_(Array_<int64_t>)>(((k_gg) + (1LL))));
return k_gf;
}
// add
static void k_at(type_(k_ar) k_gj /*me*/, int64_t k_gk /*i*/, int64_t k_gl /*v*/){
int64_t k_gm; // x
type_(Array_<int64_t>) k_gp;
(k_gm) = ((k_gk) + (1LL));
while((k_gm) <= ((k_gj)->k_gh)){
(k_gp) = ((k_gj)->k_gi);
((k_gp)->At(k_gm)) = (((k_gp)->At(k_gm)) + (k_gl));
(k_gm) = ((k_gm) + (static_cast<int64_t>(k_gr<uint64_t>((static_cast<uint64_t>(k_gm)), (nullptr), (static_cast<uint64_t>(-(k_gm)))))));
}
}
// sum
static int64_t k_au(type_(k_ar) k_gs /*me*/, int64_t k_gt /*i*/){
...
(k_gv) = (0LL);
while((k_gu) > (0LL)){
(k_gv) = ((k_gv) + (((k_gs)->k_gi)->At(k_gu)));
(k_gu) = ((k_gu) - (static_cast<int64_t>(k_gr<uint64_t>((static_cast<uint64_t>(k_gu)), (nullptr), (static_cast<uint64_t>(-(k_gu)))))));
}
return k_gv;
}
// query
static int64_t k_av(type_(k_ar) k_gz /*me*/, int64_t k_ha /*l*/, int64_t k_hb /*r*/){
return ((reinterpret_cast<k_hc>(classTable_[(k_gz)->Y + 10]))((k_gz), ((k_hb) - (1LL)))) - ((reinterpret_cast<k_hc>(classTable_[(k_gz)->Y + 10]))((k_gz), ((k_ha) - (1LL))));
}
// _dtor
static void k_aw(k_ar* k_hd){
((k_hd)->k_gi) = (nullptr);
...
(k_hf) = (new_(k_ar)());
((k_hf)->k_gh) = ((k_he)->k_gh);
((k_hf)->k_gi) = (copy_<type_(Array_<int64_t>)>()((k_he)->k_gi));
return as_<k_ar>(classTable_, (k_hf), 21);
}
// _toBin
static type_(Array_<uint8_t>) k_ay(type_(k_ar) k_hg){
type_(Array_<uint8_t>) k_hh;
(k_hh) = (newArrays_<type_(Array_<uint8_t>)>((0LL)));
(k_hh) = ((k_hh)->Cat(toBin_<int64_t>()(((k_hg)->k_gh))));
(k_hh) = ((k_hh)->Cat(toBin_<type_(Array_<int64_t>)>()(((k_hg)->k_gi))));
return k_hh;
}
// _fromBin
static type_(k_ar) k_az(type_(k_ar) k_hk, type_(Array_<uint8_t>) k_hl, int64_t* k_hm){
type_(k_ar) k_hn;
(k_hn) = (new_(k_ar)());
((k_hn)->k_gh) = (fromBin_<int64_t>()((k_hl), (*k_hm)));
((k_hn)->k_gi) = (fromBin_<type_(Array_<int64_t>)>()((k_hl), (*k_hm)));
return k_hn;
}
// print
static void k_bv(type_(Array_<char16_t>) k_ho /*str*/){
...
#if defined(_WIN32)
fputws(reinterpret_cast<const wchar_t*>(k_ho->B),stdout);
#else
std::u16string s_=k_ho->B;
...
// inputStr
static type_(Array_<char16_t>) k_fz(){
type_(Array_<char16_t>) k_ie; // buf
int64_t k_if; // ptr
char16_t k_ij; // c
(k_ie) = (newArrays_<type_(Array_<char16_t>)>((1024LL)));
(k_if) = (0LL);
while(true){
(k_ij) = ((k_ik)());
if((k_ij) == (u'\uFFFF')){
...
if((k_if) == ((k_ie)->Len())){
(k_ie) = ((k_ie)->Cat(newArrays_<type_(Array_<char16_t>)>(((k_ie)->Len()))));
}
((k_ie)->At(k_if)) = (k_ij);
(k_if) = ((k_if) + (1LL));
}
return k_ix<type_(Array_<char16_t>)>((k_ie), (nullptr), (0LL), (k_if));
}
// _toInt
static int64_t k_gc(type_(Array_<char16_t>) k_iy /*me_*/, bool* k_iz /*success*/){
std::u16string u_=k_iy->B;
...
while(true){
(k_jg) = ((k_jh)());
for(k_jl = (0LL), k_jm = (((k_al)->Len()) - (1LL)); k_jl <= k_jm; k_jl += (1LL)){
if(((k_jg) == ((k_al)->At(k_jl))) || (((k_jg) == (u'\u000D')) && (((k_al)->At(k_jl)) == (u'\u000A')))){
return u'\u0000';
}
}
if((k_jg) == (u'\u000D')){
...
classTable_[23] = reinterpret_cast<int64_t>(k_aw);
classTable_[24] = reinterpret_cast<int64_t>(k_r);
classTable_[25] = reinterpret_cast<int64_t>(k_ax);
classTable_[26] = reinterpret_cast<int64_t>(k_ay);
classTable_[27] = reinterpret_cast<int64_t>(k_az);
classTable_[28] = reinterpret_cast<int64_t>(k_v);
classTable_[29] = reinterpret_cast<int64_t>(k_as);
classTable_[30] = reinterpret_cast<int64_t>(k_at);
classTable_[31] = reinterpret_cast<int64_t>(k_au);
classTable_[32] = reinterpret_cast<int64_t>(k_av);
init_();
k_main_();
