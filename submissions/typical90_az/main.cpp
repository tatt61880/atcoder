...
(k_au) = (0LL);
for(k_ay = (0LL), k_az = (5LL); k_ay <= k_az; k_ay += (1LL)){
(k_au) = ((k_au) + ((k_an)()));
}
(k_ao) = ((k_ao) * (k_au));
(k_ao) = ((k_ao) % ((powI_((10LL), (9LL))) + (7LL)));
}
(k_bd)((((newArrayBin_<char16_t>(0,""))->Cat(k_be<int64_t>((k_ao), (nullptr))))->Cat(newArrayBin_<char16_t>(1,"\x0A\x00"))));
}
// _finVars
static void k_i(){
(k_al) = (nullptr);
