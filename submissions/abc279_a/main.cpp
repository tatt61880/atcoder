...
using k_cc=uint64_t(*)(type_(k_n));
static uint64_t k_y(type_(k_n) k_cf /*me*/);
using k_ch=uint32_t(*)(type_(k_n));
static double k_z(type_(k_n) k_ck /*me*/, double k_cl /*min*/, double k_cm /*max*/);
static void k_aa(type_(k_n) k_cp /*me*/, uint32_t k_cq /*seed*/);
static uint32_t k_ab(type_(k_n) k_cw /*me*/);
static void k_ac(k_n* k_cy);
static type_(k_n) k_ad(type_(k_n) k_cz);
static type_(Array_<uint8_t>) k_ae(type_(k_n) k_dc);
static type_(k_n) k_af(type_(k_n) k_di, type_(Array_<uint8_t>) k_dj, int64_t* k_dk);
static type_(k_n) k_ag(type_(k_n) k_dn /*me2*/, uint32_t k_do /*seed*/);
using k_dp=void(*)(type_(k_n), uint32_t);
template<typename T_>static T_ k_ai(T_ k_dq /*me_*/, type_(Array_<int64_t>) k_dr /*type*/, T_ k_ds /*n*/);
static int64_t k_aj();
static int64_t k_ak();
static type_(Array_<char16_t>) k_an();
static void k_av(type_(Array_<char16_t>) k_en /*str*/);
template<typename T_>static type_(Array_<char16_t>) k_aw(T_ k_eo /*me_*/, type_(Array_<int64_t>) k_ep /*type*/);
template<typename T_>static T_ k_ci(T_ k_eq /*me_*/, type_(Array_<int64_t>) k_er /*type*/, T_ k_es /*n*/);
template<typename T_>static T_ k_cj(T_ k_et /*me_*/, type_(Array_<int64_t>) k_eu /*type*/, int64_t k_ev /*n*/);
template<typename T_>static T_ k_cv(T_ k_ey /*me_*/, type_(Array_<int64_t>) k_ez /*type*/, int64_t k_fa /*n*/);
static char16_t k_dz();
template<typename T_>static T_ k_em(T_ k_fd /*me_*/, type_(Array_<int64_t>) k_fe /*type*/, int64_t k_ff /*start*/, int64_t k_fg /*len*/);
...
k_n::k_n(): k_o(), k_cr(), k_cs(), k_ct(), k_cu(){ Y = 8; }
k_n::~k_n(){
k_ac(this);
}
// k_main_
...
(k_ao) = ((k_ao) + ((((k_am)->At(k_as)) == (u'\u0076')) ? (1LL) : (2LL)));
}
(k_av)((((newArrayBin_<char16_t>(0,""))->Cat(k_aw<int64_t>((k_ao), (nullptr))))->Cat(newArrayBin_<char16_t>(1,"\x0A\x00"))));
}
// _finVars
static void k_i(){
(k_al) = (nullptr);
...
uint64_t k_by; // m
uint64_t k_bz; // r
(k_bx) = (static_cast<uint64_t>(((k_bu) - (k_bt)) + (1LL)));
(k_by) = ((0x0000000000000000ULL) - (((0x0000000000000000ULL) - (k_bx)) % (k_bx)));
(k_bz) = (0x0000000000000000ULL);
if((k_by) == (0x0000000000000000ULL)){
(k_bz) = ((reinterpret_cast<k_cc>(classTable_[(k_bs)->Y + 9]))((k_bs)));
}
else{
do{
(k_bz) = ((reinterpret_cast<k_cc>(classTable_[(k_bs)->Y + 9]))((k_bs)));
...
return (static_cast<int64_t>((k_bz) % (k_bx))) + (k_bt);
}
// rndBit64
static uint64_t k_y(type_(k_n) k_cf /*me*/){
uint32_t k_cg; // t
(k_cg) = ((reinterpret_cast<k_ch>(classTable_[(k_cf)->Y + 12]))((k_cf)));
return k_ci<uint64_t>((k_cj<uint64_t>((static_cast<uint64_t>(k_cg)), (nullptr), (32LL))), (nullptr), (static_cast<uint64_t>((reinterpret_cast<k_ch>(classTable_[(k_cf)->Y + 12]))((k_cf)))));
}
// rndFloat
static double k_z(type_(k_n) k_ck /*me*/, double k_cl /*min*/, double k_cm /*max*/){
return (((static_cast<double>((reinterpret_cast<k_cc>(classTable_[(k_ck)->Y + 9]))((k_ck)))) / (1.844674407370955e+19)) * ((k_cm) - (k_cl))) + (k_cl);
}
// setSeed
static void k_aa(type_(k_n) k_cp /*me*/, uint32_t k_cq /*seed*/){
((k_cp)->k_cr) = (0x075BCD15U);
((k_cp)->k_cs) = (0x159A55E5U);
((k_cp)->k_ct) = ((0x1F123BB5U) * (k_cq));
((k_cp)->k_cu) = ((0x05491333U) * (k_cv<uint32_t>(((k_cp)->k_ct), (nullptr), (1LL))));
}
// xs128
static uint32_t k_ab(type_(k_n) k_cw /*me*/){
uint32_t k_cx; // t
(k_cx) = (k_ai<uint32_t>(((k_cw)->k_cr), (nullptr), (k_cj<uint32_t>(((k_cw)->k_cr), (nullptr), (11LL)))));
((k_cw)->k_cr) = ((k_cw)->k_cs);
((k_cw)->k_cs) = ((k_cw)->k_ct);
((k_cw)->k_ct) = ((k_cw)->k_cu);
((k_cw)->k_cu) = (k_ai<uint32_t>((k_ai<uint32_t>(((k_cw)->k_cu), (nullptr), (k_cv<uint32_t>(((k_cw)->k_cu), (nullptr), (19LL))))), (nullptr), (k_ai<uint32_t>((k_cx), (nullptr), (k_cv<uint32_t>((k_cx), (nullptr), (8LL)))))));
return (k_cw)->k_cu;
}
// _dtor
static void k_ac(k_n* k_cy){
...
type_(k_n) k_da;
type_(k_n) k_db;
(k_da) = (k_db = new_(k_n)(), k_w(k_db), k_db);
((k_da)->k_cr) = ((k_cz)->k_cr);
((k_da)->k_cs) = ((k_cz)->k_cs);
((k_da)->k_ct) = ((k_cz)->k_ct);
((k_da)->k_cu) = ((k_cz)->k_cu);
return as_<k_n>(classTable_, (k_da), 8);
}
// _toBin
static type_(Array_<uint8_t>) k_ae(type_(k_n) k_dc){
type_(Array_<uint8_t>) k_dd;
(k_dd) = (newArrays_<type_(Array_<uint8_t>)>((0LL)));
(k_dd) = ((k_dd)->Cat(toBin_<uint32_t>()(((k_dc)->k_cr))));
(k_dd) = ((k_dd)->Cat(toBin_<uint32_t>()(((k_dc)->k_cs))));
(k_dd) = ((k_dd)->Cat(toBin_<uint32_t>()(((k_dc)->k_ct))));
(k_dd) = ((k_dd)->Cat(toBin_<uint32_t>()(((k_dc)->k_cu))));
return k_dd;
}
// _fromBin
static type_(k_n) k_af(type_(k_n) k_di, type_(Array_<uint8_t>) k_dj, int64_t* k_dk){
...
((k_dl)->k_cr) = (fromBin_<uint32_t>()((k_dj), (*k_dk)));
((k_dl)->k_cs) = (fromBin_<uint32_t>()((k_dj), (*k_dk)));
((k_dl)->k_ct) = (fromBin_<uint32_t>()((k_dj), (*k_dk)));
((k_dl)->k_cu) = (fromBin_<uint32_t>()((k_dj), (*k_dk)));
return k_dl;
}
// makeRnd
static type_(k_n) k_ag(type_(k_n) k_dn /*me2*/, uint32_t k_do /*seed*/){
(reinterpret_cast<k_dp>(classTable_[(k_dn)->Y + 11]))((k_dn), (k_do));
return k_dn;
}
// _xor
template<typename T_>static T_ k_ai(T_ k_dq /*me_*/, type_(Array_<int64_t>) k_dr /*type*/, T_ k_ds /*n*/){
...
// input
static type_(Array_<char16_t>) k_an(){
type_(Array_<char16_t>) k_dt; // buf
int64_t k_du; // ptr
char16_t k_dy; // c
(k_dt) = (newArrays_<type_(Array_<char16_t>)>((1024LL)));
(k_du) = (0LL);
while(true){
(k_dy) = ((k_dz)());
if((k_dy) == (u'\u000D')){
...
if((k_du) == ((k_dt)->Len())){
(k_dt) = ((k_dt)->Cat(newArrays_<type_(Array_<char16_t>)>(((k_dt)->Len()))));
}
((k_dt)->At(k_du)) = (k_dy);
(k_du) = ((k_du) + (1LL));
}
return k_em<type_(Array_<char16_t>)>((k_dt), (nullptr), (0LL), (k_du));
}
// print
static void k_av(type_(Array_<char16_t>) k_en /*str*/){
if(k_en==nullptr){
