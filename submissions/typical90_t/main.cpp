...
static void k_i();
static void k_j();
static void k_l(int64_t k_as /*excpt*/);
static void k_p(type_(k_o) k_av /*me*/);
static void k_q(k_o* k_aw);
static int64_t k_r(type_(k_o) k_ax /*me*/, type_(k_o) k_ay /*t*/);
static type_(k_o) k_s(type_(k_o) k_az /*me*/);
static type_(Array_<uint8_t>) k_t(type_(k_o) k_bc /*me*/);
static type_(k_o) k_u(type_(k_o) k_be /*me*/, type_(Array_<uint8_t>) k_bf /*bin*/, int64_t* k_bg /*idx*/);
static type_(Array_<char16_t>) k_v(type_(k_o) k_bj /*me*/);
static void k_w(type_(k_n) k_bk /*me*/);
using k_bm=void(*)(type_(k_o));
static int64_t k_x(type_(k_n) k_bn /*me*/, int64_t k_bo /*min*/, int64_t k_bp /*max*/);
...
static void k_aa(type_(k_n) k_ck /*me*/, uint32_t k_cl /*seed*/);
static uint32_t k_ab(type_(k_n) k_cr /*me*/);
static void k_ac(k_n* k_ct);
static type_(k_n) k_ad(type_(k_n) k_cu);
static type_(Array_<uint8_t>) k_ae(type_(k_n) k_cx);
static type_(k_n) k_af(type_(k_n) k_dd, type_(Array_<uint8_t>) k_de, int64_t* k_df);
static type_(k_n) k_ag(type_(k_n) k_di /*me2*/, uint32_t k_dj /*seed*/);
using k_dk=void(*)(type_(k_n), uint32_t);
template<typename T_>static T_ k_ai(T_ k_dl /*me_*/, type_(Array_<int64_t>) k_dm /*type*/, T_ k_dn /*n*/);
static int64_t k_aj();
static int64_t k_ak();
static int64_t k_an();
static void k_ar(type_(Array_<char16_t>) k_dv /*str*/);
template<typename T_>static T_ k_cd(T_ k_dw /*me_*/, type_(Array_<int64_t>) k_dx /*type*/, T_ k_dy /*n*/);
template<typename T_>static T_ k_ce(T_ k_dz /*me_*/, type_(Array_<int64_t>) k_ea /*type*/, int64_t k_eb /*n*/);
template<typename T_>static T_ k_cq(T_ k_ee /*me_*/, type_(Array_<int64_t>) k_ef /*type*/, int64_t k_eg /*n*/);
static type_(Array_<char16_t>) k_dp();
static int64_t k_ds(type_(Array_<char16_t>) k_fd /*me_*/, bool* k_fe /*success*/);
static char16_t k_ep();
template<typename T_>static T_ k_fc(T_ k_ft /*me_*/, type_(Array_<int64_t>) k_fu /*type*/, int64_t k_fv /*start*/, int64_t k_fw /*len*/);
...
k_n::k_n(): k_o(), k_cm(), k_cn(), k_co(), k_cp(){ Y = 8; }
k_n::~k_n(){
k_ac(this);
}
// k_main_
...
(k_ao) = ((k_an)());
(k_ap) = ((k_an)());
(k_aq) = ((k_am) < (powI_((k_ap), (k_ao))));
(k_ar)((((k_aq) ? (newArrayBin_<char16_t>(3,"\x59\x00\x65\x00\x73\x00")) : (newArrayBin_<char16_t>(2,"\x4E\x00\x6F\x00")))->Cat(newArrayBin_<char16_t>(1,"\x0A\x00"))));
}
// _finVars
static void k_i(){
(k_al) = (nullptr);
...
type_(k_o) k_ba;
type_(k_o) k_bb;
(k_ba) = (k_bb = new_(k_o)(), k_p(k_bb), k_bb);
return as_<k_o>(classTable_, (k_ba), 0);
}
// _toBin
static type_(Array_<uint8_t>) k_t(type_(k_o) k_bc /*me*/){
type_(Array_<uint8_t>) k_bd;
(k_bd) = (newArrays_<type_(Array_<uint8_t>)>((0LL)));
return k_bd;
}
// _fromBin
static type_(k_o) k_u(type_(k_o) k_be /*me*/, type_(Array_<uint8_t>) k_bf /*bin*/, int64_t* k_bg /*idx*/){
...
return (static_cast<int64_t>((k_bu) % (k_bs))) + (k_bo);
}
// rndBit64
static uint64_t k_y(type_(k_n) k_ca /*me*/){
uint32_t k_cb; // t
...
// setSeed
static void k_aa(type_(k_n) k_ck /*me*/, uint32_t k_cl /*seed*/){
((k_ck)->k_cm) = (0x075BCD15U);
((k_ck)->k_cn) = (0x159A55E5U);
((k_ck)->k_co) = ((0x1F123BB5U) * (k_cl));
((k_ck)->k_cp) = ((0x05491333U) * (k_cq<uint32_t>(((k_ck)->k_co), (nullptr), (1LL))));
}
// xs128
static uint32_t k_ab(type_(k_n) k_cr /*me*/){
uint32_t k_cs; // t
(k_cs) = (k_ai<uint32_t>(((k_cr)->k_cm), (nullptr), (k_ce<uint32_t>(((k_cr)->k_cm), (nullptr), (11LL)))));
((k_cr)->k_cm) = ((k_cr)->k_cn);
((k_cr)->k_cn) = ((k_cr)->k_co);
((k_cr)->k_co) = ((k_cr)->k_cp);
((k_cr)->k_cp) = (k_ai<uint32_t>((k_ai<uint32_t>(((k_cr)->k_cp), (nullptr), (k_cq<uint32_t>(((k_cr)->k_cp), (nullptr), (19LL))))), (nullptr), (k_ai<uint32_t>((k_cs), (nullptr), (k_cq<uint32_t>((k_cs), (nullptr), (8LL)))))));
return (k_cr)->k_cp;
}
// _dtor
static void k_ac(k_n* k_ct){
...
type_(k_n) k_cv;
type_(k_n) k_cw;
(k_cv) = (k_cw = new_(k_n)(), k_w(k_cw), k_cw);
((k_cv)->k_cm) = ((k_cu)->k_cm);
((k_cv)->k_cn) = ((k_cu)->k_cn);
((k_cv)->k_co) = ((k_cu)->k_co);
((k_cv)->k_cp) = ((k_cu)->k_cp);
return as_<k_n>(classTable_, (k_cv), 8);
}
// _toBin
static type_(Array_<uint8_t>) k_ae(type_(k_n) k_cx){
type_(Array_<uint8_t>) k_cy;
(k_cy) = (newArrays_<type_(Array_<uint8_t>)>((0LL)));
(k_cy) = ((k_cy)->Cat(toBin_<uint32_t>()(((k_cx)->k_cm))));
(k_cy) = ((k_cy)->Cat(toBin_<uint32_t>()(((k_cx)->k_cn))));
(k_cy) = ((k_cy)->Cat(toBin_<uint32_t>()(((k_cx)->k_co))));
(k_cy) = ((k_cy)->Cat(toBin_<uint32_t>()(((k_cx)->k_cp))));
return k_cy;
}
// _fromBin
static type_(k_n) k_af(type_(k_n) k_dd, type_(Array_<uint8_t>) k_de, int64_t* k_df){
...
((k_dg)->k_cm) = (fromBin_<uint32_t>()((k_de), (*k_df)));
((k_dg)->k_cn) = (fromBin_<uint32_t>()((k_de), (*k_df)));
((k_dg)->k_co) = (fromBin_<uint32_t>()((k_de), (*k_df)));
((k_dg)->k_cp) = (fromBin_<uint32_t>()((k_de), (*k_df)));
return k_dg;
}
// makeRnd
static type_(k_n) k_ag(type_(k_n) k_di /*me2*/, uint32_t k_dj /*seed*/){
(reinterpret_cast<k_dk>(classTable_[(k_di)->Y + 11]))((k_di), (k_dj));
return k_di;
}
// _xor
template<typename T_>static T_ k_ai(T_ k_dl /*me_*/, type_(Array_<int64_t>) k_dm /*type*/, T_ k_dn /*n*/){
...
#if defined(_WIN32)
fputws(reinterpret_cast<const wchar_t*>(k_dv->B),stdout);
#else
std::u16string s_=k_dv->B;
...
while(true){
(k_fi) = ((k_fj)());
for(k_fn = (0LL), k_fo = (((k_al)->Len()) - (1LL)); k_fn <= k_fo; k_fn += (1LL)){
if(((k_fi) == ((k_al)->At(k_fn))) || (((k_fi) == (u'\u000D')) && (((k_al)->At(k_fn)) == (u'\u000A')))){
return u'\u0000';
}
}
if((k_fi) == (u'\u000D')){
