...
func main()
	...
	var dic: dict<char, char> :: #dict<char, char>
	do dic.add('N', 'S')
	do dic.add('E', 'W')
	do dic.add('W', 'E')
	do dic.add('S', 'N')
...
static void k_i();
static void k_j();
static void k_l(int64_t k_bb /*excpt*/);
static void k_p(type_(k_o) k_be /*me*/);
static void k_q(k_o* k_bf);
static int64_t k_r(type_(k_o) k_bg /*me*/, type_(k_o) k_bh /*t*/);
static type_(k_o) k_s(type_(k_o) k_bi /*me*/);
static type_(Array_<uint8_t>) k_t(type_(k_o) k_bl /*me*/);
static type_(k_o) k_u(type_(k_o) k_bn /*me*/, type_(Array_<uint8_t>) k_bo /*bin*/, int64_t* k_bp /*idx*/);
static type_(Array_<char16_t>) k_v(type_(k_o) k_bs /*me*/);
static void k_w(type_(k_n) k_bt /*me*/);
using k_bv=void(*)(type_(k_o));
static int64_t k_x(type_(k_n) k_bw /*me*/, int64_t k_bx /*min*/, int64_t k_by /*max*/);
...
template<typename T_,typename K_,typename V_>static void k_ap(T_ k_er /*me_*/, type_(Array_<int64_t>) k_es /*type*/, K_ k_et /*key*/, V_ k_eu /*item*/);
...
static void k_az(type_(Array_<char16_t>) k_ez /*str*/);
template<typename T_>static type_(Array_<char16_t>) k_ba(T_ k_fa /*me_*/, type_(Array_<int64_t>) k_fb /*type*/);
template<typename T_>static T_ k_cm(T_ k_fc /*me_*/, type_(Array_<int64_t>) k_fd /*type*/, T_ k_fe /*n*/);
template<typename T_>static T_ k_cn(T_ k_ff /*me_*/, type_(Array_<int64_t>) k_fg /*type*/, int64_t k_fh /*n*/);
template<typename T_>static T_ k_cz(T_ k_fk /*me_*/, type_(Array_<int64_t>) k_fl /*type*/, int64_t k_fm /*n*/);
static char16_t k_ed();
template<typename T_>static T_ k_eq(T_ k_fp /*me_*/, type_(Array_<int64_t>) k_fq /*type*/, int64_t k_fr /*start*/, int64_t k_fs /*len*/);
...
type_(Dict_<char16_t, char16_t>) k_ao; // dic
type_(Array_<char16_t>) k_aq; // ans
int64_t k_au;
char16_t k_aw; // c
int64_t k_av;
bool k_ay;
(k_am) = ((k_an)());
(k_ao) = (new_(Dict_<char16_t, char16_t>)());
k_ap<type_(Dict_<char16_t, char16_t>), char16_t, char16_t>((k_ao), (nullptr), (u'\u004E'), (u'\u0053'));
k_ap<type_(Dict_<char16_t, char16_t>), char16_t, char16_t>((k_ao), (nullptr), (u'\u0045'), (u'\u0057'));
k_ap<type_(Dict_<char16_t, char16_t>), char16_t, char16_t>((k_ao), (nullptr), (u'\u0057'), (u'\u0045'));
k_ap<type_(Dict_<char16_t, char16_t>), char16_t, char16_t>((k_ao), (nullptr), (u'\u0053'), (u'\u004E'));
(k_aq) = (k_am);
for(k_au = (0LL), k_av = (((k_am)->Len()) - (1LL)); k_au <= k_av; k_au += (1LL)){
(k_aw) = ((k_am)->At(k_au));
((k_am)->At(k_au)) = (k_ax<type_(Dict_<char16_t, char16_t>), char16_t, char16_t>((k_ao), (nullptr), (k_aw), (std::memset(&k_ay, 0, sizeof(bool)), &k_ay)));
}
(k_az)((((newArrayBin_<char16_t>(0,""))->Cat(k_ba<type_(Array_<char16_t>)>((k_aq), (nullptr))))->Cat(newArrayBin_<char16_t>(1,"\x0A\x00"))));
}
// _finVars
static void k_i(){
(k_al) = (nullptr);
...
return as_<k_o>(classTable_, (k_bj), 0);
}
// _toBin
static type_(Array_<uint8_t>) k_t(type_(k_o) k_bl /*me*/){
type_(Array_<uint8_t>) k_bm;
(k_bm) = (newArrays_<type_(Array_<uint8_t>)>((0LL)));
return k_bm;
}
// _fromBin
static type_(k_o) k_u(type_(k_o) k_bn /*me*/, type_(Array_<uint8_t>) k_bo /*bin*/, int64_t* k_bp /*idx*/){
...
return (static_cast<int64_t>((k_cd) % (k_cb))) + (k_bx);
}
// rndBit64
static uint64_t k_y(type_(k_n) k_cj /*me*/){
uint32_t k_ck; // t
...
static uint32_t k_ab(type_(k_n) k_da /*me*/){
uint32_t k_db; // t
(k_db) = (k_ai<uint32_t>(((k_da)->k_cv), (nullptr), (k_cn<uint32_t>(((k_da)->k_cv), (nullptr), (11LL)))));
((k_da)->k_cv) = ((k_da)->k_cw);
((k_da)->k_cw) = ((k_da)->k_cx);
((k_da)->k_cx) = ((k_da)->k_cy);
...
// input
static type_(Array_<char16_t>) k_an(){
type_(Array_<char16_t>) k_dx; // buf
int64_t k_dy; // ptr
char16_t k_ec; // c
(k_dx) = (newArrays_<type_(Array_<char16_t>)>((1024LL)));
(k_dy) = (0LL);
while(true){
(k_ec) = ((k_ed)());
if((k_ec) == (u'\u000D')){
...
if((k_dy) == ((k_dx)->Len())){
(k_dx) = ((k_dx)->Cat(newArrays_<type_(Array_<char16_t>)>(((k_dx)->Len()))));
}
((k_dx)->At(k_dy)) = (k_ec);
(k_dy) = ((k_dy) + (1LL));
}
return k_eq<type_(Array_<char16_t>)>((k_dx), (nullptr), (0LL), (k_dy));
}
// _addDict
template<typename T_,typename K_,typename V_>static void k_ap(T_ k_er /*me_*/, type_(Array_<int64_t>) k_es /*type*/, K_ k_et /*key*/, V_ k_eu /*item*/){
k_er->Add(k_et,k_eu);
