...
static void k_i();
static void k_j();
static void k_l(int64_t k_be /*excpt*/);
static void k_p(type_(k_o) k_bh /*me*/);
static void k_q(k_o* k_bi);
static int64_t k_r(type_(k_o) k_bj /*me*/, type_(k_o) k_bk /*t*/);
static type_(k_o) k_s(type_(k_o) k_bl /*me*/);
static type_(Array_<uint8_t>) k_t(type_(k_o) k_bo /*me*/);
static type_(k_o) k_u(type_(k_o) k_bq /*me*/, type_(Array_<uint8_t>) k_br /*bin*/, int64_t* k_bs /*idx*/);
static type_(Array_<char16_t>) k_v(type_(k_o) k_bv /*me*/);
static void k_w(type_(k_n) k_bw /*me*/);
using k_by=void(*)(type_(k_o));
static int64_t k_x(type_(k_n) k_bz /*me*/, int64_t k_ca /*min*/, int64_t k_cb /*max*/);
using k_cj=uint64_t(*)(type_(k_n));
static uint64_t k_y(type_(k_n) k_cm /*me*/);
using k_co=uint32_t(*)(type_(k_n));
static double k_z(type_(k_n) k_cr /*me*/, double k_cs /*min*/, double k_ct /*max*/);
static void k_aa(type_(k_n) k_cw /*me*/, uint32_t k_cx /*seed*/);
static uint32_t k_ab(type_(k_n) k_dd /*me*/);
static void k_ac(k_n* k_df);
static type_(k_n) k_ad(type_(k_n) k_dg);
static type_(Array_<uint8_t>) k_ae(type_(k_n) k_dj);
static type_(k_n) k_af(type_(k_n) k_dp, type_(Array_<uint8_t>) k_dq, int64_t* k_dr);
static type_(k_n) k_ag(type_(k_n) k_du /*me2*/, uint32_t k_dv /*seed*/);
using k_dw=void(*)(type_(k_n), uint32_t);
template<typename T_>static T_ k_ai(T_ k_dx /*me_*/, type_(Array_<int64_t>) k_dy /*type*/, T_ k_dz /*n*/);
static int64_t k_aj();
static int64_t k_ak();
static int64_t k_an();
static void k_bc(type_(Array_<char16_t>) k_eh /*str*/);
template<typename T_>static type_(Array_<char16_t>) k_bd(T_ k_ei /*me_*/, type_(Array_<int64_t>) k_ej /*type*/);
template<typename T_>static T_ k_cp(T_ k_ek /*me_*/, type_(Array_<int64_t>) k_el /*type*/, T_ k_em /*n*/);
template<typename T_>static T_ k_cq(T_ k_en /*me_*/, type_(Array_<int64_t>) k_eo /*type*/, int64_t k_ep /*n*/);
template<typename T_>static T_ k_dc(T_ k_es /*me_*/, type_(Array_<int64_t>) k_et /*type*/, int64_t k_eu /*n*/);
static type_(Array_<char16_t>) k_eb();
static int64_t k_ee(type_(Array_<char16_t>) k_fr /*me_*/, bool* k_fs /*success*/);
static char16_t k_fd();
template<typename T_>static T_ k_fq(T_ k_gh /*me_*/, type_(Array_<int64_t>) k_gi /*type*/, int64_t k_gj /*start*/, int64_t k_gk /*len*/);
static char16_t k_fx();
...
k_n::k_n(): k_o(), k_cy(), k_cz(), k_da(), k_db(){ Y = 8; }
k_n::~k_n(){
k_ac(this);
}
// k_main_
...
(k_am) = ((k_an)());
(k_ao) = ((k_an)());
for(k_as = (0LL), k_at = (k_am); k_as <= k_at; k_as += (1LL)){
for(k_ax = (0LL), k_ay = ((k_am) - (k_as)); k_ax <= k_ay; k_ax += (1LL)){
(k_az) = (((k_am) - (k_as)) - (k_ax));
if(((((k_as) * (10000LL)) + ((k_ax) * (5000LL))) + ((k_az) * (1000LL))) == (k_ao)){
(k_bc)((((newArrayBin_<char16_t>(0,""))->Cat(k_bd<int64_t>((k_as), (nullptr))))->Cat(((newArrayBin_<char16_t>(1,"\x20\x00"))->Cat(k_bd<int64_t>((k_ax), (nullptr))))->Cat(((newArrayBin_<char16_t>(1,"\x20\x00"))->Cat(k_bd<int64_t>((k_az), (nullptr))))->Cat(newArrayBin_<char16_t>(1,"\x0A\x00"))))));
return;
}
}
}
(k_bc)((newArrayBin_<char16_t>(9,"\x2D\x00\x31\x00\x20\x00\x2D\x00\x31\x00\x20\x00\x2D\x00\x31\x00\x0A\x00")));
}
// _finVars
static void k_i(){
(k_al) = (nullptr);
...
return as_<k_o>(classTable_, (k_bm), 0);
}
// _toBin
static type_(Array_<uint8_t>) k_t(type_(k_o) k_bo /*me*/){
type_(Array_<uint8_t>) k_bp;
(k_bp) = (newArrays_<type_(Array_<uint8_t>)>((0LL)));
return k_bp;
}
// _fromBin
static type_(k_o) k_u(type_(k_o) k_bq /*me*/, type_(Array_<uint8_t>) k_br /*bin*/, int64_t* k_bs /*idx*/){
...
uint64_t k_cf; // m
uint64_t k_cg; // r
(k_ce) = (static_cast<uint64_t>(((k_cb) - (k_ca)) + (1LL)));
(k_cf) = ((0x0000000000000000ULL) - (((0x0000000000000000ULL) - (k_ce)) % (k_ce)));
(k_cg) = (0x0000000000000000ULL);
if((k_cf) == (0x0000000000000000ULL)){
(k_cg) = ((reinterpret_cast<k_cj>(classTable_[(k_bz)->Y + 9]))((k_bz)));
}
else{
do{
(k_cg) = ((reinterpret_cast<k_cj>(classTable_[(k_bz)->Y + 9]))((k_bz)));
...
return (static_cast<int64_t>((k_cg) % (k_ce))) + (k_ca);
}
// rndBit64
static uint64_t k_y(type_(k_n) k_cm /*me*/){
uint32_t k_cn; // t
(k_cn) = ((reinterpret_cast<k_co>(classTable_[(k_cm)->Y + 12]))((k_cm)));
return k_cp<uint64_t>((k_cq<uint64_t>((static_cast<uint64_t>(k_cn)), (nullptr), (32LL))), (nullptr), (static_cast<uint64_t>((reinterpret_cast<k_co>(classTable_[(k_cm)->Y + 12]))((k_cm)))));
}
// rndFloat
static double k_z(type_(k_n) k_cr /*me*/, double k_cs /*min*/, double k_ct /*max*/){
return (((static_cast<double>((reinterpret_cast<k_cj>(classTable_[(k_cr)->Y + 9]))((k_cr)))) / (1.844674407370955e+19)) * ((k_ct) - (k_cs))) + (k_cs);
}
// setSeed
static void k_aa(type_(k_n) k_cw /*me*/, uint32_t k_cx /*seed*/){
((k_cw)->k_cy) = (0x075BCD15U);
((k_cw)->k_cz) = (0x159A55E5U);
((k_cw)->k_da) = ((0x1F123BB5U) * (k_cx));
((k_cw)->k_db) = ((0x05491333U) * (k_dc<uint32_t>(((k_cw)->k_da), (nullptr), (1LL))));
}
// xs128
static uint32_t k_ab(type_(k_n) k_dd /*me*/){
uint32_t k_de; // t
(k_de) = (k_ai<uint32_t>(((k_dd)->k_cy), (nullptr), (k_cq<uint32_t>(((k_dd)->k_cy), (nullptr), (11LL)))));
((k_dd)->k_cy) = ((k_dd)->k_cz);
((k_dd)->k_cz) = ((k_dd)->k_da);
((k_dd)->k_da) = ((k_dd)->k_db);
((k_dd)->k_db) = (k_ai<uint32_t>((k_ai<uint32_t>(((k_dd)->k_db), (nullptr), (k_dc<uint32_t>(((k_dd)->k_db), (nullptr), (19LL))))), (nullptr), (k_ai<uint32_t>((k_de), (nullptr), (k_dc<uint32_t>((k_de), (nullptr), (8LL)))))));
return (k_dd)->k_db;
}
// _dtor
static void k_ac(k_n* k_df){
...
type_(k_n) k_dh;
type_(k_n) k_di;
(k_dh) = (k_di = new_(k_n)(), k_w(k_di), k_di);
((k_dh)->k_cy) = ((k_dg)->k_cy);
((k_dh)->k_cz) = ((k_dg)->k_cz);
((k_dh)->k_da) = ((k_dg)->k_da);
((k_dh)->k_db) = ((k_dg)->k_db);
return as_<k_n>(classTable_, (k_dh), 8);
}
// _toBin
static type_(Array_<uint8_t>) k_ae(type_(k_n) k_dj){
type_(Array_<uint8_t>) k_dk;
(k_dk) = (newArrays_<type_(Array_<uint8_t>)>((0LL)));
(k_dk) = ((k_dk)->Cat(toBin_<uint32_t>()(((k_dj)->k_cy))));
(k_dk) = ((k_dk)->Cat(toBin_<uint32_t>()(((k_dj)->k_cz))));
(k_dk) = ((k_dk)->Cat(toBin_<uint32_t>()(((k_dj)->k_da))));
(k_dk) = ((k_dk)->Cat(toBin_<uint32_t>()(((k_dj)->k_db))));
return k_dk;
}
// _fromBin
static type_(k_n) k_af(type_(k_n) k_dp, type_(Array_<uint8_t>) k_dq, int64_t* k_dr){
type_(k_n) k_ds;
type_(k_n) k_dt;
(k_ds) = (k_dt = new_(k_n)(), k_w(k_dt), k_dt);
((k_ds)->k_cy) = (fromBin_<uint32_t>()((k_dq), (*k_dr)));
((k_ds)->k_cz) = (fromBin_<uint32_t>()((k_dq), (*k_dr)));
((k_ds)->k_da) = (fromBin_<uint32_t>()((k_dq), (*k_dr)));
((k_ds)->k_db) = (fromBin_<uint32_t>()((k_dq), (*k_dr)));
return k_ds;
}
// makeRnd
static type_(k_n) k_ag(type_(k_n) k_du /*me2*/, uint32_t k_dv /*seed*/){
(reinterpret_cast<k_dw>(classTable_[(k_du)->Y + 11]))((k_du), (k_dv));
return k_du;
}
// _xor
template<typename T_>static T_ k_ai(T_ k_dx /*me_*/, type_(Array_<int64_t>) k_dy /*type*/, T_ k_dz /*n*/){
...
#if defined(_WIN32)
fputws(reinterpret_cast<const wchar_t*>(k_eh->B),stdout);
#else
std::u16string s_=k_eh->B;
...
// inputStr
static type_(Array_<char16_t>) k_eb(){
type_(Array_<char16_t>) k_ex; // buf
int64_t k_ey; // ptr
char16_t k_fc; // c
(k_ex) = (newArrays_<type_(Array_<char16_t>)>((1024LL)));
(k_ey) = (0LL);
while(true){
(k_fc) = ((k_fd)());
if((k_fc) == (u'\uFFFF')){
...
if((k_ey) == ((k_ex)->Len())){
(k_ex) = ((k_ex)->Cat(newArrays_<type_(Array_<char16_t>)>(((k_ex)->Len()))));
}
((k_ex)->At(k_ey)) = (k_fc);
(k_ey) = ((k_ey) + (1LL));
}
return k_fq<type_(Array_<char16_t>)>((k_ex), (nullptr), (0LL), (k_ey));
}
// _toInt
static int64_t k_ee(type_(Array_<char16_t>) k_fr /*me_*/, bool* k_fs /*success*/){
std::u16string u_=k_fr->B;
...
while(true){
(k_fw) = ((k_fx)());
for(k_gb = (0LL), k_gc = (((k_al)->Len()) - (1LL)); k_gb <= k_gc; k_gb += (1LL)){
if(((k_fw) == ((k_al)->At(k_gb))) || (((k_fw) == (u'\u000D')) && (((k_al)->At(k_gb)) == (u'\u000A')))){
return u'\u0000';
}
}
if((k_fw) == (u'\u000D')){
