...
static void k_i();
static void k_j();
static void k_l(int64_t k_cn /*excpt*/);
static void k_p(type_(k_o) k_cq /*me*/);
static void k_q(k_o* k_cr);
static int64_t k_r(type_(k_o) k_cs /*me*/, type_(k_o) k_ct /*t*/);
static type_(k_o) k_s(type_(k_o) k_cu /*me*/);
static type_(Array_<uint8_t>) k_t(type_(k_o) k_cx /*me*/);
static type_(k_o) k_u(type_(k_o) k_cz /*me*/, type_(Array_<uint8_t>) k_da /*bin*/, int64_t* k_db /*idx*/);
static type_(Array_<char16_t>) k_v(type_(k_o) k_de /*me*/);
static void k_w(type_(k_n) k_df /*me*/);
using k_dh=void(*)(type_(k_o));
static int64_t k_x(type_(k_n) k_di /*me*/, int64_t k_dj /*min*/, int64_t k_dk /*max*/);
using k_ds=uint64_t(*)(type_(k_n));
static uint64_t k_y(type_(k_n) k_dv /*me*/);
using k_dx=uint32_t(*)(type_(k_n));
static double k_z(type_(k_n) k_ea /*me*/, double k_eb /*min*/, double k_ec /*max*/);
static void k_aa(type_(k_n) k_ef /*me*/, uint32_t k_eg /*seed*/);
static uint32_t k_ab(type_(k_n) k_em /*me*/);
static void k_ac(k_n* k_eo);
static type_(k_n) k_ad(type_(k_n) k_ep);
static type_(Array_<uint8_t>) k_ae(type_(k_n) k_es);
static type_(k_n) k_af(type_(k_n) k_ey, type_(Array_<uint8_t>) k_ez, int64_t* k_fa);
static type_(k_n) k_ag(type_(k_n) k_fd /*me2*/, uint32_t k_fe /*seed*/);
using k_ff=void(*)(type_(k_n), uint32_t);
template<typename T_>static T_ k_ai(T_ k_fg /*me_*/, type_(Array_<int64_t>) k_fh /*type*/, T_ k_fi /*n*/);
static int64_t k_aj();
static int64_t k_ak();
static int64_t k_an();
template<typename T_,typename C_,typename R_>static R_ k_ch(T_ k_fq /*me_*/, type_(Array_<int64_t>) k_fr /*type*/, C_ k_fs /*key*/, bool* k_ft /*existed*/);
template<typename T_,typename K_,typename V_>static void k_cj(T_ k_fu /*me_*/, type_(Array_<int64_t>) k_fv /*type*/, K_ k_fw /*key*/, V_ k_fx /*item*/);
template<typename T_,typename R_>static R_ k_ck(T_ k_fy /*me_*/, type_(Array_<int64_t>) k_fz /*type*/);
static void k_cl(type_(Array_<char16_t>) k_ga /*str*/);
template<typename T_>static type_(Array_<char16_t>) k_cm(T_ k_gb /*me_*/, type_(Array_<int64_t>) k_gc /*type*/);
template<typename T_>static T_ k_dy(T_ k_gd /*me_*/, type_(Array_<int64_t>) k_ge /*type*/, T_ k_gf /*n*/);
template<typename T_>static T_ k_dz(T_ k_gg /*me_*/, type_(Array_<int64_t>) k_gh /*type*/, int64_t k_gi /*n*/);
template<typename T_>static T_ k_el(T_ k_gl /*me_*/, type_(Array_<int64_t>) k_gm /*type*/, int64_t k_gn /*n*/);
static type_(Array_<char16_t>) k_fk();
static int64_t k_fn(type_(Array_<char16_t>) k_hk /*me_*/, bool* k_hl /*success*/);
static char16_t k_gw();
template<typename T_>static T_ k_hj(T_ k_ia /*me_*/, type_(Array_<int64_t>) k_ib /*type*/, int64_t k_ic /*start*/, int64_t k_id /*len*/);
static char16_t k_hq();
...
k_n::k_n(): k_o(), k_eh(), k_ei(), k_ej(), k_ek(){ Y = 8; }
k_n::~k_n(){
k_ac(this);
}
// k_main_
...
int64_t k_am; // h
int64_t k_ao; // w
type_(Array_<type_(Array_<int64_t>)>) k_ap; // p
int64_t k_at;
int64_t k_ay;
int64_t k_ba; // ans
int64_t k_be;
int64_t k_bg; // i
type_(Array_<bool>) k_bh; // f
int64_t k_bl;
type_(Dict_<int64_t, int64_t>) k_bo; // map
int64_t k_bs;
int64_t k_bu; // prev
int64_t k_bv; // cnt
int64_t k_bz;
...
(k_am) = ((k_an)());
(k_ao) = ((k_an)());
(k_ap) = (newArrays_<type_(Array_<type_(Array_<int64_t>)>)>((k_am), (k_ao)));
for(k_at = (0LL), k_au = ((k_am) - (1LL)); k_at <= k_au; k_at += (1LL)){
for(k_ay = (0LL), k_az = ((k_ao) - (1LL)); k_ay <= k_az; k_ay += (1LL)){
(((k_ap)->At(k_at))->At(k_ay)) = ((k_an)());
}
}
(k_ba) = (0LL);
for(k_be = (0LL), k_bf = ((powI_((2LL), (k_am))) - (1LL)); k_be <= k_bf; k_be += (1LL)){
(k_bg) = (k_be);
(k_bh) = (newArrays_<type_(Array_<bool>)>((k_am)));
for(k_bl = (0LL), k_bm = ((k_am) - (1LL)); k_bl <= k_bm; k_bl += (1LL)){
((k_bh)->At(k_bl)) = (((k_bg) % (2LL)) == (1LL));
(k_bg) = ((k_bg) / (2LL));
}
(k_bo) = (new_(Dict_<int64_t, int64_t>)());
for(k_bs = (0LL), k_bt = ((k_ao) - (1LL)); k_bs <= k_bt; k_bs += (1LL)){
(k_bu) = (-1LL);
(k_bv) = (0LL);
for(k_bz = (0LL), k_ca = ((k_am) - (1LL)); k_bz <= k_ca; k_bz += (1LL)){
if((k_bh)->At(k_bz)){
if(((k_bu) != (-1LL)) && ((k_bu) != (((k_ap)->At(k_bz))->At(k_bs)))){
goto k_br;
}
(k_bu) = (((k_ap)->At(k_bz))->At(k_bs));
(k_bv) = ((k_bv) + (1LL));
}
}
(k_cg) = ((k_ch<type_(Dict_<int64_t, int64_t>), int64_t, int64_t>((k_bo), (nullptr), (k_bu), (std::memset(&k_ci, 0, sizeof(bool)), &k_ci))) + (k_bv));
k_cj<type_(Dict_<int64_t, int64_t>), int64_t, int64_t>((k_bo), (nullptr), (k_bu), (k_cg));
(k_ba) = (k_ck<type_(Array_<int64_t>), int64_t>((new_(Array_<int64_t>)((k_ba), (k_cg))), (nullptr)));
k_br:;
}
}
(k_cl)((((newArrayBin_<char16_t>(0,""))->Cat(k_cm<int64_t>((k_ba), (nullptr))))->Cat(newArrayBin_<char16_t>(1,"\x0A\x00"))));
}
// _finVars
static void k_i(){
(k_al) = (nullptr);
...
return as_<k_o>(classTable_, (k_cv), 0);
}
// _toBin
static type_(Array_<uint8_t>) k_t(type_(k_o) k_cx /*me*/){
type_(Array_<uint8_t>) k_cy;
(k_cy) = (newArrays_<type_(Array_<uint8_t>)>((0LL)));
return k_cy;
}
// _fromBin
static type_(k_o) k_u(type_(k_o) k_cz /*me*/, type_(Array_<uint8_t>) k_da /*bin*/, int64_t* k_db /*idx*/){
...
uint64_t k_do; // m
uint64_t k_dp; // r
(k_dn) = (static_cast<uint64_t>(((k_dk) - (k_dj)) + (1LL)));
(k_do) = ((0x0000000000000000ULL) - (((0x0000000000000000ULL) - (k_dn)) % (k_dn)));
(k_dp) = (0x0000000000000000ULL);
if((k_do) == (0x0000000000000000ULL)){
(k_dp) = ((reinterpret_cast<k_ds>(classTable_[(k_di)->Y + 9]))((k_di)));
}
else{
do{
(k_dp) = ((reinterpret_cast<k_ds>(classTable_[(k_di)->Y + 9]))((k_di)));
...
return (static_cast<int64_t>((k_dp) % (k_dn))) + (k_dj);
}
// rndBit64
static uint64_t k_y(type_(k_n) k_dv /*me*/){
uint32_t k_dw; // t
(k_dw) = ((reinterpret_cast<k_dx>(classTable_[(k_dv)->Y + 12]))((k_dv)));
return k_dy<uint64_t>((k_dz<uint64_t>((static_cast<uint64_t>(k_dw)), (nullptr), (32LL))), (nullptr), (static_cast<uint64_t>((reinterpret_cast<k_dx>(classTable_[(k_dv)->Y + 12]))((k_dv)))));
}
// rndFloat
static double k_z(type_(k_n) k_ea /*me*/, double k_eb /*min*/, double k_ec /*max*/){
return (((static_cast<double>((reinterpret_cast<k_ds>(classTable_[(k_ea)->Y + 9]))((k_ea)))) / (1.844674407370955e+19)) * ((k_ec) - (k_eb))) + (k_eb);
}
// setSeed
static void k_aa(type_(k_n) k_ef /*me*/, uint32_t k_eg /*seed*/){
((k_ef)->k_eh) = (0x075BCD15U);
((k_ef)->k_ei) = (0x159A55E5U);
((k_ef)->k_ej) = ((0x1F123BB5U) * (k_eg));
((k_ef)->k_ek) = ((0x05491333U) * (k_el<uint32_t>(((k_ef)->k_ej), (nullptr), (1LL))));
}
// xs128
static uint32_t k_ab(type_(k_n) k_em /*me*/){
uint32_t k_en; // t
(k_en) = (k_ai<uint32_t>(((k_em)->k_eh), (nullptr), (k_dz<uint32_t>(((k_em)->k_eh), (nullptr), (11LL)))));
((k_em)->k_eh) = ((k_em)->k_ei);
((k_em)->k_ei) = ((k_em)->k_ej);
((k_em)->k_ej) = ((k_em)->k_ek);
((k_em)->k_ek) = (k_ai<uint32_t>((k_ai<uint32_t>(((k_em)->k_ek), (nullptr), (k_el<uint32_t>(((k_em)->k_ek), (nullptr), (19LL))))), (nullptr), (k_ai<uint32_t>((k_en), (nullptr), (k_el<uint32_t>((k_en), (nullptr), (8LL)))))));
return (k_em)->k_ek;
}
// _dtor
static void k_ac(k_n* k_eo){
...
type_(k_n) k_eq;
type_(k_n) k_er;
(k_eq) = (k_er = new_(k_n)(), k_w(k_er), k_er);
((k_eq)->k_eh) = ((k_ep)->k_eh);
((k_eq)->k_ei) = ((k_ep)->k_ei);
((k_eq)->k_ej) = ((k_ep)->k_ej);
((k_eq)->k_ek) = ((k_ep)->k_ek);
return as_<k_n>(classTable_, (k_eq), 8);
}
// _toBin
static type_(Array_<uint8_t>) k_ae(type_(k_n) k_es){
type_(Array_<uint8_t>) k_et;
(k_et) = (newArrays_<type_(Array_<uint8_t>)>((0LL)));
(k_et) = ((k_et)->Cat(toBin_<uint32_t>()(((k_es)->k_eh))));
(k_et) = ((k_et)->Cat(toBin_<uint32_t>()(((k_es)->k_ei))));
(k_et) = ((k_et)->Cat(toBin_<uint32_t>()(((k_es)->k_ej))));
(k_et) = ((k_et)->Cat(toBin_<uint32_t>()(((k_es)->k_ek))));
return k_et;
}
// _fromBin
static type_(k_n) k_af(type_(k_n) k_ey, type_(Array_<uint8_t>) k_ez, int64_t* k_fa){
...
((k_fb)->k_eh) = (fromBin_<uint32_t>()((k_ez), (*k_fa)));
((k_fb)->k_ei) = (fromBin_<uint32_t>()((k_ez), (*k_fa)));
((k_fb)->k_ej) = (fromBin_<uint32_t>()((k_ez), (*k_fa)));
((k_fb)->k_ek) = (fromBin_<uint32_t>()((k_ez), (*k_fa)));
return k_fb;
}
// makeRnd
static type_(k_n) k_ag(type_(k_n) k_fd /*me2*/, uint32_t k_fe /*seed*/){
(reinterpret_cast<k_ff>(classTable_[(k_fd)->Y + 11]))((k_fd), (k_fe));
return k_fd;
}
// _xor
template<typename T_>static T_ k_ai(T_ k_fg /*me_*/, type_(Array_<int64_t>) k_fh /*type*/, T_ k_fi /*n*/){
...
// _getDict
template<typename T_,typename C_,typename R_>static R_ k_ch(T_ k_fq /*me_*/, type_(Array_<int64_t>) k_fr /*type*/, C_ k_fs /*key*/, bool* k_ft /*existed*/){
return dictSearch_(k_fq->B,k_fs,k_ft);
}
// _addDict
template<typename T_,typename K_,typename V_>static void k_cj(T_ k_fu /*me_*/, type_(Array_<int64_t>) k_fv /*type*/, K_ k_fw /*key*/, V_ k_fx /*item*/){
k_fu->Add(k_fw,k_fx);
...
if(cmp_(r_,k_fy->B[i_])<0)r_=k_fy->B[i_];
}
return r_;
}
// print
...
#if defined(_WIN32)
fputws(reinterpret_cast<const wchar_t*>(k_ga->B),stdout);
#else
std::u16string s_=k_ga->B;
...
// inputStr
static type_(Array_<char16_t>) k_fk(){
type_(Array_<char16_t>) k_gq; // buf
int64_t k_gr; // ptr
char16_t k_gv; // c
(k_gq) = (newArrays_<type_(Array_<char16_t>)>((1024LL)));
(k_gr) = (0LL);
while(true){
(k_gv) = ((k_gw)());
if((k_gv) == (u'\uFFFF')){
...
if((k_gr) == ((k_gq)->Len())){
(k_gq) = ((k_gq)->Cat(newArrays_<type_(Array_<char16_t>)>(((k_gq)->Len()))));
}
((k_gq)->At(k_gr)) = (k_gv);
(k_gr) = ((k_gr) + (1LL));
}
return k_hj<type_(Array_<char16_t>)>((k_gq), (nullptr), (0LL), (k_gr));
}
// _toInt
static int64_t k_fn(type_(Array_<char16_t>) k_hk /*me_*/, bool* k_hl /*success*/){
std::u16string u_=k_hk->B;
...
while(true){
(k_hp) = ((k_hq)());
for(k_hu = (0LL), k_hv = (((k_al)->Len()) - (1LL)); k_hu <= k_hv; k_hu += (1LL)){
if(((k_hp) == ((k_al)->At(k_hu))) || (((k_hp) == (u'\u000D')) && (((k_al)->At(k_hu)) == (u'\u000A')))){
return u'\u0000';
}
}
if((k_hp) == (u'\u000D')){
