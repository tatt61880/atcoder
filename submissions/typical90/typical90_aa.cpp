...
static void k_i();
static void k_j();
static void k_l(int64_t k_bc /*excpt*/);
static void k_p(type_(k_o) k_bf /*me*/);
static void k_q(k_o* k_bg);
static int64_t k_r(type_(k_o) k_bh /*me*/, type_(k_o) k_bi /*t*/);
static type_(k_o) k_s(type_(k_o) k_bj /*me*/);
static type_(Array_<uint8_t>) k_t(type_(k_o) k_bm /*me*/);
static type_(k_o) k_u(type_(k_o) k_bo /*me*/, type_(Array_<uint8_t>) k_bp /*bin*/, int64_t* k_bq /*idx*/);
static type_(Array_<char16_t>) k_v(type_(k_o) k_bt /*me*/);
static void k_w(type_(k_n) k_bu /*me*/);
using k_bw=void(*)(type_(k_o));
static int64_t k_x(type_(k_n) k_bx /*me*/, int64_t k_by /*min*/, int64_t k_bz /*max*/);
using k_ch=uint64_t(*)(type_(k_n));
static uint64_t k_y(type_(k_n) k_ck /*me*/);
using k_cm=uint32_t(*)(type_(k_n));
static double k_z(type_(k_n) k_cp /*me*/, double k_cq /*min*/, double k_cr /*max*/);
static void k_aa(type_(k_n) k_cu /*me*/, uint32_t k_cv /*seed*/);
static uint32_t k_ab(type_(k_n) k_db /*me*/);
static void k_ac(k_n* k_dd);
static type_(k_n) k_ad(type_(k_n) k_de);
static type_(Array_<uint8_t>) k_ae(type_(k_n) k_dh);
static type_(k_n) k_af(type_(k_n) k_dn, type_(Array_<uint8_t>) k_do, int64_t* k_dp);
static type_(k_n) k_ag(type_(k_n) k_ds /*me2*/, uint32_t k_dt /*seed*/);
using k_du=void(*)(type_(k_n), uint32_t);
template<typename T_>static T_ k_ai(T_ k_dv /*me_*/, type_(Array_<int64_t>) k_dw /*type*/, T_ k_dx /*n*/);
static int64_t k_aj();
static int64_t k_ak();
static int64_t k_an();
template<typename T_,typename K_,typename V_>static void k_av(T_ k_ef /*me_*/, type_(Array_<int64_t>) k_eg /*type*/, K_ k_eh /*key*/, V_ k_ei /*item*/);
static type_(Array_<char16_t>) k_aw();
static void k_ba(type_(Array_<char16_t>) k_fd /*str*/);
template<typename T_>static type_(Array_<char16_t>) k_bb(T_ k_fe /*me_*/, type_(Array_<int64_t>) k_ff /*type*/);
template<typename T_>static T_ k_cn(T_ k_fg /*me_*/, type_(Array_<int64_t>) k_fh /*type*/, T_ k_fi /*n*/);
template<typename T_>static T_ k_co(T_ k_fj /*me_*/, type_(Array_<int64_t>) k_fk /*type*/, int64_t k_fl /*n*/);
template<typename T_>static T_ k_da(T_ k_fo /*me_*/, type_(Array_<int64_t>) k_fp /*type*/, int64_t k_fq /*n*/);
static type_(Array_<char16_t>) k_dz();
static int64_t k_ec(type_(Array_<char16_t>) k_gm /*me_*/, bool* k_gn /*success*/);
static char16_t k_ep();
template<typename T_>static T_ k_fc(T_ k_go /*me_*/, type_(Array_<int64_t>) k_gp /*type*/, int64_t k_gq /*start*/, int64_t k_gr /*len*/);
...
k_n::k_n(): k_o(), k_cw(), k_cx(), k_cy(), k_cz(){ Y = 8; }
k_n::~k_n(){
k_ac(this);
}
// k_main_
...
int64_t k_am; // n
int64_t k_ao; // prev
type_(Dict_<type_(Array_<char16_t>), bool>) k_ap; // map
int64_t k_at;
int64_t k_au;
(k_am) = ((k_an)());
(k_ao) = (0LL);
(k_ap) = (new_(Dict_<type_(Array_<char16_t>), bool>)());
for(k_at = (1LL), k_au = (k_am); k_at <= k_au; k_at += (1LL)){
k_av<type_(Dict_<type_(Array_<char16_t>), bool>), type_(Array_<char16_t>), bool>((k_ap), (nullptr), ((k_aw)()), (true));
if(((k_ap)->Len()) != (k_ao)){
(k_ao) = ((k_ao) + (1LL));
(k_ba)((((newArrayBin_<char16_t>(0,""))->Cat(k_bb<int64_t>((k_at), (nullptr))))->Cat(newArrayBin_<char16_t>(1,"\x0A\x00"))));
}
}
}
// _finVars
...
type_(k_o) k_bk;
type_(k_o) k_bl;
(k_bk) = (k_bl = new_(k_o)(), k_p(k_bl), k_bl);
return as_<k_o>(classTable_, (k_bk), 0);
}
// _toBin
static type_(Array_<uint8_t>) k_t(type_(k_o) k_bm /*me*/){
type_(Array_<uint8_t>) k_bn;
(k_bn) = (newArrays_<type_(Array_<uint8_t>)>((0LL)));
return k_bn;
}
// _fromBin
static type_(k_o) k_u(type_(k_o) k_bo /*me*/, type_(Array_<uint8_t>) k_bp /*bin*/, int64_t* k_bq /*idx*/){
type_(k_o) k_br;
type_(k_o) k_bs;
(k_br) = (k_bs = new_(k_o)(), k_p(k_bs), k_bs);
return k_br;
}
// toStr
static type_(Array_<char16_t>) k_v(type_(k_o) k_bt /*me*/){
...
uint64_t k_cd; // m
uint64_t k_ce; // r
(k_cc) = (static_cast<uint64_t>(((k_bz) - (k_by)) + (1LL)));
(k_cd) = ((0x0000000000000000ULL) - (((0x0000000000000000ULL) - (k_cc)) % (k_cc)));
(k_ce) = (0x0000000000000000ULL);
if((k_cd) == (0x0000000000000000ULL)){
(k_ce) = ((reinterpret_cast<k_ch>(classTable_[(k_bx)->Y + 9]))((k_bx)));
}
else{
do{
(k_ce) = ((reinterpret_cast<k_ch>(classTable_[(k_bx)->Y + 9]))((k_bx)));
...
return (static_cast<int64_t>((k_ce) % (k_cc))) + (k_by);
}
// rndBit64
static uint64_t k_y(type_(k_n) k_ck /*me*/){
uint32_t k_cl; // t
(k_cl) = ((reinterpret_cast<k_cm>(classTable_[(k_ck)->Y + 12]))((k_ck)));
return k_cn<uint64_t>((k_co<uint64_t>((static_cast<uint64_t>(k_cl)), (nullptr), (32LL))), (nullptr), (static_cast<uint64_t>((reinterpret_cast<k_cm>(classTable_[(k_ck)->Y + 12]))((k_ck)))));
}
// rndFloat
static double k_z(type_(k_n) k_cp /*me*/, double k_cq /*min*/, double k_cr /*max*/){
return (((static_cast<double>((reinterpret_cast<k_ch>(classTable_[(k_cp)->Y + 9]))((k_cp)))) / (1.844674407370955e+19)) * ((k_cr) - (k_cq))) + (k_cq);
}
// setSeed
static void k_aa(type_(k_n) k_cu /*me*/, uint32_t k_cv /*seed*/){
((k_cu)->k_cw) = (0x075BCD15U);
((k_cu)->k_cx) = (0x159A55E5U);
((k_cu)->k_cy) = ((0x1F123BB5U) * (k_cv));
((k_cu)->k_cz) = ((0x05491333U) * (k_da<uint32_t>(((k_cu)->k_cy), (nullptr), (1LL))));
}
// xs128
static uint32_t k_ab(type_(k_n) k_db /*me*/){
uint32_t k_dc; // t
(k_dc) = (k_ai<uint32_t>(((k_db)->k_cw), (nullptr), (k_co<uint32_t>(((k_db)->k_cw), (nullptr), (11LL)))));
((k_db)->k_cw) = ((k_db)->k_cx);
((k_db)->k_cx) = ((k_db)->k_cy);
((k_db)->k_cy) = ((k_db)->k_cz);
((k_db)->k_cz) = (k_ai<uint32_t>((k_ai<uint32_t>(((k_db)->k_cz), (nullptr), (k_da<uint32_t>(((k_db)->k_cz), (nullptr), (19LL))))), (nullptr), (k_ai<uint32_t>((k_dc), (nullptr), (k_da<uint32_t>((k_dc), (nullptr), (8LL)))))));
return (k_db)->k_cz;
}
// _dtor
static void k_ac(k_n* k_dd){
...
((k_df)->k_cy) = ((k_de)->k_cy);
((k_df)->k_cz) = ((k_de)->k_cz);
return as_<k_n>(classTable_, (k_df), 8);
}
// _toBin
static type_(Array_<uint8_t>) k_ae(type_(k_n) k_dh){
type_(Array_<uint8_t>) k_di;
(k_di) = (newArrays_<type_(Array_<uint8_t>)>((0LL)));
(k_di) = ((k_di)->Cat(toBin_<uint32_t>()(((k_dh)->k_cw))));
(k_di) = ((k_di)->Cat(toBin_<uint32_t>()(((k_dh)->k_cx))));
(k_di) = ((k_di)->Cat(toBin_<uint32_t>()(((k_dh)->k_cy))));
(k_di) = ((k_di)->Cat(toBin_<uint32_t>()(((k_dh)->k_cz))));
return k_di;
}
// _fromBin
static type_(k_n) k_af(type_(k_n) k_dn, type_(Array_<uint8_t>) k_do, int64_t* k_dp){
...
((k_dq)->k_cw) = (fromBin_<uint32_t>()((k_do), (*k_dp)));
((k_dq)->k_cx) = (fromBin_<uint32_t>()((k_do), (*k_dp)));
((k_dq)->k_cy) = (fromBin_<uint32_t>()((k_do), (*k_dp)));
((k_dq)->k_cz) = (fromBin_<uint32_t>()((k_do), (*k_dp)));
return k_dq;
}
// makeRnd
static type_(k_n) k_ag(type_(k_n) k_ds /*me2*/, uint32_t k_dt /*seed*/){
(reinterpret_cast<k_du>(classTable_[(k_ds)->Y + 11]))((k_ds), (k_dt));
return k_ds;
}
// _xor
template<typename T_>static T_ k_ai(T_ k_dv /*me_*/, type_(Array_<int64_t>) k_dw /*type*/, T_ k_dx /*n*/){
...
// input
static type_(Array_<char16_t>) k_aw(){
type_(Array_<char16_t>) k_ej; // buf
int64_t k_ek; // ptr
char16_t k_eo; // c
(k_ej) = (newArrays_<type_(Array_<char16_t>)>((1024LL)));
(k_ek) = (0LL);
while(true){
(k_eo) = ((k_ep)());
if((k_eo) == (u'\u000D')){
...
if((k_ek) == ((k_ej)->Len())){
(k_ej) = ((k_ej)->Cat(newArrays_<type_(Array_<char16_t>)>(((k_ej)->Len()))));
}
((k_ej)->At(k_ek)) = (k_eo);
(k_ek) = ((k_ek) + (1LL));
}
return k_fc<type_(Array_<char16_t>)>((k_ej), (nullptr), (0LL), (k_ek));
}
// print
static void k_ba(type_(Array_<char16_t>) k_fd /*str*/){
if(k_fd==nullptr){
...
return k_fc<type_(Array_<char16_t>)>((k_ft), (nullptr), (0LL), (k_fu));
}
// _toInt
static int64_t k_ec(type_(Array_<char16_t>) k_gm /*me_*/, bool* k_gn /*success*/){
std::u16string u_=k_gm->B;
...
while(true){
(k_gv) = ((k_ep)());
for(k_gz = (0LL), k_ha = (((k_al)->Len()) - (1LL)); k_gz <= k_ha; k_gz += (1LL)){
if(((k_gv) == ((k_al)->At(k_gz))) || (((k_gv) == (u'\u000D')) && (((k_al)->At(k_gz)) == (u'\u000A')))){
return u'\u0000';
}
}
if((k_gv) == (u'\u000D')){
