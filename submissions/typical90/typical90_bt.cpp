...
static void k_i();
static void k_j();
static void k_l(int64_t k_bx /*excpt*/);
static void k_p(type_(k_o) k_ca /*me*/);
static void k_q(k_o* k_cb);
static int64_t k_r(type_(k_o) k_cc /*me*/, type_(k_o) k_cd /*t*/);
static type_(k_o) k_s(type_(k_o) k_ce /*me*/);
static type_(Array_<uint8_t>) k_t(type_(k_o) k_ch /*me*/);
static type_(k_o) k_u(type_(k_o) k_cj /*me*/, type_(Array_<uint8_t>) k_ck /*bin*/, int64_t* k_cl /*idx*/);
static type_(Array_<char16_t>) k_v(type_(k_o) k_co /*me*/);
static void k_w(type_(k_n) k_cp /*me*/);
using k_cr=void(*)(type_(k_o));
static int64_t k_x(type_(k_n) k_cs /*me*/, int64_t k_ct /*min*/, int64_t k_cu /*max*/);
using k_dc=uint64_t(*)(type_(k_n));
static uint64_t k_y(type_(k_n) k_df /*me*/);
using k_dh=uint32_t(*)(type_(k_n));
static double k_z(type_(k_n) k_dk /*me*/, double k_dl /*min*/, double k_dm /*max*/);
static void k_aa(type_(k_n) k_dp /*me*/, uint32_t k_dq /*seed*/);
static uint32_t k_ab(type_(k_n) k_dw /*me*/);
static void k_ac(k_n* k_dy);
static type_(k_n) k_ad(type_(k_n) k_dz);
static type_(Array_<uint8_t>) k_ae(type_(k_n) k_ec);
static type_(k_n) k_af(type_(k_n) k_ei, type_(Array_<uint8_t>) k_ej, int64_t* k_ek);
static type_(k_n) k_ag(type_(k_n) k_en /*me2*/, uint32_t k_eo /*seed*/);
using k_ep=void(*)(type_(k_n), uint32_t);
template<typename T_>static T_ k_ai(T_ k_eq /*me_*/, type_(Array_<int64_t>) k_er /*type*/, T_ k_es /*n*/);
static int64_t k_aj();
static int64_t k_ak();
static int64_t k_an();
static type_(Array_<char16_t>) k_av();
template<typename T_,typename R_>static R_ k_br(T_ k_fu /*me_*/, type_(Array_<int64_t>) k_fv /*type*/);
static int64_t k_bs(int64_t k_fw /*cnt*/, int64_t k_fx /*sy*/, int64_t k_fy /*sx*/, int64_t k_fz /*y*/, int64_t k_ga /*x*/, int64_t k_gb /*h*/, int64_t k_gc /*w*/, type_(Array_<type_(Array_<char16_t>)>) k_gd /*c*/, int64_t k_ge /*d*/);
static void k_bv(type_(Array_<char16_t>) k_gy /*str*/);
template<typename T_>static type_(Array_<char16_t>) k_bw(T_ k_gz /*me_*/, type_(Array_<int64_t>) k_ha /*type*/);
template<typename T_>static T_ k_di(T_ k_hb /*me_*/, type_(Array_<int64_t>) k_hc /*type*/, T_ k_hd /*n*/);
template<typename T_>static T_ k_dj(T_ k_he /*me_*/, type_(Array_<int64_t>) k_hf /*type*/, int64_t k_hg /*n*/);
template<typename T_>static T_ k_dv(T_ k_hj /*me_*/, type_(Array_<int64_t>) k_hk /*type*/, int64_t k_hl /*n*/);
static type_(Array_<char16_t>) k_eu();
static int64_t k_ex(type_(Array_<char16_t>) k_ih /*me_*/, bool* k_ii /*success*/);
static char16_t k_fg();
template<typename T_>static T_ k_ft(T_ k_ij /*me_*/, type_(Array_<int64_t>) k_ik /*type*/, int64_t k_il /*start*/, int64_t k_im /*len*/);
static char16_t k_hu();
...
k_n::k_n(): k_o(), k_dr(), k_ds(), k_dt(), k_du(){ Y = 8; }
k_n::~k_n(){
k_ac(this);
}
// k_main_
...
int64_t k_am; // h
int64_t k_ao; // w
type_(Array_<type_(Array_<char16_t>)>) k_ap; // c
int64_t k_at;
int64_t k_aw; // ans
int64_t k_bd;
int64_t k_bi;
...
(k_am) = ((k_an)());
(k_ao) = ((k_an)());
(k_ap) = (newArrays_<type_(Array_<type_(Array_<char16_t>)>)>((k_am)));
for(k_at = (0LL), k_au = ((k_am) - (1LL)); k_at <= k_au; k_at += (1LL)){
((k_ap)->At(k_at)) = ((k_av)());
}
(k_aw) = (0LL);
if(((k_am) == (1LL)) || ((k_ao) == (1LL))){
}
else{
for(k_bd = (0LL), k_be = ((k_am) - (1LL)); k_bd <= k_be; k_bd += (1LL)){
for(k_bi = (0LL), k_bj = ((k_ao) - (1LL)); k_bi <= k_bj; k_bi += (1LL)){
for(k_bn = (0LL), k_bo = (3LL); k_bn <= k_bo; k_bn += (1LL)){
if((((k_ap)->At(k_bd))->At(k_bi)) == (u'\u002E')){
(k_aw) = (k_br<type_(Array_<int64_t>), int64_t>((new_(Array_<int64_t>)((k_aw), ((k_bs)((0LL), (k_bd), (k_bi), (k_bd), (k_bi), (k_am), (k_ao), (k_ap), (k_bn))))), (nullptr)));
}
}
}
}
...
(k_bv)((((newArrayBin_<char16_t>(0,""))->Cat(k_bw<int64_t>((k_aw), (nullptr))))->Cat(newArrayBin_<char16_t>(1,"\x0A\x00"))));
}
// _finVars
static void k_i(){
(k_al) = (nullptr);
...
type_(k_o) k_cf;
type_(k_o) k_cg;
(k_cf) = (k_cg = new_(k_o)(), k_p(k_cg), k_cg);
return as_<k_o>(classTable_, (k_cf), 0);
}
// _toBin
static type_(Array_<uint8_t>) k_t(type_(k_o) k_ch /*me*/){
...
type_(k_o) k_cm;
type_(k_o) k_cn;
(k_cm) = (k_cn = new_(k_o)(), k_p(k_cn), k_cn);
return k_cm;
}
// toStr
static type_(Array_<char16_t>) k_v(type_(k_o) k_co /*me*/){
...
static uint32_t k_ab(type_(k_n) k_dw /*me*/){
uint32_t k_dx; // t
(k_dx) = (k_ai<uint32_t>(((k_dw)->k_dr), (nullptr), (k_dj<uint32_t>(((k_dw)->k_dr), (nullptr), (11LL)))));
((k_dw)->k_dr) = ((k_dw)->k_ds);
((k_dw)->k_ds) = ((k_dw)->k_dt);
((k_dw)->k_dt) = ((k_dw)->k_du);
((k_dw)->k_du) = (k_ai<uint32_t>((k_ai<uint32_t>(((k_dw)->k_du), (nullptr), (k_dv<uint32_t>(((k_dw)->k_du), (nullptr), (19LL))))), (nullptr), (k_ai<uint32_t>((k_dx), (nullptr), (k_dv<uint32_t>((k_dx), (nullptr), (8LL)))))));
return (k_dw)->k_du;
}
// _dtor
static void k_ac(k_n* k_dy){
...
((k_ea)->k_dt) = ((k_dz)->k_dt);
((k_ea)->k_du) = ((k_dz)->k_du);
return as_<k_n>(classTable_, (k_ea), 8);
}
// _toBin
static type_(Array_<uint8_t>) k_ae(type_(k_n) k_ec){
...
type_(k_n) k_el;
type_(k_n) k_em;
(k_el) = (k_em = new_(k_n)(), k_w(k_em), k_em);
...
// input
static type_(Array_<char16_t>) k_av(){
type_(Array_<char16_t>) k_fa; // buf
int64_t k_fb; // ptr
char16_t k_ff; // c
(k_fa) = (newArrays_<type_(Array_<char16_t>)>((1024LL)));
(k_fb) = (0LL);
while(true){
(k_ff) = ((k_fg)());
if((k_ff) == (u'\u000D')){
...
if((k_fb) == ((k_fa)->Len())){
(k_fa) = ((k_fa)->Cat(newArrays_<type_(Array_<char16_t>)>(((k_fa)->Len()))));
}
((k_fa)->At(k_fb)) = (k_ff);
(k_fb) = ((k_fb) + (1LL));
}
return k_ft<type_(Array_<char16_t>)>((k_fa), (nullptr), (0LL), (k_fb));
}
// _max
template<typename T_,typename R_>static R_ k_br(T_ k_fu /*me_*/, type_(Array_<int64_t>) k_fv /*type*/){
int64_t l_=k_fu->L;
...
if(cmp_(r_,k_fu->B[i_])<0)r_=k_fu->B[i_];
}
return r_;
}
// f
static int64_t k_bs(int64_t k_fw /*cnt*/, int64_t k_fx /*sy*/, int64_t k_fy /*sx*/, int64_t k_fz /*y*/, int64_t k_ga /*x*/, int64_t k_gb /*h*/, int64_t k_gc /*w*/, type_(Array_<type_(Array_<char16_t>)>) k_gd /*c*/, int64_t k_ge /*d*/){
int64_t k_gf; // res
type_(Array_<int64_t>) k_gg; // dx
type_(Array_<int64_t>) k_gh; // dy
int64_t k_gl;
int64_t k_gn; // nd
int64_t k_go; // nx
int64_t k_gp; // ny
int64_t k_gm;
(k_gf) = (0LL);
(k_gg) = (new_(Array_<int64_t>)((1LL), (0LL), (-1LL), (0LL)));
(k_gh) = (new_(Array_<int64_t>)((0LL), (1LL), (0LL), (-1LL)));
for(k_gl = (-1LL), k_gm = (1LL); k_gl <= k_gm; k_gl += (1LL)){
(k_gn) = ((((k_ge) + (k_gl)) + (4LL)) % (4LL));
(k_go) = ((k_ga) + ((k_gg)->At(k_gn)));
(k_gp) = ((k_fz) + ((k_gh)->At(k_gn)));
if(((k_go) == (-1LL)) || ((k_go) == (k_gc))){
continue;
}
if(((k_gp) == (-1LL)) || ((k_gp) == (k_gb))){
continue;
}
if(((k_gp) == (k_fx)) && ((k_go) == (k_fy))){
(k_gf) = (k_br<type_(Array_<int64_t>), int64_t>((new_(Array_<int64_t>)((k_gf), ((k_fw) + (1LL)))), (nullptr)));
continue;
}
if((((k_gd)->At(k_gp))->At(k_go)) == (u'\u0023')){
continue;
}
(((k_gd)->At(k_gp))->At(k_go)) = (u'\u0023');
(k_gf) = (k_br<type_(Array_<int64_t>), int64_t>((new_(Array_<int64_t>)((k_gf), ((k_bs)(((k_fw) + (1LL)), (k_fx), (k_fy), (k_gp), (k_go), (k_gb), (k_gc), (k_gd), (k_gn))))), (nullptr)));
(((k_gd)->At(k_gp))->At(k_go)) = (u'\u002E');
}
return k_gf;
}
...
#if defined(_WIN32)
fputws(reinterpret_cast<const wchar_t*>(k_gy->B),stdout);
#else
std::u16string s_=k_gy->B;
...
// inputStr
static type_(Array_<char16_t>) k_eu(){
type_(Array_<char16_t>) k_ho; // buf
int64_t k_hp; // ptr
char16_t k_ht; // c
(k_ho) = (newArrays_<type_(Array_<char16_t>)>((1024LL)));
(k_hp) = (0LL);
while(true){
(k_ht) = ((k_hu)());
if((k_ht) == (u'\uFFFF')){
...
if((k_hp) == ((k_ho)->Len())){
(k_ho) = ((k_ho)->Cat(newArrays_<type_(Array_<char16_t>)>(((k_ho)->Len()))));
}
((k_ho)->At(k_hp)) = (k_ht);
(k_hp) = ((k_hp) + (1LL));
}
return k_ft<type_(Array_<char16_t>)>((k_ho), (nullptr), (0LL), (k_hp));
}
// _toInt
static int64_t k_ex(type_(Array_<char16_t>) k_ih /*me_*/, bool* k_ii /*success*/){
std::u16string u_=k_ih->B;
...
while(true){
(k_iq) = ((k_fg)());
for(k_iu = (0LL), k_iv = (((k_al)->Len()) - (1LL)); k_iu <= k_iv; k_iu += (1LL)){
if(((k_iq) == ((k_al)->At(k_iu))) || (((k_iq) == (u'\u000D')) && (((k_al)->At(k_iu)) == (u'\u000A')))){
return u'\u0000';
}
}
if((k_iq) == (u'\u000D')){
