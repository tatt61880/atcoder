...
while(@queue)
{
	...
	#print "[$x, $y]\n";
	if($y &
gt; 0      && $maze[$y - 1][$x] eq '.') {$maze[$y - 1][$x] = $maze[$y][$x] + 1; push @queue, [$x, $y - 1]}
...
while(@queue)
{
	...
	if($y < $R - 1 && $maze[$y + 1][$x] eq '.') {$maze[$y + 1][$x] = $maze[$y][$x] - 1; push @queue, [$x, $y + 1]}
	if($x > 0      && $maze[$
y][$x - 1] eq '.') {$maze[$y][$x - 1] = $maze[$y][$x] - 1; push @queue, [$x - 1, $y]}
	if($x < $C - 1 && $maze[$y][$x + 1] eq '.') {$maze[$y][$x + 1] = $maze[$y][$x] - 1; push @queue, [$x + 1, $y]}
	shift @queue;
}
...
for my $y(0 .. $R - 1){
	for my $x(0 .. $C - 1){
		if($maze[$y][$x] eq '#'){
			...
			$$p = ($p = \($maze[$y + 1][$x] > 0 ? $M : 
$m), $$p == 0) || abs $$p > abs $maze[$y + 1][$x] ? $maze[$y + 1][$x] : $$p if($y < $R - 1 && $maze[$y + 1][$x] !~ m/#|\./);
			if($m < 0 && $M > 0){
				my $val = $M - $m;
				$max = $val if($val > $max);
			}
