...
static void k_i();
static void k_j();
static void k_l(int64_t k_cu /*excpt*/);
static void k_p(type_(k_o) k_cx /*me*/);
static void k_q(k_o* k_cy);
static int64_t k_r(type_(k_o) k_cz /*me*/, type_(k_o) k_da /*t*/);
static type_(k_o) k_s(type_(k_o) k_db /*me*/);
static type_(Array_<uint8_t>) k_t(type_(k_o) k_de /*me*/);
static type_(k_o) k_u(type_(k_o) k_dg /*me*/, type_(Array_<uint8_t>) k_dh /*bin*/, int64_t* k_di /*idx*/);
static type_(Array_<char16_t>) k_v(type_(k_o) k_dl /*me*/);
static void k_w(type_(k_n) k_dm /*me*/);
using k_do=void(*)(type_(k_o));
static int64_t k_x(type_(k_n) k_dp /*me*/, int64_t k_dq /*min*/, int64_t k_dr /*max*/);
using k_dz=uint64_t(*)(type_(k_n));
static uint64_t k_y(type_(k_n) k_ec /*me*/);
using k_ee=uint32_t(*)(type_(k_n));
static double k_z(type_(k_n) k_eh /*me*/, double k_ei /*min*/, double k_ej /*max*/);
static void k_aa(type_(k_n) k_em /*me*/, uint32_t k_en /*seed*/);
static uint32_t k_ab(type_(k_n) k_et /*me*/);
static void k_ac(k_n* k_ev);
static type_(k_n) k_ad(type_(k_n) k_ew);
static type_(Array_<uint8_t>) k_ae(type_(k_n) k_ez);
static type_(k_n) k_af(type_(k_n) k_ff, type_(Array_<uint8_t>) k_fg, int64_t* k_fh);
static type_(k_n) k_ag(type_(k_n) k_fk /*me2*/, uint32_t k_fl /*seed*/);
using k_fm=void(*)(type_(k_n), uint32_t);
template<typename T_>static T_ k_ai(T_ k_fn /*me_*/, type_(Array_<int64_t>) k_fo /*type*/, T_ k_fp /*n*/);
static int64_t k_aj();
static int64_t k_ak();
static int64_t k_an();
static double k_av();
static double k_bm(double k_gd /*x*/, double k_ge /*y*/, double k_gf /*centerX*/, double k_gg /*centerY*/);
template<typename T_>static void k_bp(T_ k_gh /*me_*/, type_(Array_<int64_t>) k_gi /*type*/);
static int64_t k_ck(int64_t k_gj /*me_*/);
template<typename T_,typename R_>static R_ k_cp(T_ k_gk /*me_*/, type_(Array_<int64_t>) k_gl /*type*/);
static double k_cq(double k_gm /*t*/);
static void k_cr(type_(Array_<char16_t>) k_gp /*str*/);
template<typename T_>static type_(Array_<char16_t>) k_cs(T_ k_gq /*me_*/, type_(Array_<int64_t>) k_gr /*type*/);
static double k_ct(double k_gs /*rad*/);
template<typename T_>static T_ k_ef(T_ k_gt /*me_*/, type_(Array_<int64_t>) k_gu /*type*/, T_ k_gv /*n*/);
template<typename T_>static T_ k_eg(T_ k_gw /*me_*/, type_(Array_<int64_t>) k_gx /*type*/, int64_t k_gy /*n*/);
template<typename T_>static T_ k_es(T_ k_hb /*me_*/, type_(Array_<int64_t>) k_hc /*type*/, int64_t k_hd /*n*/);
static type_(Array_<char16_t>) k_fr();
static int64_t k_fu(type_(Array_<char16_t>) k_ia /*me_*/, bool* k_ib /*success*/);
static double k_ga(type_(Array_<char16_t>) k_ic /*me_*/, bool* k_id /*success*/);
static char16_t k_hm();
template<typename T_>static T_ k_hz(T_ k_is /*me_*/, type_(Array_<int64_t>) k_it /*type*/, int64_t k_iu /*start*/, int64_t k_iv /*len*/);
static char16_t k_ii();
...
k_n::k_n(): k_o(), k_eo(), k_ep(), k_eq(), k_er(){ Y = 8; }
k_n::~k_n(){
k_ac(this);
}
// k_main_
...
static void k_g(){
int64_t k_am; // n
type_(Array_<double>) k_ao; // x
type_(Array_<double>) k_ap; // y
int64_t k_at;
double k_aw; // ans
int64_t k_ba;
type_(Array_<double>) k_bc; // rots
int64_t k_bg;
type_(Array_<double>) k_bq; // rots2
int64_t k_bu;
int64_t k_bz;
int64_t k_ce;
double k_cg; // target
...
int64_t k_cf;
(k_am) = ((k_an)());
(k_ao) = (newArrays_<type_(Array_<double>)>((k_am)));
(k_ap) = (newArrays_<type_(Array_<double>)>((k_am)));
...
(k_bc) = (newArrays_<type_(Array_<double>)>((k_am)));
for(k_bg = (0LL), k_bh = ((k_am) - (1LL)); k_bg <= k_bh; k_bg += (1LL)){
if((k_ba) == (k_bg)){
if((k_ba) == (0LL)){
((k_bc)->At(k_bg)) = ((k_bm)(((k_ao)->At((k_bg) + (1LL))), ((k_ap)->At((k_bg) + (1LL))), ((k_ao)->At(k_ba)), ((k_ap)->At(k_ba))));
}
else{
((k_bc)->At(k_bg)) = ((k_bm)(((k_ao)->At((k_bg) - (1LL))), ((k_ap)->At((k_bg) - (1LL))), ((k_ao)->At(k_ba)), ((k_ap)->At(k_ba))));
}
...
((k_bc)->At(k_bg)) = ((k_bm)(((k_ao)->At(k_bg)), ((k_ap)->At(k_bg)), ((k_ao)->At(k_ba)), ((k_ap)->At(k_ba))));
}
}
k_bp<type_(Array_<double>)>((k_bc), (nullptr));
(k_bq) = (newArrays_<type_(Array_<double>)>(((2LL) * (k_am))));
for(k_bu = (0LL), k_bv = ((k_am) - (1LL)); k_bu <= k_bv; k_bu += (1LL)){
((k_bq)->At(k_bu)) = ((k_bc)->At(k_bu));
}
for(k_bz = (k_am), k_ca = (((2LL) * (k_am)) - (1LL)); k_bz <= k_ca; k_bz += (1LL)){
((k_bq)->At(k_bz)) = (((k_bc)->At((k_bz) - (k_am))) + (6.283185307179586));
}
for(k_ce = (0LL), k_cf = ((k_am) - (1LL)); k_ce <= k_cf; k_ce += (1LL)){
(k_cg) = (((k_bc)->At(k_ce)) + (3.141592653589793));
(k_ch) = (k_ce);
(k_ci) = ((k_ce) + (k_am));
while(((k_ck)(((k_ch) - (k_ci)))) > (1LL)){
(k_cl) = (((k_ch) + (k_ci)) / (2LL));
if(((k_bq)->At(k_cl)) <= (k_cg)){
(k_ch) = (k_cl);
}
else{
(k_ci) = (k_cl);
...
(k_aw) = (k_cp<type_(Array_<double>), double>((new_(Array_<double>)((k_aw), ((k_cq)((((k_bq)->At(k_ch)) - ((k_bc)->At(k_ce))))))), (nullptr)));
(k_aw) = (k_cp<type_(Array_<double>), double>((new_(Array_<double>)((k_aw), ((k_cq)((((k_bq)->At((k_ch) + (1LL))) - ((k_bc)->At(k_ce))))))), (nullptr)));
}
}
(k_cr)((((newArrayBin_<char16_t>(0,""))->Cat(k_cs<double>(((k_ct)((k_aw))), (nullptr))))->Cat(newArrayBin_<char16_t>(1,"\x0A\x00"))));
}
// _finVars
static void k_i(){
(k_al) = (nullptr);
...
type_(k_o) k_dc;
type_(k_o) k_dd;
(k_dc) = (k_dd = new_(k_o)(), k_p(k_dd), k_dd);
return as_<k_o>(classTable_, (k_dc), 0);
}
// _toBin
static type_(Array_<uint8_t>) k_t(type_(k_o) k_de /*me*/){
...
type_(k_o) k_dj;
type_(k_o) k_dk;
(k_dj) = (k_dk = new_(k_o)(), k_p(k_dk), k_dk);
return k_dj;
}
// toStr
static type_(Array_<char16_t>) k_v(type_(k_o) k_dl /*me*/){
...
uint64_t k_dv; // m
uint64_t k_dw; // r
(k_du) = (static_cast<uint64_t>(((k_dr) - (k_dq)) + (1LL)));
(k_dv) = ((0x0000000000000000ULL) - (((0x0000000000000000ULL) - (k_du)) % (k_du)));
(k_dw) = (0x0000000000000000ULL);
if((k_dv) == (0x0000000000000000ULL)){
(k_dw) = ((reinterpret_cast<k_dz>(classTable_[(k_dp)->Y + 9]))((k_dp)));
}
else{
do{
(k_dw) = ((reinterpret_cast<k_dz>(classTable_[(k_dp)->Y + 9]))((k_dp)));
...
return (static_cast<int64_t>((k_dw) % (k_du))) + (k_dq);
}
// rndBit64
static uint64_t k_y(type_(k_n) k_ec /*me*/){
uint32_t k_ed; // t
(k_ed) = ((reinterpret_cast<k_ee>(classTable_[(k_ec)->Y + 12]))((k_ec)));
return k_ef<uint64_t>((k_eg<uint64_t>((static_cast<uint64_t>(k_ed)), (nullptr), (32LL))), (nullptr), (static_cast<uint64_t>((reinterpret_cast<k_ee>(classTable_[(k_ec)->Y + 12]))((k_ec)))));
}
// rndFloat
static double k_z(type_(k_n) k_eh /*me*/, double k_ei /*min*/, double k_ej /*max*/){
return (((static_cast<double>((reinterpret_cast<k_dz>(classTable_[(k_eh)->Y + 9]))((k_eh)))) / (1.844674407370955e+19)) * ((k_ej) - (k_ei))) + (k_ei);
}
// setSeed
static void k_aa(type_(k_n) k_em /*me*/, uint32_t k_en /*seed*/){
((k_em)->k_eo) = (0x075BCD15U);
((k_em)->k_ep) = (0x159A55E5U);
((k_em)->k_eq) = ((0x1F123BB5U) * (k_en));
((k_em)->k_er) = ((0x05491333U) * (k_es<uint32_t>(((k_em)->k_eq), (nullptr), (1LL))));
}
// xs128
static uint32_t k_ab(type_(k_n) k_et /*me*/){
uint32_t k_eu; // t
(k_eu) = (k_ai<uint32_t>(((k_et)->k_eo), (nullptr), (k_eg<uint32_t>(((k_et)->k_eo), (nullptr), (11LL)))));
((k_et)->k_eo) = ((k_et)->k_ep);
((k_et)->k_ep) = ((k_et)->k_eq);
((k_et)->k_eq) = ((k_et)->k_er);
((k_et)->k_er) = (k_ai<uint32_t>((k_ai<uint32_t>(((k_et)->k_er), (nullptr), (k_es<uint32_t>(((k_et)->k_er), (nullptr), (19LL))))), (nullptr), (k_ai<uint32_t>((k_eu), (nullptr), (k_es<uint32_t>((k_eu), (nullptr), (8LL)))))));
return (k_et)->k_er;
}
// _dtor
static void k_ac(k_n* k_ev){
...
((k_ex)->k_eq) = ((k_ew)->k_eq);
((k_ex)->k_er) = ((k_ew)->k_er);
return as_<k_n>(classTable_, (k_ex), 8);
}
// _toBin
static type_(Array_<uint8_t>) k_ae(type_(k_n) k_ez){
type_(Array_<uint8_t>) k_fa;
(k_fa) = (newArrays_<type_(Array_<uint8_t>)>((0LL)));
(k_fa) = ((k_fa)->Cat(toBin_<uint32_t>()(((k_ez)->k_eo))));
(k_fa) = ((k_fa)->Cat(toBin_<uint32_t>()(((k_ez)->k_ep))));
(k_fa) = ((k_fa)->Cat(toBin_<uint32_t>()(((k_ez)->k_eq))));
(k_fa) = ((k_fa)->Cat(toBin_<uint32_t>()(((k_ez)->k_er))));
return k_fa;
}
// _fromBin
static type_(k_n) k_af(type_(k_n) k_ff, type_(Array_<uint8_t>) k_fg, int64_t* k_fh){
type_(k_n) k_fi;
type_(k_n) k_fj;
(k_fi) = (k_fj = new_(k_n)(), k_w(k_fj), k_fj);
((k_fi)->k_eo) = (fromBin_<uint32_t>()((k_fg), (*k_fh)));
((k_fi)->k_ep) = (fromBin_<uint32_t>()((k_fg), (*k_fh)));
((k_fi)->k_eq) = (fromBin_<uint32_t>()((k_fg), (*k_fh)));
((k_fi)->k_er) = (fromBin_<uint32_t>()((k_fg), (*k_fh)));
return k_fi;
}
// makeRnd
static type_(k_n) k_ag(type_(k_n) k_fk /*me2*/, uint32_t k_fl /*seed*/){
(reinterpret_cast<k_fm>(classTable_[(k_fk)->Y + 11]))((k_fk), (k_fl));
return k_fk;
}
// _xor
template<typename T_>static T_ k_ai(T_ k_fn /*me_*/, type_(Array_<int64_t>) k_fo /*type*/, T_ k_fp /*n*/){
...
// invRot
static double k_bm(double k_gd /*x*/, double k_ge /*y*/, double k_gf /*centerX*/, double k_gg /*centerY*/){
double r_=std::atan2(k_ge-k_gg,k_gd-k_gf);
...
// f
static double k_cq(double k_gm /*t*/){
if((k_gm) > (3.141592653589793)){
(k_gm) = ((6.283185307179586) - (k_gm));
}
return k_gm;
}
// print
...
// toDegree
static double k_ct(double k_gs /*rad*/){
return ((k_gs) * (180.0)) / (3.141592653589793);
}
// _or
template<typename T_>static T_ k_ef(T_ k_gt /*me_*/, type_(Array_<int64_t>) k_gu /*type*/, T_ k_gv /*n*/){
return k_gt|k_gv;
...
// inputStr
static type_(Array_<char16_t>) k_fr(){
type_(Array_<char16_t>) k_hg; // buf
int64_t k_hh; // ptr
char16_t k_hl; // c
(k_hg) = (newArrays_<type_(Array_<char16_t>)>((1024LL)));
(k_hh) = (0LL);
while(true){
(k_hl) = ((k_hm)());
if((k_hl) == (u'\uFFFF')){
...
if((k_hh) == ((k_hg)->Len())){
(k_hg) = ((k_hg)->Cat(newArrays_<type_(Array_<char16_t>)>(((k_hg)->Len()))));
}
((k_hg)->At(k_hh)) = (k_hl);
(k_hh) = ((k_hh) + (1LL));
}
return k_hz<type_(Array_<char16_t>)>((k_hg), (nullptr), (0LL), (k_hh));
}
// _toInt
static int64_t k_fu(type_(Array_<char16_t>) k_ia /*me_*/, bool* k_ib /*success*/){
std::u16string u_=k_ia->B;
...
while(true){
(k_ih) = ((k_ii)());
for(k_im = (0LL), k_in = (((k_al)->Len()) - (1LL)); k_im <= k_in; k_im += (1LL)){
if(((k_ih) == ((k_al)->At(k_im))) || (((k_ih) == (u'\u000D')) && (((k_al)->At(k_im)) == (u'\u000A')))){
return u'\u0000';
}
}
if((k_ih) == (u'\u000D')){
