...
static void k_i();
static void k_j();
static void k_l(int64_t k_av /*excpt*/);
static void k_p(type_(k_o) k_ay /*me*/);
static void k_q(k_o* k_az);
static int64_t k_r(type_(k_o) k_ba /*me*/, type_(k_o) k_bb /*t*/);
static type_(k_o) k_s(type_(k_o) k_bc /*me*/);
static type_(Array_<uint8_t>) k_t(type_(k_o) k_bf /*me*/);
static type_(k_o) k_u(type_(k_o) k_bh /*me*/, type_(Array_<uint8_t>) k_bi /*bin*/, int64_t* k_bj /*idx*/);
static type_(Array_<char16_t>) k_v(type_(k_o) k_bm /*me*/);
static void k_w(type_(k_n) k_bn /*me*/);
using k_bp=void(*)(type_(k_o));
static int64_t k_x(type_(k_n) k_bq /*me*/, int64_t k_br /*min*/, int64_t k_bs /*max*/);
using k_ca=uint64_t(*)(type_(k_n));
static uint64_t k_y(type_(k_n) k_cd /*me*/);
using k_cf=uint32_t(*)(type_(k_n));
static double k_z(type_(k_n) k_ci /*me*/, double k_cj /*min*/, double k_ck /*max*/);
...
static int64_t k_ak();
static int64_t k_an();
static void k_at(type_(Array_<char16_t>) k_dy /*str*/);
template<typename T_>static type_(Array_<char16_t>) k_au(T_ k_dz /*me_*/, type_(Array_<int64_t>) k_ea /*type*/);
template<typename T_>static T_ k_cg(T_ k_eb /*me_*/, type_(Array_<int64_t>) k_ec /*type*/, T_ k_ed /*n*/);
template<typename T_>static T_ k_ch(T_ k_ee /*me_*/, type_(Array_<int64_t>) k_ef /*type*/, int64_t k_eg /*n*/);
template<typename T_>static T_ k_ct(T_ k_ej /*me_*/, type_(Array_<int64_t>) k_ek /*type*/, int64_t k_el /*n*/);
static type_(Array_<char16_t>) k_ds();
static int64_t k_dv(type_(Array_<char16_t>) k_fi /*me_*/, bool* k_fj /*success*/);
static char16_t k_eu();
template<typename T_>static T_ k_fh(T_ k_fy /*me_*/, type_(Array_<int64_t>) k_fz /*type*/, int64_t k_ga /*start*/, int64_t k_gb /*len*/);
...
(k_ap) = ((((k_am) + (1LL)) / (2LL)) * (((k_ao) + (1LL)) / (2LL)));
}
(k_at)((((newArrayBin_<char16_t>(0,""))->Cat(k_au<int64_t>((k_ap), (nullptr))))->Cat(newArrayBin_<char16_t>(1,"\x0A\x00"))));
}
// _finVars
static void k_i(){
(k_al) = (nullptr);
...
type_(k_o) k_bd;
type_(k_o) k_be;
(k_bd) = (k_be = new_(k_o)(), k_p(k_be), k_be);
return as_<k_o>(classTable_, (k_bd), 0);
}
// _toBin
static type_(Array_<uint8_t>) k_t(type_(k_o) k_bf /*me*/){
type_(Array_<uint8_t>) k_bg;
(k_bg) = (newArrays_<type_(Array_<uint8_t>)>((0LL)));
return k_bg;
}
// _fromBin
static type_(k_o) k_u(type_(k_o) k_bh /*me*/, type_(Array_<uint8_t>) k_bi /*bin*/, int64_t* k_bj /*idx*/){
...
uint64_t k_bw; // m
uint64_t k_bx; // r
(k_bv) = (static_cast<uint64_t>(((k_bs) - (k_br)) + (1LL)));
(k_bw) = ((0x0000000000000000ULL) - (((0x0000000000000000ULL) - (k_bv)) % (k_bv)));
(k_bx) = (0x0000000000000000ULL);
if((k_bw) == (0x0000000000000000ULL)){
(k_bx) = ((reinterpret_cast<k_ca>(classTable_[(k_bq)->Y + 9]))((k_bq)));
}
else{
do{
(k_bx) = ((reinterpret_cast<k_ca>(classTable_[(k_bq)->Y + 9]))((k_bq)));
...
return (static_cast<int64_t>((k_bx) % (k_bv))) + (k_br);
}
// rndBit64
static uint64_t k_y(type_(k_n) k_cd /*me*/){
uint32_t k_ce; // t
(k_ce) = ((reinterpret_cast<k_cf>(classTable_[(k_cd)->Y + 12]))((k_cd)));
return k_cg<uint64_t>((k_ch<uint64_t>((static_cast<uint64_t>(k_ce)), (nullptr), (32LL))), (nullptr), (static_cast<uint64_t>((reinterpret_cast<k_cf>(classTable_[(k_cd)->Y + 12]))((k_cd)))));
}
// rndFloat
static double k_z(type_(k_n) k_ci /*me*/, double k_cj /*min*/, double k_ck /*max*/){
return (((static_cast<double>((reinterpret_cast<k_ca>(classTable_[(k_ci)->Y + 9]))((k_ci)))) / (1.844674407370955e+19)) * ((k_ck) - (k_cj))) + (k_cj);
}
// setSeed
static void k_aa(type_(k_n) k_cn /*me*/, uint32_t k_co /*seed*/){
...
static uint32_t k_ab(type_(k_n) k_cu /*me*/){
uint32_t k_cv; // t
(k_cv) = (k_ai<uint32_t>(((k_cu)->k_cp), (nullptr), (k_ch<uint32_t>(((k_cu)->k_cp), (nullptr), (11LL)))));
((k_cu)->k_cp) = ((k_cu)->k_cq);
((k_cu)->k_cq) = ((k_cu)->k_cr);
((k_cu)->k_cr) = ((k_cu)->k_cs);
...
#if defined(_WIN32)
fputws(reinterpret_cast<const wchar_t*>(k_dy->B),stdout);
#else
std::u16string s_=k_dy->B;
...
// inputStr
static type_(Array_<char16_t>) k_ds(){
type_(Array_<char16_t>) k_eo; // buf
int64_t k_ep; // ptr
char16_t k_et; // c
(k_eo) = (newArrays_<type_(Array_<char16_t>)>((1024LL)));
(k_ep) = (0LL);
while(true){
(k_et) = ((k_eu)());
if((k_et) == (u'\uFFFF')){
...
if((k_ep) == ((k_eo)->Len())){
(k_eo) = ((k_eo)->Cat(newArrays_<type_(Array_<char16_t>)>(((k_eo)->Len()))));
}
((k_eo)->At(k_ep)) = (k_et);
(k_ep) = ((k_ep) + (1LL));
}
return k_fh<type_(Array_<char16_t>)>((k_eo), (nullptr), (0LL), (k_ep));
}
// _toInt
static int64_t k_dv(type_(Array_<char16_t>) k_fi /*me_*/, bool* k_fj /*success*/){
std::u16string u_=k_fi->B;
...
while(true){
(k_fn) = ((k_fo)());
for(k_fs = (0LL), k_ft = (((k_al)->Len()) - (1LL)); k_fs <= k_ft; k_fs += (1LL)){
if(((k_fn) == ((k_al)->At(k_fs))) || (((k_fn) == (u'\u000D')) && (((k_al)->At(k_fs)) == (u'\u000A')))){
return u'\u0000';
}
}
if((k_fn) == (u'\u000D')){
