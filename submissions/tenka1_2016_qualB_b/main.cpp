...
int main(){
  string S = in();
  ll N = sz(S);
  // dp[i文字目][level] = そこまでの操作回数。
  vvi dp = Vector(N, N, inf);
  if(S[0] == '('){
    dp[0][1] = 0;
  }else{
    dp[0][1] = 1;
  ...
  rep(i, 1, N){
    rep(level, N){
      if(dp[i - 1][level] == -1) continue;
      if(S[i] == '('){
        chmin(dp[i][level + 1], dp[i - 1][level] + 1);
        if(level != 0) chmin(dp[i][level - 1], dp[i - 1][level] + 2);
      }else{
        if(level != 0) chmin(dp[i][level - 1], dp[i - 1][level] + 1);
        chmin(dp[i][level + 1], dp[i - 1][level] + 2);
      }
    }
  }
  // x文字目でlevelの状態から、それ以降変更せずに対応の取れた文字列になっているか否か。
  auto f = [&](ll x, ll level){
    rep(i, x + 1, N){
      if(S[i] == '(') level++;
      else level--;
  ...
  rep(i, N){
    rep(level, N){
      if(f(i, level)){ // これ以降操作不要
        chmin(ans, dp[i][level]);
      }
    }
  }
  out(ans);
