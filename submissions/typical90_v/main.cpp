...
static void k_j();
static void k_k();
static void k_m(int64_t k_bm /*excpt*/);
static void k_q(type_(k_p) k_bp /*me*/);
static void k_r(k_p* k_bq);
static int64_t k_s(type_(k_p) k_br /*me*/, type_(k_p) k_bs /*t*/);
static type_(k_p) k_t(type_(k_p) k_bt /*me*/);
static type_(Array_<uint8_t>) k_u(type_(k_p) k_bw /*me*/);
static type_(k_p) k_v(type_(k_p) k_by /*me*/, type_(Array_<uint8_t>) k_bz /*bin*/, int64_t* k_ca /*idx*/);
static type_(Array_<char16_t>) k_w(type_(k_p) k_cd /*me*/);
static void k_x(type_(k_o) k_ce /*me*/);
using k_cg=void(*)(type_(k_p));
static int64_t k_y(type_(k_o) k_ch /*me*/, int64_t k_ci /*min*/, int64_t k_cj /*max*/);
using k_cr=uint64_t(*)(type_(k_o));
static uint64_t k_z(type_(k_o) k_cu /*me*/);
using k_cw=uint32_t(*)(type_(k_o));
static double k_aa(type_(k_o) k_cz /*me*/, double k_da /*min*/, double k_db /*max*/);
static void k_ab(type_(k_o) k_de /*me*/, uint32_t k_df /*seed*/);
static uint32_t k_ac(type_(k_o) k_dl /*me*/);
static void k_ad(k_o* k_dn);
static type_(k_o) k_ae(type_(k_o) k_do);
static type_(Array_<uint8_t>) k_af(type_(k_o) k_dr);
static type_(k_o) k_ag(type_(k_o) k_dx, type_(Array_<uint8_t>) k_dy, int64_t* k_dz);
static type_(k_o) k_ah(type_(k_o) k_ec /*me2*/, uint32_t k_ed /*seed*/);
using k_ee=void(*)(type_(k_o), uint32_t);
template<typename T_>static T_ k_aj(T_ k_ef /*me_*/, type_(Array_<int64_t>) k_eg /*type*/, T_ k_eh /*n*/);
static int64_t k_ak();
static int64_t k_al();
static int64_t k_av();
static int64_t k_bc(int64_t k_ep /*a*/, int64_t k_eq /*b*/);
static void k_bk(type_(Array_<char16_t>) k_fk /*str*/);
template<typename T_>static type_(Array_<char16_t>) k_bl(T_ k_fl /*me_*/, type_(Array_<int64_t>) k_fm /*type*/);
template<typename T_>static T_ k_cx(T_ k_fn /*me_*/, type_(Array_<int64_t>) k_fo /*type*/, T_ k_fp /*n*/);
template<typename T_>static T_ k_cy(T_ k_fq /*me_*/, type_(Array_<int64_t>) k_fr /*type*/, int64_t k_fs /*n*/);
template<typename T_>static T_ k_dk(T_ k_fv /*me_*/, type_(Array_<int64_t>) k_fw /*type*/, int64_t k_fx /*n*/);
static type_(Array_<char16_t>) k_ej();
static int64_t k_em(type_(Array_<char16_t>) k_gu /*me_*/, bool* k_gv /*success*/);
static char16_t k_gg();
template<typename T_>static T_ k_gt(T_ k_hk /*me_*/, type_(Array_<int64_t>) k_hl /*type*/, int64_t k_hm /*start*/, int64_t k_hn /*len*/);
static char16_t k_ha();
...
k_o::k_o(): k_p(), k_dg(), k_dh(), k_di(), k_dj(){ Y = 8; }
k_o::~k_o(){
k_ad(this);
}
// k_main_
...
int64_t k_bb;
int64_t k_bi;
(k_ap) = (newArrays_<type_(Array_<int64_t>)>((3LL)));
for(k_at = (0LL), k_au = (2LL); k_at <= k_au; k_at += (1LL)){
((k_ap)->At(k_at)) = ((k_av)());
}
(k_aw) = ((k_ap)->At(0LL));
for(k_ba = (1LL), k_bb = (2LL); k_ba <= k_bb; k_ba += (1LL)){
(k_aw) = ((k_bc)((k_aw), ((k_ap)->At(k_ba))));
}
(k_bd) = (0LL);
for(k_bh = (0LL), k_bi = (2LL); k_bh <= k_bi; k_bh += (1LL)){
(k_bd) = ((k_bd) + ((((k_ap)->At(k_bh)) / ((k_bc)((k_aw), ((k_ap)->At(k_bh))))) - (1LL)));
}
(k_bk)((((newArrayBin_<char16_t>(0,""))->Cat(k_bl<int64_t>((k_bd), (nullptr))))->Cat(newArrayBin_<char16_t>(1,"\x0A\x00"))));
}
// _finVars
static void k_j(){
(k_ao) = (nullptr);
...
type_(k_p) k_bu;
type_(k_p) k_bv;
(k_bu) = (k_bv = new_(k_p)(), k_q(k_bv), k_bv);
return as_<k_p>(classTable_, (k_bu), 0);
}
// _toBin
static type_(Array_<uint8_t>) k_u(type_(k_p) k_bw /*me*/){
...
type_(k_p) k_cb;
type_(k_p) k_cc;
(k_cb) = (k_cc = new_(k_p)(), k_q(k_cc), k_cc);
return k_cb;
}
// toStr
static type_(Array_<char16_t>) k_w(type_(k_p) k_cd /*me*/){
...
static uint32_t k_ac(type_(k_o) k_dl /*me*/){
uint32_t k_dm; // t
(k_dm) = (k_aj<uint32_t>(((k_dl)->k_dg), (nullptr), (k_cy<uint32_t>(((k_dl)->k_dg), (nullptr), (11LL)))));
((k_dl)->k_dg) = ((k_dl)->k_dh);
((k_dl)->k_dh) = ((k_dl)->k_di);
((k_dl)->k_di) = ((k_dl)->k_dj);
((k_dl)->k_dj) = (k_aj<uint32_t>((k_aj<uint32_t>(((k_dl)->k_dj), (nullptr), (k_dk<uint32_t>(((k_dl)->k_dj), (nullptr), (19LL))))), (nullptr), (k_aj<uint32_t>((k_dm), (nullptr), (k_dk<uint32_t>((k_dm), (nullptr), (8LL)))))));
return (k_dl)->k_dj;
}
// _dtor
static void k_ad(k_o* k_dn){
...
type_(k_o) k_dp;
type_(k_o) k_dq;
(k_dp) = (k_dq = new_(k_o)(), k_x(k_dq), k_dq);
((k_dp)->k_dg) = ((k_do)->k_dg);
((k_dp)->k_dh) = ((k_do)->k_dh);
((k_dp)->k_di) = ((k_do)->k_di);
((k_dp)->k_dj) = ((k_do)->k_dj);
return as_<k_o>(classTable_, (k_dp), 8);
}
// _toBin
static type_(Array_<uint8_t>) k_af(type_(k_o) k_dr){
...
#if defined(_WIN32)
fputws(reinterpret_cast<const wchar_t*>(k_fk->B),stdout);
#else
std::u16string s_=k_fk->B;
...
// inputStr
static type_(Array_<char16_t>) k_ej(){
type_(Array_<char16_t>) k_ga; // buf
int64_t k_gb; // ptr
char16_t k_gf; // c
(k_ga) = (newArrays_<type_(Array_<char16_t>)>((1024LL)));
(k_gb) = (0LL);
while(true){
(k_gf) = ((k_gg)());
if((k_gf) == (u'\uFFFF')){
...
if((k_gb) == ((k_ga)->Len())){
(k_ga) = ((k_ga)->Cat(newArrays_<type_(Array_<char16_t>)>(((k_ga)->Len()))));
}
((k_ga)->At(k_gb)) = (k_gf);
(k_gb) = ((k_gb) + (1LL));
}
return k_gt<type_(Array_<char16_t>)>((k_ga), (nullptr), (0LL), (k_gb));
}
// _toInt
static int64_t k_em(type_(Array_<char16_t>) k_gu /*me_*/, bool* k_gv /*success*/){
std::u16string u_=k_gu->B;
...
while(true){
(k_gz) = ((k_ha)());
for(k_he = (0LL), k_hf = (((k_ao)->Len()) - (1LL)); k_he <= k_hf; k_he += (1LL)){
if(((k_gz) == ((k_ao)->At(k_he))) || (((k_gz) == (u'\u000D')) && (((k_ao)->At(k_he)) == (u'\u000A')))){
return u'\u0000';
}
}
if((k_gz) == (u'\u000D')){
