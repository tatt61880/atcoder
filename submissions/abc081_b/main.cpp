...
static void k_i();
static void k_j();
static void k_l(int64_t k_bj /*excpt*/);
static void k_p(type_(k_o) k_bm /*me*/);
static void k_q(k_o* k_bn);
static int64_t k_r(type_(k_o) k_bo /*me*/, type_(k_o) k_bp /*t*/);
static type_(k_o) k_s(type_(k_o) k_bq /*me*/);
static type_(Array_<uint8_t>) k_t(type_(k_o) k_bt /*me*/);
static type_(k_o) k_u(type_(k_o) k_bv /*me*/, type_(Array_<uint8_t>) k_bw /*bin*/, int64_t* k_bx /*idx*/);
static type_(Array_<char16_t>) k_v(type_(k_o) k_ca /*me*/);
static void k_w(type_(k_n) k_cb /*me*/);
using k_cd=void(*)(type_(k_o));
static int64_t k_x(type_(k_n) k_ce /*me*/, int64_t k_cf /*min*/, int64_t k_cg /*max*/);
...
static int64_t k_ak();
static int64_t k_an();
static void k_bh(type_(Array_<char16_t>) k_em /*str*/);
template<typename T_>static type_(Array_<char16_t>) k_bi(T_ k_en /*me_*/, type_(Array_<int64_t>) k_eo /*type*/);
template<typename T_>static T_ k_cu(T_ k_ep /*me_*/, type_(Array_<int64_t>) k_eq /*type*/, T_ k_er /*n*/);
template<typename T_>static T_ k_cv(T_ k_es /*me_*/, type_(Array_<int64_t>) k_et /*type*/, int64_t k_eu /*n*/);
...
k_aw:;
(k_bh)((((newArrayBin_<char16_t>(0,""))->Cat(k_bi<int64_t>((k_au), (nullptr))))->Cat(newArrayBin_<char16_t>(1,"\x0A\x00"))));
}
// _finVars
static void k_i(){
(k_al) = (nullptr);
...
return as_<k_o>(classTable_, (k_br), 0);
}
// _toBin
static type_(Array_<uint8_t>) k_t(type_(k_o) k_bt /*me*/){
type_(Array_<uint8_t>) k_bu;
(k_bu) = (newArrays_<type_(Array_<uint8_t>)>((0LL)));
return k_bu;
}
// _fromBin
static type_(k_o) k_u(type_(k_o) k_bv /*me*/, type_(Array_<uint8_t>) k_bw /*bin*/, int64_t* k_bx /*idx*/){
type_(k_o) k_by;
type_(k_o) k_bz;
(k_by) = (k_bz = new_(k_o)(), k_p(k_bz), k_bz);
return k_by;
}
// toStr
static type_(Array_<char16_t>) k_v(type_(k_o) k_ca /*me*/){
...
return (static_cast<int64_t>((k_cl) % (k_cj))) + (k_cf);
}
// rndBit64
static uint64_t k_y(type_(k_n) k_cr /*me*/){
uint32_t k_cs; // t
...
static uint32_t k_ab(type_(k_n) k_di /*me*/){
uint32_t k_dj; // t
(k_dj) = (k_ai<uint32_t>(((k_di)->k_dd), (nullptr), (k_cv<uint32_t>(((k_di)->k_dd), (nullptr), (11LL)))));
((k_di)->k_dd) = ((k_di)->k_de);
((k_di)->k_de) = ((k_di)->k_df);
((k_di)->k_df) = ((k_di)->k_dg);
