...
static int64_t k_ak();
static int64_t k_an();
template<typename T_,typename C_>static void k_bb(T_ k_ez /*me_*/, type_(Array_<int64_t>) k_fa /*type*/, C_ k_fb /*item*/);
static void k_bd(type_(Array_<type_(List_<int64_t>)>) k_fc /*graph*/, int64_t k_fd /*u*/, int64_t k_fe /*p*/, type_(Array_<int64_t>) k_ff /*a*/);
template<typename T_>static void k_bk(T_ k_fl /*me_*/, type_(Array_<int64_t>) k_fm /*type*/);
template<typename T_>static bool k_bm(T_ k_fn /*me_*/, type_(Array_<int64_t>) k_fo /*type*/);
template<typename T_,typename R_>static R_ k_bo(T_ k_fp /*me_*/, type_(Array_<int64_t>) k_fq /*type*/);
template<typename T_,typename R_>static R_ k_bq(T_ k_ft /*me_*/, type_(Array_<int64_t>) k_fu /*type*/);
template<typename T_>static void k_bs(T_ k_fv /*me_*/, type_(Array_<int64_t>) k_fw /*type*/);
static void k_bu(type_(Array_<char16_t>) k_fx /*str*/);
template<typename T_>static type_(Array_<char16_t>) k_bv(T_ k_fy /*me_*/, type_(Array_<int64_t>) k_fz /*type*/);
template<typename T_>static T_ k_dh(T_ k_ga /*me_*/, type_(Array_<int64_t>) k_gb /*type*/, T_ k_gc /*n*/);
template<typename T_>static T_ k_di(T_ k_gd /*me_*/, type_(Array_<int64_t>) k_ge /*type*/, int64_t k_gf /*n*/);
template<typename T_>static T_ k_du(T_ k_gi /*me_*/, type_(Array_<int64_t>) k_gj /*type*/, int64_t k_gk /*n*/);
static type_(Array_<char16_t>) k_et();
static int64_t k_ew(type_(Array_<char16_t>) k_hh /*me_*/, bool* k_hi /*success*/);
static char16_t k_gt();
template<typename T_>static T_ k_hg(T_ k_hx /*me_*/, type_(Array_<int64_t>) k_hy /*type*/, int64_t k_hz /*start*/, int64_t k_ia /*len*/);
static char16_t k_hn();
...
(k_bc) = (newArrays_<type_(Array_<int64_t>)>((k_am)));
(k_bd)((k_ao), (0LL), (-1LL), (k_bc));
(k_be) = (0LL);
for(k_bi = (0LL), k_bj = ((k_am) - (1LL)); k_bi <= k_bj; k_bi += (1LL)){
k_bk<type_(List_<int64_t>)>(((k_ao)->At(k_bi)), (nullptr));
while(!(k_bm<type_(List_<int64_t>)>(((k_ao)->At(k_bi)), (nullptr)))){
(k_bn) = (k_bo<type_(List_<int64_t>), int64_t>(((k_ao)->At(k_bi)), (nullptr)));
(k_bp) = (k_bq<type_(Array_<int64_t>), int64_t>((new_(Array_<int64_t>)(((k_bc)->At(k_bi)), ((k_bc)->At(k_bn)))), (nullptr)));
(k_be) = ((k_be) + ((k_bp) * ((k_am) - (k_bp))));
k_bs<type_(List_<int64_t>)>(((k_ao)->At(k_bi)), (nullptr));
}
}
(k_be) = ((k_be) / (2LL));
(k_bu)((((newArrayBin_<char16_t>(0,""))->Cat(k_bv<int64_t>((k_be), (nullptr))))->Cat(newArrayBin_<char16_t>(1,"\x0A\x00"))));
}
// _finVars
static void k_i(){
(k_al) = (nullptr);
...
// _addList
template<typename T_,typename C_>static void k_bb(T_ k_ez /*me_*/, type_(Array_<int64_t>) k_fa /*type*/, C_ k_fb /*item*/){
k_ez->B.push_back(k_fb);
}
// dfs
static void k_bd(type_(Array_<type_(List_<int64_t>)>) k_fc /*graph*/, int64_t k_fd /*u*/, int64_t k_fe /*p*/, type_(Array_<int64_t>) k_ff /*a*/){
int64_t k_fh; // v
((k_ff)->At(k_fd)) = (1LL);
k_bk<type_(List_<int64_t>)>(((k_fc)->At(k_fd)), (nullptr));
while(!(k_bm<type_(List_<int64_t>)>(((k_fc)->At(k_fd)), (nullptr)))){
(k_fh) = (k_bo<type_(List_<int64_t>), int64_t>(((k_fc)->At(k_fd)), (nullptr)));
if((k_fh) != (k_fe)){
(k_bd)((k_fc), (k_fh), (k_fd), (k_ff));
((k_ff)->At(k_fd)) = (((k_ff)->At(k_fd)) + ((k_ff)->At(k_fh)));
}
k_bs<type_(List_<int64_t>)>(((k_fc)->At(k_fd)), (nullptr));
}
}
// _head
template<typename T_>static void k_bk(T_ k_fl /*me_*/, type_(Array_<int64_t>) k_fm /*type*/){
...
if(cmp_(r_,k_ft->B[i_])>0)r_=k_ft->B[i_];
}
return r_;
}
// _next
template<typename T_>static void k_bs(T_ k_fv /*me_*/, type_(Array_<int64_t>) k_fw /*type*/){
if(k_fv->I==k_fv->B.end())return;
++k_fv->I;
}
// print
static void k_bu(type_(Array_<char16_t>) k_fx /*str*/){
...
#if defined(_WIN32)
fputws(reinterpret_cast<const wchar_t*>(k_fx->B),stdout);
#else
std::u16string s_=k_fx->B;
...
// inputStr
static type_(Array_<char16_t>) k_et(){
type_(Array_<char16_t>) k_gn; // buf
int64_t k_go; // ptr
char16_t k_gs; // c
(k_gn) = (newArrays_<type_(Array_<char16_t>)>((1024LL)));
(k_go) = (0LL);
while(true){
(k_gs) = ((k_gt)());
if((k_gs) == (u'\uFFFF')){
...
if((k_go) == ((k_gn)->Len())){
(k_gn) = ((k_gn)->Cat(newArrays_<type_(Array_<char16_t>)>(((k_gn)->Len()))));
}
((k_gn)->At(k_go)) = (k_gs);
(k_go) = ((k_go) + (1LL));
}
return k_hg<type_(Array_<char16_t>)>((k_gn), (nullptr), (0LL), (k_go));
}
// _toInt
static int64_t k_ew(type_(Array_<char16_t>) k_hh /*me_*/, bool* k_hi /*success*/){
std::u16string u_=k_hh->B;
...
while(true){
(k_hm) = ((k_hn)());
for(k_hr = (0LL), k_hs = (((k_al)->Len()) - (1LL)); k_hr <= k_hs; k_hr += (1LL)){
if(((k_hm) == ((k_al)->At(k_hr))) || (((k_hm) == (u'\u000D')) && (((k_al)->At(k_hr)) == (u'\u000A')))){
return u'\u0000';
}
}
if((k_hm) == (u'\u000D')){
