...
static void k_j();
static void k_k();
static void k_m(int64_t k_aw /*excpt*/);
static void k_q(type_(k_p) k_az /*me*/);
static void k_r(k_p* k_ba);
static int64_t k_s(type_(k_p) k_bb /*me*/, type_(k_p) k_bc /*t*/);
static type_(k_p) k_t(type_(k_p) k_bd /*me*/);
static type_(Array_<uint8_t>) k_u(type_(k_p) k_bg /*me*/);
static type_(k_p) k_v(type_(k_p) k_bi /*me*/, type_(Array_<uint8_t>) k_bj /*bin*/, int64_t* k_bk /*idx*/);
static type_(Array_<char16_t>) k_w(type_(k_p) k_bn /*me*/);
static void k_x(type_(k_o) k_bo /*me*/);
using k_bq=void(*)(type_(k_p));
static int64_t k_y(type_(k_o) k_br /*me*/, int64_t k_bs /*min*/, int64_t k_bt /*max*/);
using k_cb=uint64_t(*)(type_(k_o));
static uint64_t k_z(type_(k_o) k_ce /*me*/);
using k_cg=uint32_t(*)(type_(k_o));
static double k_aa(type_(k_o) k_cj /*me*/, double k_ck /*min*/, double k_cl /*max*/);
static void k_ab(type_(k_o) k_co /*me*/, uint32_t k_cp /*seed*/);
static uint32_t k_ac(type_(k_o) k_cv /*me*/);
static void k_ad(k_o* k_cx);
static type_(k_o) k_ae(type_(k_o) k_cy);
static type_(Array_<uint8_t>) k_af(type_(k_o) k_db);
static type_(k_o) k_ag(type_(k_o) k_dh, type_(Array_<uint8_t>) k_di, int64_t* k_dj);
static type_(k_o) k_ah(type_(k_o) k_dm /*me2*/, uint32_t k_dn /*seed*/);
using k_do=void(*)(type_(k_o), uint32_t);
template<typename T_>static T_ k_aj(T_ k_dp /*me_*/, type_(Array_<int64_t>) k_dq /*type*/, T_ k_dr /*n*/);
static int64_t k_ak();
static int64_t k_al();
static int64_t k_aq();
static int64_t k_at(int64_t k_dz /*n*/);
static void k_au(type_(Array_<char16_t>) k_ew /*str*/);
template<typename T_>static type_(Array_<char16_t>) k_av(T_ k_ex /*me_*/, type_(Array_<int64_t>) k_ey /*type*/);
template<typename T_>static T_ k_ch(T_ k_ez /*me_*/, type_(Array_<int64_t>) k_fa /*type*/, T_ k_fb /*n*/);
template<typename T_>static T_ k_ci(T_ k_fc /*me_*/, type_(Array_<int64_t>) k_fd /*type*/, int64_t k_fe /*n*/);
template<typename T_>static T_ k_cu(T_ k_fh /*me_*/, type_(Array_<int64_t>) k_fi /*type*/, int64_t k_fj /*n*/);
static type_(Array_<char16_t>) k_dt();
static int64_t k_dw(type_(Array_<char16_t>) k_gg /*me_*/, bool* k_gh /*success*/);
static int64_t k_es(int64_t k_gi /*l*/, int64_t k_gj /*r*/);
static char16_t k_fs();
template<typename T_>static T_ k_gf(T_ k_gz /*me_*/, type_(Array_<int64_t>) k_ha /*type*/, int64_t k_hb /*start*/, int64_t k_hc /*len*/);
static int64_t k_gk(int64_t k_hd /*n*/);
static char16_t k_gp();
static int64_t k_hi(int64_t k_hk /*value*/, int64_t k_hl /*exponent*/, int64_t k_hm /*modulus*/);
static uint64_t k_hp(uint64_t k_hq /*value*/, uint64_t k_hr /*exponent*/, uint64_t k_hs /*modulus*/);
template<typename T_>static T_ k_hw(T_ k_hz /*me_*/, type_(Array_<int64_t>) k_ia /*type*/, T_ k_ib /*n*/);
static uint64_t k_hy(uint64_t k_ic /*a*/, uint64_t k_id /*b*/, uint64_t k_ie /*modulus*/);
...
k_o::k_o(): k_p(), k_cq(), k_cr(), k_cs(), k_ct(){ Y = 8; }
k_o::~k_o(){
k_ad(this);
}
// k_main_
...
(k_ap) = ((k_aq)());
(k_ar) = ((k_aq)());
(k_as) = ((((1000000007LL) + ((k_at)((k_ar)))) - ((k_at)(((k_ap) - (1LL))))) % (1000000007LL));
(k_au)((((newArrayBin_<char16_t>(0,""))->Cat(k_av<int64_t>((k_as), (nullptr))))->Cat(newArrayBin_<char16_t>(1,"\x0A\x00"))));
}
// _finVars
static void k_j(){
(k_ao) = (nullptr);
...
type_(k_p) k_be;
type_(k_p) k_bf;
(k_be) = (k_bf = new_(k_p)(), k_q(k_bf), k_bf);
return as_<k_p>(classTable_, (k_be), 0);
}
// _toBin
static type_(Array_<uint8_t>) k_u(type_(k_p) k_bg /*me*/){
type_(Array_<uint8_t>) k_bh;
(k_bh) = (newArrays_<type_(Array_<uint8_t>)>((0LL)));
return k_bh;
}
// _fromBin
static type_(k_p) k_v(type_(k_p) k_bi /*me*/, type_(Array_<uint8_t>) k_bj /*bin*/, int64_t* k_bk /*idx*/){
type_(k_p) k_bl;
type_(k_p) k_bm;
(k_bl) = (k_bm = new_(k_p)(), k_q(k_bm), k_bm);
return k_bl;
}
// toStr
static type_(Array_<char16_t>) k_w(type_(k_p) k_bn /*me*/){
...
return (static_cast<int64_t>((k_by) % (k_bw))) + (k_bs);
}
// rndBit64
static uint64_t k_z(type_(k_o) k_ce /*me*/){
uint32_t k_cf; // t
...
// setSeed
static void k_ab(type_(k_o) k_co /*me*/, uint32_t k_cp /*seed*/){
((k_co)->k_cq) = (0x075BCD15U);
((k_co)->k_cr) = (0x159A55E5U);
((k_co)->k_cs) = ((0x1F123BB5U) * (k_cp));
((k_co)->k_ct) = ((0x05491333U) * (k_cu<uint32_t>(((k_co)->k_cs), (nullptr), (1LL))));
}
// xs128
static uint32_t k_ac(type_(k_o) k_cv /*me*/){
uint32_t k_cw; // t
(k_cw) = (k_aj<uint32_t>(((k_cv)->k_cq), (nullptr), (k_ci<uint32_t>(((k_cv)->k_cq), (nullptr), (11LL)))));
((k_cv)->k_cq) = ((k_cv)->k_cr);
((k_cv)->k_cr) = ((k_cv)->k_cs);
((k_cv)->k_cs) = ((k_cv)->k_ct);
((k_cv)->k_ct) = (k_aj<uint32_t>((k_aj<uint32_t>(((k_cv)->k_ct), (nullptr), (k_cu<uint32_t>(((k_cv)->k_ct), (nullptr), (19LL))))), (nullptr), (k_aj<uint32_t>((k_cw), (nullptr), (k_cu<uint32_t>((k_cw), (nullptr), (8LL)))))));
return (k_cv)->k_ct;
}
// _dtor
static void k_ad(k_o* k_cx){
...
type_(k_o) k_cz;
type_(k_o) k_da;
(k_cz) = (k_da = new_(k_o)(), k_x(k_da), k_da);
((k_cz)->k_cq) = ((k_cy)->k_cq);
((k_cz)->k_cr) = ((k_cy)->k_cr);
((k_cz)->k_cs) = ((k_cy)->k_cs);
((k_cz)->k_ct) = ((k_cy)->k_ct);
return as_<k_o>(classTable_, (k_cz), 8);
}
// _toBin
static type_(Array_<uint8_t>) k_af(type_(k_o) k_db){
type_(Array_<uint8_t>) k_dc;
(k_dc) = (newArrays_<type_(Array_<uint8_t>)>((0LL)));
(k_dc) = ((k_dc)->Cat(toBin_<uint32_t>()(((k_db)->k_cq))));
(k_dc) = ((k_dc)->Cat(toBin_<uint32_t>()(((k_db)->k_cr))));
(k_dc) = ((k_dc)->Cat(toBin_<uint32_t>()(((k_db)->k_cs))));
(k_dc) = ((k_dc)->Cat(toBin_<uint32_t>()(((k_db)->k_ct))));
return k_dc;
}
// _fromBin
static type_(k_o) k_ag(type_(k_o) k_dh, type_(Array_<uint8_t>) k_di, int64_t* k_dj){
type_(k_o) k_dk;
type_(k_o) k_dl;
(k_dk) = (k_dl = new_(k_o)(), k_x(k_dl), k_dl);
((k_dk)->k_cq) = (fromBin_<uint32_t>()((k_di), (*k_dj)));
((k_dk)->k_cr) = (fromBin_<uint32_t>()((k_di), (*k_dj)));
((k_dk)->k_cs) = (fromBin_<uint32_t>()((k_di), (*k_dj)));
((k_dk)->k_ct) = (fromBin_<uint32_t>()((k_di), (*k_dj)));
return k_dk;
}
// makeRnd
static type_(k_o) k_ah(type_(k_o) k_dm /*me2*/, uint32_t k_dn /*seed*/){
(reinterpret_cast<k_do>(classTable_[(k_dm)->Y + 11]))((k_dm), (k_dn));
return k_dm;
}
// _xor
template<typename T_>static T_ k_aj(T_ k_dp /*me_*/, type_(Array_<int64_t>) k_dq /*type*/, T_ k_dr /*n*/){
...
(k_ec) = (newArrays_<type_(Array_<int64_t>)>((20LL)));
for(k_eg = (0LL), k_eh = (18LL); k_eg <= k_eh; k_eg += (1LL)){
((k_ec)->At(k_eg)) = (powI_((10LL), (k_eg)));
}
((k_ec)->At(19LL)) = ((powI_((10LL), (18LL))) + (1LL));
(k_ei) = (0LL);
for(k_em = (0LL), k_en = (18LL); k_em <= k_en; k_em += (1LL)){
(k_eo) = ((k_em) + (1LL));
if((k_dz) < ((k_ec)->At((k_em) + (1LL)))){
(k_ei) = ((k_ei) + ((((k_es)(((k_ec)->At(k_em)), (k_dz))) * (k_eo)) % (1000000007LL)));
(k_ei) = ((k_ei) % (1000000007LL));
break;
}
(k_ei) = ((k_ei) + ((((k_es)(((k_ec)->At(k_em)), (((k_ec)->At((k_em) + (1LL))) - (1LL)))) * (k_eo)) % (1000000007LL)));
(k_ei) = ((k_ei) % (1000000007LL));
}
return k_ei;
}
...
#if defined(_WIN32)
fputws(reinterpret_cast<const wchar_t*>(k_ew->B),stdout);
#else
std::u16string s_=k_ew->B;
...
// inputStr
static type_(Array_<char16_t>) k_dt(){
type_(Array_<char16_t>) k_fm; // buf
int64_t k_fn; // ptr
char16_t k_fr; // c
(k_fm) = (newArrays_<type_(Array_<char16_t>)>((1024LL)));
(k_fn) = (0LL);
while(true){
(k_fr) = ((k_fs)());
if((k_fr) == (u'\uFFFF')){
...
if((k_fn) == ((k_fm)->Len())){
(k_fm) = ((k_fm)->Cat(newArrays_<type_(Array_<char16_t>)>(((k_fm)->Len()))));
}
((k_fm)->At(k_fn)) = (k_fr);
(k_fn) = ((k_fn) + (1LL));
}
return k_gf<type_(Array_<char16_t>)>((k_fm), (nullptr), (0LL), (k_fn));
}
// _toInt
static int64_t k_dw(type_(Array_<char16_t>) k_gg /*me_*/, bool* k_gh /*success*/){
std::u16string u_=k_gg->B;
...
// g
static int64_t k_es(int64_t k_gi /*l*/, int64_t k_gj /*r*/){
return (((1000000007LL) + ((k_gk)((k_gj)))) - ((k_gk)(((k_gi) - (1LL))))) % (1000000007LL);
}
...
while(true){
(k_go) = ((k_gp)());
for(k_gt = (0LL), k_gu = (((k_ao)->Len()) - (1LL)); k_gt <= k_gu; k_gt += (1LL)){
if(((k_go) == ((k_ao)->At(k_gt))) || (((k_go) == (u'\u000D')) && (((k_ao)->At(k_gt)) == (u'\u000A')))){
return u'\u0000';
}
}
if((k_go) == (u'\u000D')){
...
static int64_t k_gk(int64_t k_hd /*n*/){
int64_t k_he; // res
(k_he) = ((k_hd) % (1000000007LL));
(k_he) = ((k_he) * (((k_hd) + (1LL)) % (1000000007LL)));
(k_he) = ((k_he) % (1000000007LL));
(k_he) = ((k_he) * ((k_hi)((2LL), (1000000005LL), (1000000007LL))));
(k_he) = ((k_he) % (1000000007LL));
return k_he;
}
// inputLetter
...
// modPow
static int64_t k_hi(int64_t k_hk /*value*/, int64_t k_hl /*exponent*/, int64_t k_hm /*modulus*/){
if((((k_hk) < (0LL)) || ((k_hl) < (0LL))) || ((k_hm) < (0LL))){
throw 3910598662LL;
}
return static_cast<int64_t>((k_hp)((static_cast<uint64_t>(k_hk)), (static_cast<uint64_t>(k_hl)), (static_cast<uint64_t>(k_hm))));
}
// modPowUnsigned
static uint64_t k_hp(uint64_t k_hq /*value*/, uint64_t k_hr /*exponent*/, uint64_t k_hs /*modulus*/){
uint64_t k_ht; // w
(k_ht) = (0x0000000000000001ULL);
while((k_hr) > (0x0000000000000000ULL)){
if((k_hw<uint64_t>((k_hr), (nullptr), (0x0000000000000001ULL))) != (0x0000000000000000ULL)){
(k_ht) = ((k_hy)((k_ht), (k_hq), (k_hs)));
}
(k_hq) = ((k_hy)((k_hq), (k_hq), (k_hs)));
(k_hr) = (k_cu<uint64_t>((k_hr), (nullptr), (1LL)));
}
return k_ht;
}
// _and
...
(k_ii) = (0x0000000000000000ULL);
while((k_ic) > (0x0000000000000000ULL)){
if((k_hw<uint64_t>((k_ic), (nullptr), (0x0000000000000001ULL))) != (0x0000000000000000ULL)){
(k_ii) = (((k_ii) + (k_id)) % (k_ie));
}
(k_id) = ((k_ci<uint64_t>((k_id), (nullptr), (1LL))) % (k_ie));
(k_ic) = (k_cu<uint64_t>((k_ic), (nullptr), (1LL)));
}
return k_ii;
}
