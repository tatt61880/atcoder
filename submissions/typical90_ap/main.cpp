...
static void k_i();
static void k_j();
static void k_l(int64_t k_bi /*excpt*/);
static void k_p(type_(k_o) k_bl /*me*/);
static void k_q(k_o* k_bm);
static int64_t k_r(type_(k_o) k_bn /*me*/, type_(k_o) k_bo /*t*/);
static type_(k_o) k_s(type_(k_o) k_bp /*me*/);
static type_(Array_<uint8_t>) k_t(type_(k_o) k_bs /*me*/);
static type_(k_o) k_u(type_(k_o) k_bu /*me*/, type_(Array_<uint8_t>) k_bv /*bin*/, int64_t* k_bw /*idx*/);
static type_(Array_<char16_t>) k_v(type_(k_o) k_bz /*me*/);
static void k_w(type_(k_n) k_ca /*me*/);
using k_cc=void(*)(type_(k_o));
static int64_t k_x(type_(k_n) k_cd /*me*/, int64_t k_ce /*min*/, int64_t k_cf /*max*/);
...
static void k_aa(type_(k_n) k_da /*me*/, uint32_t k_db /*seed*/);
static uint32_t k_ab(type_(k_n) k_dh /*me*/);
static void k_ac(k_n* k_dj);
static type_(k_n) k_ad(type_(k_n) k_dk);
static type_(Array_<uint8_t>) k_ae(type_(k_n) k_dn);
static type_(k_n) k_af(type_(k_n) k_dt, type_(Array_<uint8_t>) k_du, int64_t* k_dv);
static type_(k_n) k_ag(type_(k_n) k_dy /*me2*/, uint32_t k_dz /*seed*/);
using k_ea=void(*)(type_(k_n), uint32_t);
template<typename T_>static T_ k_ai(T_ k_eb /*me_*/, type_(Array_<int64_t>) k_ec /*type*/, T_ k_ed /*n*/);
static int64_t k_aj();
static int64_t k_ak();
static int64_t k_an();
static void k_aq(type_(Array_<char16_t>) k_el /*str*/);
template<typename T_>static type_(Array_<char16_t>) k_bh(T_ k_em /*me_*/, type_(Array_<int64_t>) k_en /*type*/);
template<typename T_>static T_ k_ct(T_ k_eo /*me_*/, type_(Array_<int64_t>) k_ep /*type*/, T_ k_eq /*n*/);
template<typename T_>static T_ k_cu(T_ k_er /*me_*/, type_(Array_<int64_t>) k_es /*type*/, int64_t k_et /*n*/);
template<typename T_>static T_ k_dg(T_ k_ew /*me_*/, type_(Array_<int64_t>) k_ex /*type*/, int64_t k_ey /*n*/);
static type_(Array_<char16_t>) k_ef();
static int64_t k_ei(type_(Array_<char16_t>) k_fv /*me_*/, bool* k_fw /*success*/);
static char16_t k_fh();
template<typename T_>static T_ k_fu(T_ k_gl /*me_*/, type_(Array_<int64_t>) k_gm /*type*/, int64_t k_gn /*start*/, int64_t k_go /*len*/);
static char16_t k_gb();
...
k_n::k_n(): k_o(), k_dc(), k_dd(), k_de(), k_df(){ Y = 8; }
k_n::~k_n(){
k_ac(this);
}
// k_main_
...
(k_am) = ((k_an)());
if(((k_am) % (9LL)) != (0LL)){
(k_aq)((newArrayBin_<char16_t>(2,"\x30\x00\x0A\x00")));
return;
}
(k_ar) = (newArrays_<type_(Array_<int64_t>)>(((k_am) + (9LL))));
((k_ar)->At(0LL)) = (1LL);
for(k_av = (0LL), k_aw = ((k_am) - (1LL)); k_av <= k_aw; k_av += (1LL)){
for(k_ba = (1LL), k_bb = (9LL); k_ba <= k_bb; k_ba += (1LL)){
(k_bd) = ((k_av) + (k_ba));
((k_ar)->At(k_bd)) = (((k_ar)->At(k_bd)) + ((k_ar)->At(k_av)));
(k_bf) = ((k_av) + (k_ba));
((k_ar)->At(k_bf)) = (((k_ar)->At(k_bf)) % ((powI_((10LL), (9LL))) + (7LL)));
}
}
(k_bg) = ((k_ar)->At(k_am));
(k_aq)((((newArrayBin_<char16_t>(0,""))->Cat(k_bh<int64_t>((k_bg), (nullptr))))->Cat(newArrayBin_<char16_t>(1,"\x0A\x00"))));
}
// _finVars
static void k_i(){
(k_al) = (nullptr);
...
type_(k_o) k_bq;
type_(k_o) k_br;
(k_bq) = (k_br = new_(k_o)(), k_p(k_br), k_br);
return as_<k_o>(classTable_, (k_bq), 0);
}
// _toBin
static type_(Array_<uint8_t>) k_t(type_(k_o) k_bs /*me*/){
type_(Array_<uint8_t>) k_bt;
(k_bt) = (newArrays_<type_(Array_<uint8_t>)>((0LL)));
return k_bt;
}
// _fromBin
static type_(k_o) k_u(type_(k_o) k_bu /*me*/, type_(Array_<uint8_t>) k_bv /*bin*/, int64_t* k_bw /*idx*/){
type_(k_o) k_bx;
type_(k_o) k_by;
(k_bx) = (k_by = new_(k_o)(), k_p(k_by), k_by);
return k_bx;
}
// toStr
static type_(Array_<char16_t>) k_v(type_(k_o) k_bz /*me*/){
...
return (static_cast<int64_t>((k_ck) % (k_ci))) + (k_ce);
}
// rndBit64
static uint64_t k_y(type_(k_n) k_cq /*me*/){
uint32_t k_cr; // t
...
// setSeed
static void k_aa(type_(k_n) k_da /*me*/, uint32_t k_db /*seed*/){
((k_da)->k_dc) = (0x075BCD15U);
((k_da)->k_dd) = (0x159A55E5U);
((k_da)->k_de) = ((0x1F123BB5U) * (k_db));
((k_da)->k_df) = ((0x05491333U) * (k_dg<uint32_t>(((k_da)->k_de), (nullptr), (1LL))));
}
// xs128
static uint32_t k_ab(type_(k_n) k_dh /*me*/){
uint32_t k_di; // t
(k_di) = (k_ai<uint32_t>(((k_dh)->k_dc), (nullptr), (k_cu<uint32_t>(((k_dh)->k_dc), (nullptr), (11LL)))));
((k_dh)->k_dc) = ((k_dh)->k_dd);
((k_dh)->k_dd) = ((k_dh)->k_de);
((k_dh)->k_de) = ((k_dh)->k_df);
((k_dh)->k_df) = (k_ai<uint32_t>((k_ai<uint32_t>(((k_dh)->k_df), (nullptr), (k_dg<uint32_t>(((k_dh)->k_df), (nullptr), (19LL))))), (nullptr), (k_ai<uint32_t>((k_di), (nullptr), (k_dg<uint32_t>((k_di), (nullptr), (8LL)))))));
return (k_dh)->k_df;
}
// _dtor
static void k_ac(k_n* k_dj){
...
type_(k_n) k_dl;
type_(k_n) k_dm;
(k_dl) = (k_dm = new_(k_n)(), k_w(k_dm), k_dm);
((k_dl)->k_dc) = ((k_dk)->k_dc);
((k_dl)->k_dd) = ((k_dk)->k_dd);
((k_dl)->k_de) = ((k_dk)->k_de);
((k_dl)->k_df) = ((k_dk)->k_df);
return as_<k_n>(classTable_, (k_dl), 8);
}
// _toBin
static type_(Array_<uint8_t>) k_ae(type_(k_n) k_dn){
type_(Array_<uint8_t>) k_do;
(k_do) = (newArrays_<type_(Array_<uint8_t>)>((0LL)));
(k_do) = ((k_do)->Cat(toBin_<uint32_t>()(((k_dn)->k_dc))));
(k_do) = ((k_do)->Cat(toBin_<uint32_t>()(((k_dn)->k_dd))));
(k_do) = ((k_do)->Cat(toBin_<uint32_t>()(((k_dn)->k_de))));
(k_do) = ((k_do)->Cat(toBin_<uint32_t>()(((k_dn)->k_df))));
return k_do;
}
// _fromBin
static type_(k_n) k_af(type_(k_n) k_dt, type_(Array_<uint8_t>) k_du, int64_t* k_dv){
type_(k_n) k_dw;
type_(k_n) k_dx;
(k_dw) = (k_dx = new_(k_n)(), k_w(k_dx), k_dx);
((k_dw)->k_dc) = (fromBin_<uint32_t>()((k_du), (*k_dv)));
((k_dw)->k_dd) = (fromBin_<uint32_t>()((k_du), (*k_dv)));
((k_dw)->k_de) = (fromBin_<uint32_t>()((k_du), (*k_dv)));
((k_dw)->k_df) = (fromBin_<uint32_t>()((k_du), (*k_dv)));
return k_dw;
}
// makeRnd
static type_(k_n) k_ag(type_(k_n) k_dy /*me2*/, uint32_t k_dz /*seed*/){
(reinterpret_cast<k_ea>(classTable_[(k_dy)->Y + 11]))((k_dy), (k_dz));
return k_dy;
}
// _xor
template<typename T_>static T_ k_ai(T_ k_eb /*me_*/, type_(Array_<int64_t>) k_ec /*type*/, T_ k_ed /*n*/){
...
#if defined(_WIN32)
fputws(reinterpret_cast<const wchar_t*>(k_el->B),stdout);
#else
std::u16string s_=k_el->B;
...
// inputStr
static type_(Array_<char16_t>) k_ef(){
type_(Array_<char16_t>) k_fb; // buf
int64_t k_fc; // ptr
char16_t k_fg; // c
(k_fb) = (newArrays_<type_(Array_<char16_t>)>((1024LL)));
(k_fc) = (0LL);
while(true){
(k_fg) = ((k_fh)());
if((k_fg) == (u'\uFFFF')){
...
if((k_fc) == ((k_fb)->Len())){
(k_fb) = ((k_fb)->Cat(newArrays_<type_(Array_<char16_t>)>(((k_fb)->Len()))));
}
((k_fb)->At(k_fc)) = (k_fg);
(k_fc) = ((k_fc) + (1LL));
}
return k_fu<type_(Array_<char16_t>)>((k_fb), (nullptr), (0LL), (k_fc));
}
// _toInt
static int64_t k_ei(type_(Array_<char16_t>) k_fv /*me_*/, bool* k_fw /*success*/){
std::u16string u_=k_fv->B;
...
while(true){
(k_ga) = ((k_gb)());
for(k_gf = (0LL), k_gg = (((k_al)->Len()) - (1LL)); k_gf <= k_gg; k_gf += (1LL)){
if(((k_ga) == ((k_al)->At(k_gf))) || (((k_ga) == (u'\u000D')) && (((k_al)->At(k_gf)) == (u'\u000A')))){
return u'\u0000';
}
}
if((k_ga) == (u'\u000D')){
