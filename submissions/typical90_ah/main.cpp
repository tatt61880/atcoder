...
static int64_t k_ak();
static int64_t k_an();
template<typename T_,typename C_,typename R_>static R_ k_be(T_ k_ez /*me_*/, type_(Array_<int64_t>) k_fa /*type*/, C_ k_fb /*key*/, bool* k_fc /*existed*/);
template<typename T_,typename C_>static void k_bi(T_ k_fd /*me_*/, type_(Array_<int64_t>) k_fe /*type*/, C_ k_ff /*key*/);
template<typename T_,typename K_,typename V_>static void k_bk(T_ k_fg /*me_*/, type_(Array_<int64_t>) k_fh /*type*/, K_ k_fi /*key*/, V_ k_fj /*item*/);
template<typename T_,typename R_>static R_ k_bt(T_ k_fk /*me_*/, type_(Array_<int64_t>) k_fl /*type*/);
static void k_bu(type_(Array_<char16_t>) k_fm /*str*/);
template<typename T_>static type_(Array_<char16_t>) k_bv(T_ k_fn /*me_*/, type_(Array_<int64_t>) k_fo /*type*/);
template<typename T_>static T_ k_dh(T_ k_fp /*me_*/, type_(Array_<int64_t>) k_fq /*type*/, T_ k_fr /*n*/);
template<typename T_>static T_ k_di(T_ k_fs /*me_*/, type_(Array_<int64_t>) k_ft /*type*/, int64_t k_fu /*n*/);
template<typename T_>static T_ k_du(T_ k_fx /*me_*/, type_(Array_<int64_t>) k_fy /*type*/, int64_t k_fz /*n*/);
static type_(Array_<char16_t>) k_et();
static int64_t k_ew(type_(Array_<char16_t>) k_gw /*me_*/, bool* k_gx /*success*/);
static char16_t k_gi();
template<typename T_>static T_ k_gv(T_ k_hm /*me_*/, type_(Array_<int64_t>) k_hn /*type*/, int64_t k_ho /*start*/, int64_t k_hp /*len*/);
...
int64_t k_at;
type_(Dict_<int64_t, int64_t>) k_av; // map
int64_t k_aw; // l
int64_t k_ax; // r
int64_t k_ay; // ans
int64_t k_bc; // key
...
(k_av) = (new_(Dict_<int64_t, int64_t>)());
(k_aw) = (0LL);
(k_ax) = (0LL);
(k_ay) = (0LL);
while((k_aw) != (k_am)){
if(((k_ax) == (k_am)) || (((k_av)->Len()) > (k_ao))){
(k_bc) = ((k_ap)->At(k_aw));
(k_bd) = ((k_be<type_(Dict_<int64_t, int64_t>), int64_t, int64_t>((k_av), (nullptr), (k_bc), (std::memset(&k_bf, 0, sizeof(bool)), &k_bf))) - (1LL));
if((k_bd) == (0LL)){
k_bi<type_(Dict_<int64_t, int64_t>), int64_t>((k_av), (nullptr), (k_bc));
}
else{
k_bk<type_(Dict_<int64_t, int64_t>), int64_t, int64_t>((k_av), (nullptr), (k_bc), (k_bd));
}
(k_aw) = ((k_aw) + (1LL));
}
else{
(k_bn) = ((k_ap)->At(k_ax));
(k_bo) = ((k_be<type_(Dict_<int64_t, int64_t>), int64_t, int64_t>((k_av), (nullptr), (k_bn), (std::memset(&k_bp, 0, sizeof(bool)), &k_bp))) + (1LL));
k_bk<type_(Dict_<int64_t, int64_t>), int64_t, int64_t>((k_av), (nullptr), (k_bn), (k_bo));
(k_ax) = ((k_ax) + (1LL));
}
if(((k_av)->Len()) <= (k_ao)){
(k_ay) = (k_bt<type_(Array_<int64_t>), int64_t>((new_(Array_<int64_t>)((k_ay), ((k_ax) - (k_aw)))), (nullptr)));
}
}
(k_bu)((((newArrayBin_<char16_t>(0,""))->Cat(k_bv<int64_t>((k_ay), (nullptr))))->Cat(newArrayBin_<char16_t>(1,"\x0A\x00"))));
}
// _finVars
static void k_i(){
(k_al) = (nullptr);
...
// _getDict
template<typename T_,typename C_,typename R_>static R_ k_be(T_ k_ez /*me_*/, type_(Array_<int64_t>) k_fa /*type*/, C_ k_fb /*key*/, bool* k_fc /*existed*/){
return dictSearch_(k_ez->B,k_fb,k_fc);
}
// _delDict
template<typename T_,typename C_>static void k_bi(T_ k_fd /*me_*/, type_(Array_<int64_t>) k_fe /*type*/, C_ k_ff /*key*/){
k_fd->Del(k_ff);
...
if(cmp_(r_,k_fk->B[i_])<0)r_=k_fk->B[i_];
}
return r_;
}
// print
...
#if defined(_WIN32)
fputws(reinterpret_cast<const wchar_t*>(k_fm->B),stdout);
#else
std::u16string s_=k_fm->B;
...
// inputStr
static type_(Array_<char16_t>) k_et(){
type_(Array_<char16_t>) k_gc; // buf
int64_t k_gd; // ptr
char16_t k_gh; // c
(k_gc) = (newArrays_<type_(Array_<char16_t>)>((1024LL)));
(k_gd) = (0LL);
while(true){
(k_gh) = ((k_gi)());
if((k_gh) == (u'\uFFFF')){
...
if((k_gd) == ((k_gc)->Len())){
(k_gc) = ((k_gc)->Cat(newArrays_<type_(Array_<char16_t>)>(((k_gc)->Len()))));
}
((k_gc)->At(k_gd)) = (k_gh);
(k_gd) = ((k_gd) + (1LL));
}
return k_gv<type_(Array_<char16_t>)>((k_gc), (nullptr), (0LL), (k_gd));
}
// _toInt
static int64_t k_ew(type_(Array_<char16_t>) k_gw /*me_*/, bool* k_gx /*success*/){
std::u16string u_=k_gw->B;
...
while(true){
(k_hb) = ((k_hc)());
for(k_hg = (0LL), k_hh = (((k_al)->Len()) - (1LL)); k_hg <= k_hh; k_hg += (1LL)){
if(((k_hb) == ((k_al)->At(k_hg))) || (((k_hb) == (u'\u000D')) && (((k_al)->At(k_hg)) == (u'\u000A')))){
return u'\u0000';
}
}
if((k_hb) == (u'\u000D')){
