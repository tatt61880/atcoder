...
int main(){
  ...
  val[n/2] = 0;
  rep(i, n){
    val[n / 2] += abs(x[i] - x[n / 2]);
  }
  rep(i, n / 2 + 1, n){
    val[i] = val[i - 1] + (2 * (i - n / 2) - (n % 2)) * (x[i] - x[i - 1]);
  }
  if(n % 2 == 0){
    val[n / 2 - 1] = val[n / 2];
  }
  rrep(i, (n - 1) / 2){
    val[i] = val[i + 1] + (2 * ((n - 1) / 2 - i) - (n % 2)) * (x[i + 1] - x[i]);
  }
  debug(val);
  loop(q){
    ll t = in();
    if(t <= x[0]){
      ...
      out(val[n - 1] + n * (t - x[n - 1]));
    }else{
      ll a = lower_bound(x, t) - x.begin() - 1;
      ll b = lower_bound(x, t) - x.begin();
      mp::cpp_int xa = x[a];
      mp::cpp_int xb = x[b];
      mp::cpp_int va = val[a];
      mp::cpp_int vb = val[b];
      out((va * (xb - t) + vb * (t - xa)) / (xb - xa));
    }
  }
}
