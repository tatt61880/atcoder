...
static int64_t k_ak();
static int64_t k_an();
static void k_ck(type_(Array_<char16_t>) k_fp /*str*/);
...
int64_t k_ao; // b
int64_t k_ap; // k
type_(Array_<int64_t>) k_aq; // dp
int64_t k_au;
int64_t k_aw; // c
int64_t k_ay;
type_(Array_<int64_t>) k_az; // ans
int64_t k_ba; // pow10
type_(Array_<int64_t>) k_be; // ansNext
int64_t k_bi;
int64_t k_bn;
int64_t k_bq;
int64_t k_bs;
type_(Array_<int64_t>) k_bu; // dpNext
int64_t k_by;
int64_t k_cd;
int64_t k_cg;
int64_t k_ci;
...
for(k_au = (0LL), k_av = ((k_ap) - (1LL)); k_au <= k_av; k_au += (1LL)){
(k_aw) = ((k_an)());
(k_ay) = ((k_aw) % (k_ao));
((k_aq)->At(k_ay)) = (((k_aq)->At(k_ay)) + (1LL));
}
(k_az) = (newArrays_<type_(Array_<int64_t>)>((k_ao)));
(k_ba) = ((10LL) % (k_ao));
((k_az)->At(0LL)) = (1LL);
while((k_am) > (0LL)){
if(((k_am) % (2LL)) == (1LL)){
(k_be) = (newArrays_<type_(Array_<int64_t>)>((k_ao)));
for(k_bi = (0LL), k_bj = ((k_ao) - (1LL)); k_bi <= k_bj; k_bi += (1LL)){
for(k_bn = (0LL), k_bo = ((k_ao) - (1LL)); k_bn <= k_bo; k_bn += (1LL)){
(k_bq) = ((((k_bi) * (k_ba)) + (k_bn)) % (k_ao));
((k_be)->At(k_bq)) = (((k_be)->At(k_bq)) + (((k_az)->At(k_bi)) * ((k_aq)->At(k_bn))));
(k_bs) = ((((k_bi) * (k_ba)) + (k_bn)) % (k_ao));
((k_be)->At(k_bs)) = (((k_be)->At(k_bs)) % (1000000007LL));
}
}
(k_az) = (k_be);
}
(k_bu) = (newArrays_<type_(Array_<int64_t>)>((k_ao)));
for(k_by = (0LL), k_bz = ((k_ao) - (1LL)); k_by <= k_bz; k_by += (1LL)){
for(k_cd = (0LL), k_ce = ((k_ao) - (1LL)); k_cd <= k_ce; k_cd += (1LL)){
(k_cg) = ((((k_by) * (k_ba)) + (k_cd)) % (k_ao));
((k_bu)->At(k_cg)) = (((k_bu)->At(k_cg)) + (((k_aq)->At(k_by)) * ((k_aq)->At(k_cd))));
(k_ci) = ((((k_by) * (k_ba)) + (k_cd)) % (k_ao));
((k_bu)->At(k_ci)) = (((k_bu)->At(k_ci)) % (1000000007LL));
}
}
(k_aq) = (k_bu);
(k_ba) = ((powI_((k_ba), (2LL))) % (k_ao));
(k_am) = ((k_am) / (2LL));
}
(k_ck)((((newArrayBin_<char16_t>(0,""))->Cat(k_cl<int64_t>(((k_az)->At(0LL)), (nullptr))))->Cat(newArrayBin_<char16_t>(1,"\x0A\x00"))));
}
// _finVars
static void k_i(){
(k_al) = (nullptr);
