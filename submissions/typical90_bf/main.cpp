...
static void k_i();
static void k_j();
static void k_l(int64_t k_bn /*excpt*/);
static void k_p(type_(k_o) k_bq /*me*/);
static void k_q(k_o* k_br);
static int64_t k_r(type_(k_o) k_bs /*me*/, type_(k_o) k_bt /*t*/);
static type_(k_o) k_s(type_(k_o) k_bu /*me*/);
static type_(Array_<uint8_t>) k_t(type_(k_o) k_bx /*me*/);
static type_(k_o) k_u(type_(k_o) k_bz /*me*/, type_(Array_<uint8_t>) k_ca /*bin*/, int64_t* k_cb /*idx*/);
static type_(Array_<char16_t>) k_v(type_(k_o) k_ce /*me*/);
static void k_w(type_(k_n) k_cf /*me*/);
using k_ch=void(*)(type_(k_o));
static int64_t k_x(type_(k_n) k_ci /*me*/, int64_t k_cj /*min*/, int64_t k_ck /*max*/);
using k_cs=uint64_t(*)(type_(k_n));
static uint64_t k_y(type_(k_n) k_cv /*me*/);
using k_cx=uint32_t(*)(type_(k_n));
static double k_z(type_(k_n) k_da /*me*/, double k_db /*min*/, double k_dc /*max*/);
static void k_aa(type_(k_n) k_df /*me*/, uint32_t k_dg /*seed*/);
static uint32_t k_ab(type_(k_n) k_dm /*me*/);
static void k_ac(k_n* k_do);
static type_(k_n) k_ad(type_(k_n) k_dp);
static type_(Array_<uint8_t>) k_ae(type_(k_n) k_ds);
static type_(k_n) k_af(type_(k_n) k_dy, type_(Array_<uint8_t>) k_dz, int64_t* k_ea);
static type_(k_n) k_ag(type_(k_n) k_ed /*me2*/, uint32_t k_ee /*seed*/);
using k_ef=void(*)(type_(k_n), uint32_t);
template<typename T_>static T_ k_ai(T_ k_eg /*me_*/, type_(Array_<int64_t>) k_eh /*type*/, T_ k_ei /*n*/);
static int64_t k_aj();
static int64_t k_ak();
static int64_t k_an();
static int64_t k_av(int64_t k_eq /*n*/);
template<typename T_,typename C_,typename R_>static R_ k_ax(T_ k_ev /*me_*/, type_(Array_<int64_t>) k_ew /*type*/, C_ k_ex /*key*/, bool* k_ey /*existed*/);
template<typename T_,typename K_,typename V_>static void k_bj(T_ k_ez /*me_*/, type_(Array_<int64_t>) k_fa /*type*/, K_ k_fb /*key*/, V_ k_fc /*item*/);
static void k_bl(type_(Array_<char16_t>) k_fd /*str*/);
template<typename T_>static type_(Array_<char16_t>) k_bm(T_ k_fe /*me_*/, type_(Array_<int64_t>) k_ff /*type*/);
template<typename T_>static T_ k_cy(T_ k_fg /*me_*/, type_(Array_<int64_t>) k_fh /*type*/, T_ k_fi /*n*/);
template<typename T_>static T_ k_cz(T_ k_fj /*me_*/, type_(Array_<int64_t>) k_fk /*type*/, int64_t k_fl /*n*/);
template<typename T_>static T_ k_dl(T_ k_fo /*me_*/, type_(Array_<int64_t>) k_fp /*type*/, int64_t k_fq /*n*/);
static type_(Array_<char16_t>) k_ek();
static int64_t k_en(type_(Array_<char16_t>) k_gn /*me_*/, bool* k_go /*success*/);
static char16_t k_fz();
template<typename T_>static T_ k_gm(T_ k_hd /*me_*/, type_(Array_<int64_t>) k_he /*type*/, int64_t k_hf /*start*/, int64_t k_hg /*len*/);
static char16_t k_gt();
...
k_n::k_n(): k_o(), k_dh(), k_di(), k_dj(), k_dk(){ Y = 8; }
k_n::~k_n(){
k_ac(this);
}
// k_main_
...
int64_t k_am; // n
int64_t k_ao; // k
type_(Dict_<int64_t, int64_t>) k_ap; // map
int64_t k_at;
int64_t k_aw; // prev
int64_t k_bb; // len
int64_t k_bh;
...
(k_am) = ((k_an)());
(k_ao) = ((k_an)());
(k_ap) = (new_(Dict_<int64_t, int64_t>)());
for(k_at = (1LL), k_au = (k_ao); k_at <= k_au; k_at += (1LL)){
(k_am) = ((k_av)((k_am)));
(k_aw) = (k_ax<type_(Dict_<int64_t, int64_t>), int64_t, int64_t>((k_ap), (nullptr), (k_am), (std::memset(&k_ay, 0, sizeof(bool)), &k_ay)));
if((k_aw) != (0LL)){
(k_bb) = ((k_at) - (k_aw));
(k_ao) = ((k_ao) - (k_aw));
(k_ao) = ((k_ao) % (k_bb));
for(k_bh = (1LL), k_bi = (k_ao); k_bh <= k_bi; k_bh += (1LL)){
(k_am) = ((k_av)((k_am)));
}
break;
}
k_bj<type_(Dict_<int64_t, int64_t>), int64_t, int64_t>((k_ap), (nullptr), (k_am), (k_at));
}
(k_bk) = (k_am);
(k_bl)((((newArrayBin_<char16_t>(0,""))->Cat(k_bm<int64_t>((k_bk), (nullptr))))->Cat(newArrayBin_<char16_t>(1,"\x0A\x00"))));
}
// _finVars
static void k_i(){
(k_al) = (nullptr);
...
return as_<k_o>(classTable_, (k_bv), 0);
}
// _toBin
static type_(Array_<uint8_t>) k_t(type_(k_o) k_bx /*me*/){
type_(Array_<uint8_t>) k_by;
(k_by) = (newArrays_<type_(Array_<uint8_t>)>((0LL)));
return k_by;
}
// _fromBin
static type_(k_o) k_u(type_(k_o) k_bz /*me*/, type_(Array_<uint8_t>) k_ca /*bin*/, int64_t* k_cb /*idx*/){
...
uint64_t k_co; // m
uint64_t k_cp; // r
(k_cn) = (static_cast<uint64_t>(((k_ck) - (k_cj)) + (1LL)));
(k_co) = ((0x0000000000000000ULL) - (((0x0000000000000000ULL) - (k_cn)) % (k_cn)));
(k_cp) = (0x0000000000000000ULL);
if((k_co) == (0x0000000000000000ULL)){
(k_cp) = ((reinterpret_cast<k_cs>(classTable_[(k_ci)->Y + 9]))((k_ci)));
}
else{
do{
(k_cp) = ((reinterpret_cast<k_cs>(classTable_[(k_ci)->Y + 9]))((k_ci)));
...
return (static_cast<int64_t>((k_cp) % (k_cn))) + (k_cj);
}
// rndBit64
static uint64_t k_y(type_(k_n) k_cv /*me*/){
uint32_t k_cw; // t
(k_cw) = ((reinterpret_cast<k_cx>(classTable_[(k_cv)->Y + 12]))((k_cv)));
return k_cy<uint64_t>((k_cz<uint64_t>((static_cast<uint64_t>(k_cw)), (nullptr), (32LL))), (nullptr), (static_cast<uint64_t>((reinterpret_cast<k_cx>(classTable_[(k_cv)->Y + 12]))((k_cv)))));
}
// rndFloat
static double k_z(type_(k_n) k_da /*me*/, double k_db /*min*/, double k_dc /*max*/){
return (((static_cast<double>((reinterpret_cast<k_cs>(classTable_[(k_da)->Y + 9]))((k_da)))) / (1.844674407370955e+19)) * ((k_dc) - (k_db))) + (k_db);
}
// setSeed
static void k_aa(type_(k_n) k_df /*me*/, uint32_t k_dg /*seed*/){
((k_df)->k_dh) = (0x075BCD15U);
((k_df)->k_di) = (0x159A55E5U);
((k_df)->k_dj) = ((0x1F123BB5U) * (k_dg));
((k_df)->k_dk) = ((0x05491333U) * (k_dl<uint32_t>(((k_df)->k_dj), (nullptr), (1LL))));
}
// xs128
static uint32_t k_ab(type_(k_n) k_dm /*me*/){
uint32_t k_dn; // t
(k_dn) = (k_ai<uint32_t>(((k_dm)->k_dh), (nullptr), (k_cz<uint32_t>(((k_dm)->k_dh), (nullptr), (11LL)))));
((k_dm)->k_dh) = ((k_dm)->k_di);
((k_dm)->k_di) = ((k_dm)->k_dj);
((k_dm)->k_dj) = ((k_dm)->k_dk);
((k_dm)->k_dk) = (k_ai<uint32_t>((k_ai<uint32_t>(((k_dm)->k_dk), (nullptr), (k_dl<uint32_t>(((k_dm)->k_dk), (nullptr), (19LL))))), (nullptr), (k_ai<uint32_t>((k_dn), (nullptr), (k_dl<uint32_t>((k_dn), (nullptr), (8LL)))))));
return (k_dm)->k_dk;
}
// _dtor
static void k_ac(k_n* k_do){
...
type_(k_n) k_dq;
type_(k_n) k_dr;
(k_dq) = (k_dr = new_(k_n)(), k_w(k_dr), k_dr);
((k_dq)->k_dh) = ((k_dp)->k_dh);
((k_dq)->k_di) = ((k_dp)->k_di);
((k_dq)->k_dj) = ((k_dp)->k_dj);
((k_dq)->k_dk) = ((k_dp)->k_dk);
return as_<k_n>(classTable_, (k_dq), 8);
}
// _toBin
static type_(Array_<uint8_t>) k_ae(type_(k_n) k_ds){
type_(Array_<uint8_t>) k_dt;
(k_dt) = (newArrays_<type_(Array_<uint8_t>)>((0LL)));
(k_dt) = ((k_dt)->Cat(toBin_<uint32_t>()(((k_ds)->k_dh))));
(k_dt) = ((k_dt)->Cat(toBin_<uint32_t>()(((k_ds)->k_di))));
(k_dt) = ((k_dt)->Cat(toBin_<uint32_t>()(((k_ds)->k_dj))));
(k_dt) = ((k_dt)->Cat(toBin_<uint32_t>()(((k_ds)->k_dk))));
return k_dt;
}
// _fromBin
static type_(k_n) k_af(type_(k_n) k_dy, type_(Array_<uint8_t>) k_dz, int64_t* k_ea){
type_(k_n) k_eb;
type_(k_n) k_ec;
(k_eb) = (k_ec = new_(k_n)(), k_w(k_ec), k_ec);
((k_eb)->k_dh) = (fromBin_<uint32_t>()((k_dz), (*k_ea)));
((k_eb)->k_di) = (fromBin_<uint32_t>()((k_dz), (*k_ea)));
((k_eb)->k_dj) = (fromBin_<uint32_t>()((k_dz), (*k_ea)));
((k_eb)->k_dk) = (fromBin_<uint32_t>()((k_dz), (*k_ea)));
return k_eb;
}
// makeRnd
static type_(k_n) k_ag(type_(k_n) k_ed /*me2*/, uint32_t k_ee /*seed*/){
(reinterpret_cast<k_ef>(classTable_[(k_ed)->Y + 11]))((k_ed), (k_ee));
return k_ed;
}
// _xor
template<typename T_>static T_ k_ai(T_ k_eg /*me_*/, type_(Array_<int64_t>) k_eh /*type*/, T_ k_ei /*n*/){
...
return (k_er) % (powI_((10LL), (5LL)));
}
// _getDict
template<typename T_,typename C_,typename R_>static R_ k_ax(T_ k_ev /*me_*/, type_(Array_<int64_t>) k_ew /*type*/, C_ k_ex /*key*/, bool* k_ey /*existed*/){
return dictSearch_(k_ev->B,k_ex,k_ey);
}
// _addDict
template<typename T_,typename K_,typename V_>static void k_bj(T_ k_ez /*me_*/, type_(Array_<int64_t>) k_fa /*type*/, K_ k_fb /*key*/, V_ k_fc /*item*/){
k_ez->Add(k_fb,k_fc);
...
#if defined(_WIN32)
fputws(reinterpret_cast<const wchar_t*>(k_fd->B),stdout);
#else
std::u16string s_=k_fd->B;
...
// inputStr
static type_(Array_<char16_t>) k_ek(){
type_(Array_<char16_t>) k_ft; // buf
int64_t k_fu; // ptr
char16_t k_fy; // c
(k_ft) = (newArrays_<type_(Array_<char16_t>)>((1024LL)));
(k_fu) = (0LL);
while(true){
(k_fy) = ((k_fz)());
if((k_fy) == (u'\uFFFF')){
...
if((k_fu) == ((k_ft)->Len())){
(k_ft) = ((k_ft)->Cat(newArrays_<type_(Array_<char16_t>)>(((k_ft)->Len()))));
}
((k_ft)->At(k_fu)) = (k_fy);
(k_fu) = ((k_fu) + (1LL));
}
return k_gm<type_(Array_<char16_t>)>((k_ft), (nullptr), (0LL), (k_fu));
}
// _toInt
static int64_t k_en(type_(Array_<char16_t>) k_gn /*me_*/, bool* k_go /*success*/){
std::u16string u_=k_gn->B;
...
while(true){
(k_gs) = ((k_gt)());
for(k_gx = (0LL), k_gy = (((k_al)->Len()) - (1LL)); k_gx <= k_gy; k_gx += (1LL)){
if(((k_gs) == ((k_al)->At(k_gx))) || (((k_gs) == (u'\u000D')) && (((k_al)->At(k_gx)) == (u'\u000A')))){
return u'\u0000';
}
}
if((k_gs) == (u'\u000D')){
