...
static void k_f();
static void k_g();
using k_be=type_(k_aq)(*)(type_(k_aq), int64_t, int64_t);
using k_bp=type_(k_bh)(*)(type_(k_bh), int64_t);
using k_ce=int64_t(*)(type_(k_bh), int64_t);
using k_cg=void(*)(type_(k_bh), int64_t, int64_t);
static void k_i();
static void k_j();
static void k_l(int64_t k_cj /*excpt*/);
static void k_p(type_(k_o) k_cm /*me*/);
static void k_q(k_o* k_cn);
static int64_t k_r(type_(k_o) k_co /*me*/, type_(k_o) k_cp /*t*/);
static type_(k_o) k_s(type_(k_o) k_cq /*me*/);
static type_(Array_<uint8_t>) k_t(type_(k_o) k_ct /*me*/);
static type_(k_o) k_u(type_(k_o) k_cv /*me*/, type_(Array_<uint8_t>) k_cw /*bin*/, int64_t* k_cx /*idx*/);
static type_(Array_<char16_t>) k_v(type_(k_o) k_da /*me*/);
static void k_w(type_(k_n) k_db /*me*/);
using k_dd=void(*)(type_(k_o));
static int64_t k_x(type_(k_n) k_de /*me*/, int64_t k_df /*min*/, int64_t k_dg /*max*/);
using k_do=uint64_t(*)(type_(k_n));
static uint64_t k_y(type_(k_n) k_dr /*me*/);
using k_dt=uint32_t(*)(type_(k_n));
static double k_z(type_(k_n) k_dw /*me*/, double k_dx /*min*/, double k_dy /*max*/);
static void k_aa(type_(k_n) k_eb /*me*/, uint32_t k_ec /*seed*/);
static uint32_t k_ab(type_(k_n) k_ei /*me*/);
static void k_ac(k_n* k_ek);
static type_(k_n) k_ad(type_(k_n) k_el);
static type_(Array_<uint8_t>) k_ae(type_(k_n) k_eo);
static type_(k_n) k_af(type_(k_n) k_eu, type_(Array_<uint8_t>) k_ev, int64_t* k_ew);
static type_(k_n) k_ag(type_(k_n) k_ez /*me2*/, uint32_t k_fa /*seed*/);
using k_fb=void(*)(type_(k_n), uint32_t);
template<typename T_>static T_ k_ai(T_ k_fc /*me_*/, type_(Array_<int64_t>) k_fd /*type*/, T_ k_fe /*n*/);
static int64_t k_aj();
static int64_t k_ak();
static int64_t k_an();
static type_(k_aq) k_ar(type_(k_aq) k_fm /*me*/, int64_t k_fn /*l*/, int64_t k_fo /*r*/);
static int64_t k_as(type_(k_aq) k_fp /*me*/, type_(k_o) k_fq /*t*/);
using k_fw=int64_t(*)(type_(k_o), type_(k_o));
static void k_at(k_aq* k_fx);
static type_(k_aq) k_au(type_(k_aq) k_fy);
static type_(Array_<uint8_t>) k_av(type_(k_aq) k_ga);
static type_(k_aq) k_aw(type_(k_aq) k_ge, type_(Array_<uint8_t>) k_gf, int64_t* k_gg);
template<typename T_>static void k_bf(T_ k_gi /*me_*/, type_(Array_<int64_t>) k_gj /*type*/);
static type_(k_bh) k_bi(type_(k_bh) k_gk /*me*/, int64_t k_gl /*n*/);
static void k_bj(type_(k_bh) k_go /*me*/, int64_t k_gp /*i*/, int64_t k_gq /*v*/);
static int64_t k_bk(type_(k_bh) k_gx /*me*/, int64_t k_gy /*i*/);
static void k_bl(k_bh* k_he);
static type_(k_bh) k_bm(type_(k_bh) k_hf);
static type_(Array_<uint8_t>) k_bn(type_(k_bh) k_hh);
static type_(k_bh) k_bo(type_(k_bh) k_hl, type_(Array_<uint8_t>) k_hm, int64_t* k_hn);
static void k_ch(type_(Array_<char16_t>) k_hp /*str*/);
template<typename T_>static type_(Array_<char16_t>) k_ci(T_ k_hq /*me_*/, type_(Array_<int64_t>) k_hr /*type*/);
template<typename T_>static T_ k_du(T_ k_hs /*me_*/, type_(Array_<int64_t>) k_ht /*type*/, T_ k_hu /*n*/);
template<typename T_>static T_ k_dv(T_ k_hv /*me_*/, type_(Array_<int64_t>) k_hw /*type*/, int64_t k_hx /*n*/);
template<typename T_>static T_ k_eh(T_ k_ia /*me_*/, type_(Array_<int64_t>) k_ib /*type*/, int64_t k_ic /*n*/);
static type_(Array_<char16_t>) k_fg();
static int64_t k_fj(type_(Array_<char16_t>) k_iz /*me_*/, bool* k_ja /*success*/);
template<typename T_>static T_ k_gw(T_ k_jb /*me_*/, type_(Array_<int64_t>) k_jc /*type*/, T_ k_jd /*n*/);
static char16_t k_il();
template<typename T_>static T_ k_iy(T_ k_js /*me_*/, type_(Array_<int64_t>) k_jt /*type*/, int64_t k_ju /*start*/, int64_t k_jv /*len*/);
static char16_t k_ji();
...
// Bit
class k_bh : public k_o{
public:
k_bh();
virtual ~k_bh();
int64_t k_gm; // n
type_(Array_<int64_t>) k_gn; // data
};
...
k_n::k_n(): k_o(), k_ed(), k_ee(), k_ef(), k_eg(){ Y = 8; }
k_n::~k_n(){
k_ac(this);
}
k_aq::k_aq(): k_o(), k_by(), k_ca(){ Y = 21; }
k_aq::~k_aq(){
k_at(this);
}
k_bh::k_bh(): k_o(), k_gm(), k_gn(){ Y = 30; }
k_bh::~k_bh(){
k_bl(this);
}
// k_main_
...
int64_t k_am; // n
int64_t k_ao; // m
type_(Array_<type_(k_aq)>) k_ap; // lr
int64_t k_ba;
int64_t k_bc; // l
int64_t k_bd; // r
type_(k_bh) k_bg; // bitL
type_(k_bh) k_bq; // bitR
int64_t k_br; // ans
int64_t k_bv;
int64_t k_bx; // l
int64_t k_bz; // r
...
(k_am) = ((k_an)());
(k_ao) = ((k_an)());
(k_ap) = (newArrays_<type_(Array_<type_(k_aq)>)>((k_ao)));
for(k_ba = (0LL), k_bb = ((k_ao) - (1LL)); k_ba <= k_bb; k_ba += (1LL)){
(k_bc) = (((k_an)()) - (1LL));
(k_bd) = (((k_an)()) - (1LL));
((k_ap)->At(k_ba)) = ((reinterpret_cast<k_be>(classTable_[(new_(k_aq)())->Y + 8]))((new_(k_aq)()), (k_bc), (k_bd)));
}
k_bf<type_(Array_<type_(k_aq)>)>((k_ap), (nullptr));
(k_bg) = ((reinterpret_cast<k_bp>(classTable_[(new_(k_bh)())->Y + 8]))((new_(k_bh)()), (k_am)));
(k_bq) = ((reinterpret_cast<k_bp>(classTable_[(new_(k_bh)())->Y + 8]))((new_(k_bh)()), (k_am)));
(k_br) = (0LL);
for(k_bv = (0LL), k_bw = ((k_ao) - (1LL)); k_bv <= k_bw; k_bv += (1LL)){
(k_bx) = (((k_ap)->At(k_bv))->k_by);
(k_bz) = (((k_ap)->At(k_bv))->k_ca);
if((k_bx) != (0LL)){
(k_br) = ((k_br) + ((reinterpret_cast<k_ce>(classTable_[(k_bg)->Y + 10]))((k_bg), ((k_bx) - (1LL)))));
(k_br) = ((k_br) - ((reinterpret_cast<k_ce>(classTable_[(k_bq)->Y + 10]))((k_bq), (k_bx))));
}
(reinterpret_cast<k_cg>(classTable_[(k_bg)->Y + 9]))((k_bg), (k_bx), (1LL));
(reinterpret_cast<k_cg>(classTable_[(k_bq)->Y + 9]))((k_bq), (k_bz), (1LL));
}
(k_ch)((((newArrayBin_<char16_t>(0,""))->Cat(k_ci<int64_t>((k_br), (nullptr))))->Cat(newArrayBin_<char16_t>(1,"\x0A\x00"))));
}
// _finVars
static void k_i(){
(k_al) = (nullptr);
...
type_(k_o) k_cr;
type_(k_o) k_cs;
(k_cr) = (k_cs = new_(k_o)(), k_p(k_cs), k_cs);
return as_<k_o>(classTable_, (k_cr), 0);
}
// _toBin
static type_(Array_<uint8_t>) k_t(type_(k_o) k_ct /*me*/){
type_(Array_<uint8_t>) k_cu;
(k_cu) = (newArrays_<type_(Array_<uint8_t>)>((0LL)));
return k_cu;
}
// _fromBin
static type_(k_o) k_u(type_(k_o) k_cv /*me*/, type_(Array_<uint8_t>) k_cw /*bin*/, int64_t* k_cx /*idx*/){
type_(k_o) k_cy;
type_(k_o) k_cz;
(k_cy) = (k_cz = new_(k_o)(), k_p(k_cz), k_cz);
return k_cy;
}
// toStr
static type_(Array_<char16_t>) k_v(type_(k_o) k_da /*me*/){
...
uint64_t k_dk; // m
uint64_t k_dl; // r
(k_dj) = (static_cast<uint64_t>(((k_dg) - (k_df)) + (1LL)));
(k_dk) = ((0x0000000000000000ULL) - (((0x0000000000000000ULL) - (k_dj)) % (k_dj)));
(k_dl) = (0x0000000000000000ULL);
if((k_dk) == (0x0000000000000000ULL)){
(k_dl) = ((reinterpret_cast<k_do>(classTable_[(k_de)->Y + 9]))((k_de)));
}
else{
do{
(k_dl) = ((reinterpret_cast<k_do>(classTable_[(k_de)->Y + 9]))((k_de)));
...
return (static_cast<int64_t>((k_dl) % (k_dj))) + (k_df);
}
// rndBit64
static uint64_t k_y(type_(k_n) k_dr /*me*/){
uint32_t k_ds; // t
(k_ds) = ((reinterpret_cast<k_dt>(classTable_[(k_dr)->Y + 12]))((k_dr)));
return k_du<uint64_t>((k_dv<uint64_t>((static_cast<uint64_t>(k_ds)), (nullptr), (32LL))), (nullptr), (static_cast<uint64_t>((reinterpret_cast<k_dt>(classTable_[(k_dr)->Y + 12]))((k_dr)))));
}
// rndFloat
static double k_z(type_(k_n) k_dw /*me*/, double k_dx /*min*/, double k_dy /*max*/){
return (((static_cast<double>((reinterpret_cast<k_do>(classTable_[(k_dw)->Y + 9]))((k_dw)))) / (1.844674407370955e+19)) * ((k_dy) - (k_dx))) + (k_dx);
}
// setSeed
static void k_aa(type_(k_n) k_eb /*me*/, uint32_t k_ec /*seed*/){
((k_eb)->k_ed) = (0x075BCD15U);
((k_eb)->k_ee) = (0x159A55E5U);
((k_eb)->k_ef) = ((0x1F123BB5U) * (k_ec));
((k_eb)->k_eg) = ((0x05491333U) * (k_eh<uint32_t>(((k_eb)->k_ef), (nullptr), (1LL))));
}
// xs128
static uint32_t k_ab(type_(k_n) k_ei /*me*/){
uint32_t k_ej; // t
(k_ej) = (k_ai<uint32_t>(((k_ei)->k_ed), (nullptr), (k_dv<uint32_t>(((k_ei)->k_ed), (nullptr), (11LL)))));
((k_ei)->k_ed) = ((k_ei)->k_ee);
((k_ei)->k_ee) = ((k_ei)->k_ef);
((k_ei)->k_ef) = ((k_ei)->k_eg);
((k_ei)->k_eg) = (k_ai<uint32_t>((k_ai<uint32_t>(((k_ei)->k_eg), (nullptr), (k_eh<uint32_t>(((k_ei)->k_eg), (nullptr), (19LL))))), (nullptr), (k_ai<uint32_t>((k_ej), (nullptr), (k_eh<uint32_t>((k_ej), (nullptr), (8LL)))))));
return (k_ei)->k_eg;
}
// _dtor
static void k_ac(k_n* k_ek){
...
type_(k_n) k_em;
type_(k_n) k_en;
(k_em) = (k_en = new_(k_n)(), k_w(k_en), k_en);
((k_em)->k_ed) = ((k_el)->k_ed);
((k_em)->k_ee) = ((k_el)->k_ee);
((k_em)->k_ef) = ((k_el)->k_ef);
((k_em)->k_eg) = ((k_el)->k_eg);
return as_<k_n>(classTable_, (k_em), 8);
}
// _toBin
static type_(Array_<uint8_t>) k_ae(type_(k_n) k_eo){
type_(Array_<uint8_t>) k_ep;
(k_ep) = (newArrays_<type_(Array_<uint8_t>)>((0LL)));
(k_ep) = ((k_ep)->Cat(toBin_<uint32_t>()(((k_eo)->k_ed))));
(k_ep) = ((k_ep)->Cat(toBin_<uint32_t>()(((k_eo)->k_ee))));
(k_ep) = ((k_ep)->Cat(toBin_<uint32_t>()(((k_eo)->k_ef))));
(k_ep) = ((k_ep)->Cat(toBin_<uint32_t>()(((k_eo)->k_eg))));
return k_ep;
}
// _fromBin
static type_(k_n) k_af(type_(k_n) k_eu, type_(Array_<uint8_t>) k_ev, int64_t* k_ew){
type_(k_n) k_ex;
type_(k_n) k_ey;
(k_ex) = (k_ey = new_(k_n)(), k_w(k_ey), k_ey);
((k_ex)->k_ed) = (fromBin_<uint32_t>()((k_ev), (*k_ew)));
((k_ex)->k_ee) = (fromBin_<uint32_t>()((k_ev), (*k_ew)));
((k_ex)->k_ef) = (fromBin_<uint32_t>()((k_ev), (*k_ew)));
((k_ex)->k_eg) = (fromBin_<uint32_t>()((k_ev), (*k_ew)));
return k_ex;
}
// makeRnd
static type_(k_n) k_ag(type_(k_n) k_ez /*me2*/, uint32_t k_fa /*seed*/){
(reinterpret_cast<k_fb>(classTable_[(k_ez)->Y + 11]))((k_ez), (k_fa));
return k_ez;
}
// _xor
template<typename T_>static T_ k_ai(T_ k_fc /*me_*/, type_(Array_<int64_t>) k_fd /*type*/, T_ k_fe /*n*/){
...
k_fw k_fr; // super
(k_fr) = (k_r);
if(((k_fp)->k_ca) < ((as_<k_aq>(classTable_, (k_fq), 21))->k_ca)){
return -1LL;
}
else if(((k_fp)->k_ca) > ((as_<k_aq>(classTable_, (k_fq), 21))->k_ca)){
return 1LL;
}
else if(((k_fp)->k_by) > ((as_<k_aq>(classTable_, (k_fq), 21))->k_by)){
return -1LL;
}
return 0LL;
}
...
((k_fz)->k_by) = ((k_fy)->k_by);
((k_fz)->k_ca) = ((k_fy)->k_ca);
return as_<k_aq>(classTable_, (k_fz), 21);
}
// _toBin
static type_(Array_<uint8_t>) k_av(type_(k_aq) k_ga){
type_(Array_<uint8_t>) k_gb;
(k_gb) = (newArrays_<type_(Array_<uint8_t>)>((0LL)));
(k_gb) = ((k_gb)->Cat(toBin_<int64_t>()(((k_ga)->k_by))));
(k_gb) = ((k_gb)->Cat(toBin_<int64_t>()(((k_ga)->k_ca))));
return k_gb;
}
// _fromBin
static type_(k_aq) k_aw(type_(k_aq) k_ge, type_(Array_<uint8_t>) k_gf, int64_t* k_gg){
type_(k_aq) k_gh;
(k_gh) = (new_(k_aq)());
((k_gh)->k_by) = (fromBin_<int64_t>()((k_gf), (*k_gg)));
((k_gh)->k_ca) = (fromBin_<int64_t>()((k_gf), (*k_gg)));
return k_gh;
}
// _sort
template<typename T_>static void k_bf(T_ k_gi /*me_*/, type_(Array_<int64_t>) k_gj /*type*/){
...
static type_(k_bh) k_bi(type_(k_bh) k_gk /*me*/, int64_t k_gl /*n*/){
((k_gk)->k_gm) = (k_gl);
((k_gk)->k_gn) = (newArrays_<type_(Array_<int64_t>)>(((k_gl) + (1LL))));
return k_gk;
}
// add
static void k_bj(type_(k_bh) k_go /*me*/, int64_t k_gp /*i*/, int64_t k_gq /*v*/){
int64_t k_gr; // x
type_(Array_<int64_t>) k_gu;
(k_gr) = ((k_gp) + (1LL));
while((k_gr) <= ((k_go)->k_gm)){
(k_gu) = ((k_go)->k_gn);
((k_gu)->At(k_gr)) = (((k_gu)->At(k_gr)) + (k_gq));
(k_gr) = ((k_gr) + (static_cast<int64_t>(k_gw<uint64_t>((static_cast<uint64_t>(k_gr)), (nullptr), (static_cast<uint64_t>(-(k_gr)))))));
}
}
// sum
static int64_t k_bk(type_(k_bh) k_gx /*me*/, int64_t k_gy /*i*/){
...
(k_ha) = (0LL);
while((k_gz) > (0LL)){
(k_ha) = ((k_ha) + (((k_gx)->k_gn)->At(k_gz)));
(k_gz) = ((k_gz) - (static_cast<int64_t>(k_gw<uint64_t>((static_cast<uint64_t>(k_gz)), (nullptr), (static_cast<uint64_t>(-(k_gz)))))));
}
return k_ha;
}
// _dtor
...
(k_hg) = (new_(k_bh)());
((k_hg)->k_gm) = ((k_hf)->k_gm);
((k_hg)->k_gn) = (copy_<type_(Array_<int64_t>)>()((k_hf)->k_gn));
return as_<k_bh>(classTable_, (k_hg), 30);
}
// _toBin
static type_(Array_<uint8_t>) k_bn(type_(k_bh) k_hh){
type_(Array_<uint8_t>) k_hi;
(k_hi) = (newArrays_<type_(Array_<uint8_t>)>((0LL)));
(k_hi) = ((k_hi)->Cat(toBin_<int64_t>()(((k_hh)->k_gm))));
(k_hi) = ((k_hi)->Cat(toBin_<type_(Array_<int64_t>)>()(((k_hh)->k_gn))));
return k_hi;
}
// _fromBin
static type_(k_bh) k_bo(type_(k_bh) k_hl, type_(Array_<uint8_t>) k_hm, int64_t* k_hn){
type_(k_bh) k_ho;
(k_ho) = (new_(k_bh)());
((k_ho)->k_gm) = (fromBin_<int64_t>()((k_hm), (*k_hn)));
((k_ho)->k_gn) = (fromBin_<type_(Array_<int64_t>)>()((k_hm), (*k_hn)));
return k_ho;
}
// print
static void k_ch(type_(Array_<char16_t>) k_hp /*str*/){
...
#if defined(_WIN32)
fputws(reinterpret_cast<const wchar_t*>(k_hp->B),stdout);
#else
std::u16string s_=k_hp->B;
...
// inputStr
static type_(Array_<char16_t>) k_fg(){
type_(Array_<char16_t>) k_if; // buf
int64_t k_ig; // ptr
char16_t k_ik; // c
(k_if) = (newArrays_<type_(Array_<char16_t>)>((1024LL)));
(k_ig) = (0LL);
while(true){
(k_ik) = ((k_il)());
if((k_ik) == (u'\uFFFF')){
...
if((k_ig) == ((k_if)->Len())){
(k_if) = ((k_if)->Cat(newArrays_<type_(Array_<char16_t>)>(((k_if)->Len()))));
}
((k_if)->At(k_ig)) = (k_ik);
(k_ig) = ((k_ig) + (1LL));
}
return k_iy<type_(Array_<char16_t>)>((k_if), (nullptr), (0LL), (k_ig));
}
// _toInt
static int64_t k_fj(type_(Array_<char16_t>) k_iz /*me_*/, bool* k_ja /*success*/){
std::u16string u_=k_iz->B;
...
while(true){
(k_jh) = ((k_ji)());
for(k_jm = (0LL), k_jn = (((k_al)->Len()) - (1LL)); k_jm <= k_jn; k_jm += (1LL)){
if(((k_jh) == ((k_al)->At(k_jm))) || (((k_jh) == (u'\u000D')) && (((k_al)->At(k_jm)) == (u'\u000A')))){
return u'\u0000';
}
}
if((k_jh) == (u'\u000D')){
...
classTable_[24] = reinterpret_cast<int64_t>(k_as);
...
classTable_[30] = 0;
classTable_[31] = reinterpret_cast<int64_t>(k_p);
classTable_[32] = reinterpret_cast<int64_t>(k_bl);
classTable_[33] = reinterpret_cast<int64_t>(k_r);
classTable_[34] = reinterpret_cast<int64_t>(k_bm);
classTable_[35] = reinterpret_cast<int64_t>(k_bn);
classTable_[36] = reinterpret_cast<int64_t>(k_bo);
classTable_[37] = reinterpret_cast<int64_t>(k_v);
classTable_[38] = reinterpret_cast<int64_t>(k_bi);
classTable_[39] = reinterpret_cast<int64_t>(k_bj);
classTable_[40] = reinterpret_cast<int64_t>(k_bk);
init_();
k_main_();
