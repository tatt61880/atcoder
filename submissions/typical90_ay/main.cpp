...
static void k_i();
static void k_j();
static void k_l(int64_t k_ef /*excpt*/);
static void k_p(type_(k_o) k_ei /*me*/);
static void k_q(k_o* k_ej);
static int64_t k_r(type_(k_o) k_ek /*me*/, type_(k_o) k_el /*t*/);
static type_(k_o) k_s(type_(k_o) k_em /*me*/);
static type_(Array_<uint8_t>) k_t(type_(k_o) k_ep /*me*/);
static type_(k_o) k_u(type_(k_o) k_er /*me*/, type_(Array_<uint8_t>) k_es /*bin*/, int64_t* k_et /*idx*/);
static type_(Array_<char16_t>) k_v(type_(k_o) k_ew /*me*/);
static void k_w(type_(k_n) k_ex /*me*/);
using k_ez=void(*)(type_(k_o));
static int64_t k_x(type_(k_n) k_fa /*me*/, int64_t k_fb /*min*/, int64_t k_fc /*max*/);
using k_fk=uint64_t(*)(type_(k_n));
static uint64_t k_y(type_(k_n) k_fn /*me*/);
using k_fp=uint32_t(*)(type_(k_n));
static double k_z(type_(k_n) k_fs /*me*/, double k_ft /*min*/, double k_fu /*max*/);
static void k_aa(type_(k_n) k_fx /*me*/, uint32_t k_fy /*seed*/);
static uint32_t k_ab(type_(k_n) k_ge /*me*/);
static void k_ac(k_n* k_gg);
static type_(k_n) k_ad(type_(k_n) k_gh);
static type_(Array_<uint8_t>) k_ae(type_(k_n) k_gk);
static type_(k_n) k_af(type_(k_n) k_gq, type_(Array_<uint8_t>) k_gr, int64_t* k_gs);
static type_(k_n) k_ag(type_(k_n) k_gv /*me2*/, uint32_t k_gw /*seed*/);
using k_gx=void(*)(type_(k_n), uint32_t);
template<typename T_>static T_ k_ai(T_ k_gy /*me_*/, type_(Array_<int64_t>) k_gz /*type*/, T_ k_ha /*n*/);
static int64_t k_aj();
static int64_t k_ak();
static int64_t k_an();
template<typename T_,typename C_>static void k_cd(T_ k_hi /*me_*/, type_(Array_<int64_t>) k_hj /*type*/, C_ k_hk /*item*/);
template<typename T_>static void k_ck(T_ k_hl /*me_*/, type_(Array_<int64_t>) k_hm /*type*/);
template<typename T_,typename R_>static R_ k_cq(T_ k_hn /*me_*/, type_(Array_<int64_t>) k_ho /*type*/);
template<typename T_>static void k_cr(T_ k_hr /*me_*/, type_(Array_<int64_t>) k_hs /*type*/);
template<typename T_>static void k_cs(T_ k_ht /*me_*/, type_(Array_<int64_t>) k_hu /*type*/);
static int64_t k_dx(int64_t k_hv /*me_*/);
static void k_ed(type_(Array_<char16_t>) k_hw /*str*/);
template<typename T_>static type_(Array_<char16_t>) k_ee(T_ k_hx /*me_*/, type_(Array_<int64_t>) k_hy /*type*/);
template<typename T_>static T_ k_fq(T_ k_hz /*me_*/, type_(Array_<int64_t>) k_ia /*type*/, T_ k_ib /*n*/);
template<typename T_>static T_ k_fr(T_ k_ic /*me_*/, type_(Array_<int64_t>) k_id /*type*/, int64_t k_ie /*n*/);
template<typename T_>static T_ k_gd(T_ k_ih /*me_*/, type_(Array_<int64_t>) k_ii /*type*/, int64_t k_ij /*n*/);
static type_(Array_<char16_t>) k_hc();
static int64_t k_hf(type_(Array_<char16_t>) k_jg /*me_*/, bool* k_jh /*success*/);
static char16_t k_is();
template<typename T_>static T_ k_jf(T_ k_jw /*me_*/, type_(Array_<int64_t>) k_jx /*type*/, int64_t k_jy /*start*/, int64_t k_jz /*len*/);
static char16_t k_jm();
...
k_n::k_n(): k_o(), k_fz(), k_ga(), k_gb(), k_gc(){ Y = 8; }
k_n::~k_n(){
k_ac(this);
}
// k_main_
...
int64_t k_au;
int64_t k_aw; // m
type_(Array_<type_(List_<int64_t>)>) k_ax; // x
int64_t k_bb;
int64_t k_bg;
int64_t k_bi; // _
type_(Array_<bool>) k_bj; // f
int64_t k_bn;
int64_t k_bq; // sum
int64_t k_br; // cnt
int64_t k_bv;
type_(Array_<type_(Array_<int64_t>)>) k_ce; // y
int64_t k_ci;
int64_t k_co;
int64_t k_ct; // ans
int64_t k_cx;
int64_t k_cz; // _
type_(Array_<bool>) k_da; // f
int64_t k_de;
int64_t k_dh; // sum
int64_t k_di; // cnt
int64_t k_dm;
...
(k_aw) = ((k_am) / (2LL));
(k_ax) = (newArrays_<type_(Array_<type_(List_<int64_t>)>)>(((k_ao) + (1LL))));
for(k_bb = (0LL), k_bc = (k_ao); k_bb <= k_bc; k_bb += (1LL)){
((k_ax)->At(k_bb)) = (new_(List_<int64_t>)());
}
for(k_bg = (0LL), k_bh = ((powI_((2LL), (k_aw))) - (1LL)); k_bg <= k_bh; k_bg += (1LL)){
(k_bi) = (k_bg);
(k_bj) = (newArrays_<type_(Array_<bool>)>((k_aw)));
for(k_bn = (0LL), k_bo = ((k_aw) - (1LL)); k_bn <= k_bo; k_bn += (1LL)){
((k_bj)->At(k_bn)) = (((k_bi) % (2LL)) == (1LL));
(k_bi) = ((k_bi) / (2LL));
}
(k_bq) = (0LL);
(k_br) = (0LL);
for(k_bv = (0LL), k_bw = ((k_aw) - (1LL)); k_bv <= k_bw; k_bv += (1LL)){
if((k_bj)->At(k_bv)){
(k_br) = ((k_br) + (1LL));
(k_bq) = ((k_bq) + ((k_aq)->At(k_bv)));
}
}
if((k_br) <= (k_ao)){
k_cd<type_(List_<int64_t>), int64_t>(((k_ax)->At(k_br)), (nullptr), (k_bq));
}
}
(k_ce) = (newArrays_<type_(Array_<type_(Array_<int64_t>)>)>(((k_ao) + (1LL))));
for(k_ci = (0LL), k_cj = (k_ao); k_ci <= k_cj; k_ci += (1LL)){
((k_ce)->At(k_ci)) = (newArrays_<type_(Array_<int64_t>)>((((k_ax)->At(k_ci))->Len())));
k_ck<type_(List_<int64_t>)>(((k_ax)->At(k_ci)), (nullptr));
for(k_co = (0LL), k_cp = ((((k_ax)->At(k_ci))->Len()) - (1LL)); k_co <= k_cp; k_co += (1LL)){
(((k_ce)->At(k_ci))->At(k_co)) = (k_cq<type_(List_<int64_t>), int64_t>(((k_ax)->At(k_ci)), (nullptr)));
k_cr<type_(List_<int64_t>)>(((k_ax)->At(k_ci)), (nullptr));
}
k_cs<type_(Array_<int64_t>)>(((k_ce)->At(k_ci)), (nullptr));
}
(k_ct) = (0LL);
for(k_cx = (0LL), k_cy = ((powI_((2LL), ((k_am) - (k_aw)))) - (1LL)); k_cx <= k_cy; k_cx += (1LL)){
(k_cz) = (k_cx);
(k_da) = (newArrays_<type_(Array_<bool>)>(((k_am) - (k_aw))));
for(k_de = (0LL), k_df = (((k_am) - (k_aw)) - (1LL)); k_de <= k_df; k_de += (1LL)){
((k_da)->At(k_de)) = (((k_cz) % (2LL)) == (1LL));
(k_cz) = ((k_cz) / (2LL));
}
(k_dh) = (0LL);
(k_di) = (0LL);
for(k_dm = (0LL), k_dn = (((k_am) - (k_aw)) - (1LL)); k_dm <= k_dn; k_dm += (1LL)){
if((k_da)->At(k_dm)){
(k_di) = ((k_di) + (1LL));
(k_dh) = ((k_dh) + ((k_aq)->At((k_aw) + (k_dm))));
}
}
if((k_di) <= (k_ao)){
(k_du) = (-1LL);
(k_dv) = (((k_ce)->At((k_ao) - (k_di)))->Len());
while(((k_dx)(((k_du) - (k_dv)))) > (1LL)){
(k_dy) = (((k_du) + (k_dv)) / (2LL));
if(((k_dh) + (((k_ce)->At((k_ao) - (k_di)))->At(k_dy))) <= (k_ap)){
(k_du) = (k_dy);
}
else{
(k_dv) = (k_dy);
...
(k_ed)((((newArrayBin_<char16_t>(0,""))->Cat(k_ee<int64_t>((k_ct), (nullptr))))->Cat(newArrayBin_<char16_t>(1,"\x0A\x00"))));
}
// _finVars
static void k_i(){
(k_al) = (nullptr);
...
type_(k_o) k_en;
type_(k_o) k_eo;
(k_en) = (k_eo = new_(k_o)(), k_p(k_eo), k_eo);
return as_<k_o>(classTable_, (k_en), 0);
}
// _toBin
static type_(Array_<uint8_t>) k_t(type_(k_o) k_ep /*me*/){
...
type_(k_o) k_eu;
type_(k_o) k_ev;
(k_eu) = (k_ev = new_(k_o)(), k_p(k_ev), k_ev);
return k_eu;
}
// toStr
static type_(Array_<char16_t>) k_v(type_(k_o) k_ew /*me*/){
...
uint64_t k_fg; // m
uint64_t k_fh; // r
(k_ff) = (static_cast<uint64_t>(((k_fc) - (k_fb)) + (1LL)));
(k_fg) = ((0x0000000000000000ULL) - (((0x0000000000000000ULL) - (k_ff)) % (k_ff)));
(k_fh) = (0x0000000000000000ULL);
if((k_fg) == (0x0000000000000000ULL)){
(k_fh) = ((reinterpret_cast<k_fk>(classTable_[(k_fa)->Y + 9]))((k_fa)));
}
else{
do{
(k_fh) = ((reinterpret_cast<k_fk>(classTable_[(k_fa)->Y + 9]))((k_fa)));
...
return (static_cast<int64_t>((k_fh) % (k_ff))) + (k_fb);
}
// rndBit64
static uint64_t k_y(type_(k_n) k_fn /*me*/){
uint32_t k_fo; // t
(k_fo) = ((reinterpret_cast<k_fp>(classTable_[(k_fn)->Y + 12]))((k_fn)));
return k_fq<uint64_t>((k_fr<uint64_t>((static_cast<uint64_t>(k_fo)), (nullptr), (32LL))), (nullptr), (static_cast<uint64_t>((reinterpret_cast<k_fp>(classTable_[(k_fn)->Y + 12]))((k_fn)))));
}
// rndFloat
static double k_z(type_(k_n) k_fs /*me*/, double k_ft /*min*/, double k_fu /*max*/){
return (((static_cast<double>((reinterpret_cast<k_fk>(classTable_[(k_fs)->Y + 9]))((k_fs)))) / (1.844674407370955e+19)) * ((k_fu) - (k_ft))) + (k_ft);
}
// setSeed
static void k_aa(type_(k_n) k_fx /*me*/, uint32_t k_fy /*seed*/){
((k_fx)->k_fz) = (0x075BCD15U);
((k_fx)->k_ga) = (0x159A55E5U);
((k_fx)->k_gb) = ((0x1F123BB5U) * (k_fy));
((k_fx)->k_gc) = ((0x05491333U) * (k_gd<uint32_t>(((k_fx)->k_gb), (nullptr), (1LL))));
}
// xs128
static uint32_t k_ab(type_(k_n) k_ge /*me*/){
uint32_t k_gf; // t
(k_gf) = (k_ai<uint32_t>(((k_ge)->k_fz), (nullptr), (k_fr<uint32_t>(((k_ge)->k_fz), (nullptr), (11LL)))));
((k_ge)->k_fz) = ((k_ge)->k_ga);
((k_ge)->k_ga) = ((k_ge)->k_gb);
((k_ge)->k_gb) = ((k_ge)->k_gc);
((k_ge)->k_gc) = (k_ai<uint32_t>((k_ai<uint32_t>(((k_ge)->k_gc), (nullptr), (k_gd<uint32_t>(((k_ge)->k_gc), (nullptr), (19LL))))), (nullptr), (k_ai<uint32_t>((k_gf), (nullptr), (k_gd<uint32_t>((k_gf), (nullptr), (8LL)))))));
return (k_ge)->k_gc;
}
// _dtor
static void k_ac(k_n* k_gg){
...
type_(k_n) k_gi;
type_(k_n) k_gj;
(k_gi) = (k_gj = new_(k_n)(), k_w(k_gj), k_gj);
((k_gi)->k_fz) = ((k_gh)->k_fz);
((k_gi)->k_ga) = ((k_gh)->k_ga);
((k_gi)->k_gb) = ((k_gh)->k_gb);
((k_gi)->k_gc) = ((k_gh)->k_gc);
return as_<k_n>(classTable_, (k_gi), 8);
}
// _toBin
static type_(Array_<uint8_t>) k_ae(type_(k_n) k_gk){
type_(Array_<uint8_t>) k_gl;
(k_gl) = (newArrays_<type_(Array_<uint8_t>)>((0LL)));
(k_gl) = ((k_gl)->Cat(toBin_<uint32_t>()(((k_gk)->k_fz))));
(k_gl) = ((k_gl)->Cat(toBin_<uint32_t>()(((k_gk)->k_ga))));
(k_gl) = ((k_gl)->Cat(toBin_<uint32_t>()(((k_gk)->k_gb))));
(k_gl) = ((k_gl)->Cat(toBin_<uint32_t>()(((k_gk)->k_gc))));
return k_gl;
}
// _fromBin
static type_(k_n) k_af(type_(k_n) k_gq, type_(Array_<uint8_t>) k_gr, int64_t* k_gs){
type_(k_n) k_gt;
type_(k_n) k_gu;
(k_gt) = (k_gu = new_(k_n)(), k_w(k_gu), k_gu);
((k_gt)->k_fz) = (fromBin_<uint32_t>()((k_gr), (*k_gs)));
((k_gt)->k_ga) = (fromBin_<uint32_t>()((k_gr), (*k_gs)));
((k_gt)->k_gb) = (fromBin_<uint32_t>()((k_gr), (*k_gs)));
((k_gt)->k_gc) = (fromBin_<uint32_t>()((k_gr), (*k_gs)));
return k_gt;
}
// makeRnd
static type_(k_n) k_ag(type_(k_n) k_gv /*me2*/, uint32_t k_gw /*seed*/){
(reinterpret_cast<k_gx>(classTable_[(k_gv)->Y + 11]))((k_gv), (k_gw));
return k_gv;
}
// _xor
template<typename T_>static T_ k_ai(T_ k_gy /*me_*/, type_(Array_<int64_t>) k_gz /*type*/, T_ k_ha /*n*/){
...
#if defined(_WIN32)
fputws(reinterpret_cast<const wchar_t*>(k_hw->B),stdout);
#else
std::u16string s_=k_hw->B;
...
// inputStr
static type_(Array_<char16_t>) k_hc(){
type_(Array_<char16_t>) k_im; // buf
int64_t k_in; // ptr
char16_t k_ir; // c
(k_im) = (newArrays_<type_(Array_<char16_t>)>((1024LL)));
(k_in) = (0LL);
while(true){
(k_ir) = ((k_is)());
if((k_ir) == (u'\uFFFF')){
...
if((k_in) == ((k_im)->Len())){
(k_im) = ((k_im)->Cat(newArrays_<type_(Array_<char16_t>)>(((k_im)->Len()))));
}
((k_im)->At(k_in)) = (k_ir);
(k_in) = ((k_in) + (1LL));
}
return k_jf<type_(Array_<char16_t>)>((k_im), (nullptr), (0LL), (k_in));
}
// _toInt
static int64_t k_hf(type_(Array_<char16_t>) k_jg /*me_*/, bool* k_jh /*success*/){
std::u16string u_=k_jg->B;
...
while(true){
(k_jl) = ((k_jm)());
for(k_jq = (0LL), k_jr = (((k_al)->Len()) - (1LL)); k_jq <= k_jr; k_jq += (1LL)){
if(((k_jl) == ((k_al)->At(k_jq))) || (((k_jl) == (u'\u000D')) && (((k_al)->At(k_jq)) == (u'\u000A')))){
return u'\u0000';
}
}
if((k_jl) == (u'\u000D')){
