...
func main()
	...
	var n: int :: ^s
	var targets: [][]char :: ["eraser", "erase", "dreamer", "dream"]
	for id(0, 3)
		var target: []char :: targets[id]
		for i(0, n - ^target)
			for j(0, ^target - 1)
				if(s[i + j] <> target[j])
					skip i
...
static void k_bv(type_(Array_<char16_t>) k_fm /*str*/);
template<typename T_>static T_ k_dh(T_ k_fn /*me_*/, type_(Array_<int64_t>) k_fo /*type*/, T_ k_fp /*n*/);
template<typename T_>static T_ k_di(T_ k_fq /*me_*/, type_(Array_<int64_t>) k_fr /*type*/, int64_t k_fs /*n*/);
template<typename T_>static T_ k_du(T_ k_fv /*me_*/, type_(Array_<int64_t>) k_fw /*type*/, int64_t k_fx /*n*/);
static char16_t k_ey();
template<typename T_>static T_ k_fl(T_ k_ga /*me_*/, type_(Array_<int64_t>) k_gb /*type*/, int64_t k_gc /*start*/, int64_t k_gd /*len*/);
...
int64_t k_ao; // n
type_(Array_<type_(Array_<char16_t>)>) k_ap; // targets
int64_t k_at;
type_(Array_<char16_t>) k_av; // target
int64_t k_az;
int64_t k_be;
int64_t k_bl;
bool k_bn; // ans
...
(k_am) = ((k_an)());
(k_ao) = ((k_am)->Len());
(k_ap) = (new_(Array_<type_(Array_<char16_t>)>)((newArrayBin_<char16_t>(6,"\x65\x00\x72\x00\x61\x00\x73\x00\x65\x00\x72\x00")), (newArrayBin_<char16_t>(5,"\x65\x00\x72\x00\x61\x00\x73\x00\x65\x00")), (newArrayBin_<char16_t>(7,"\x64\x00\x72\x00\x65\x00\x61\x00\x6D\x00\x65\x00\x72\x00")), (newArrayBin_<char16_t>(5,"\x64\x00\x72\x00\x65\x00\x61\x00\x6D\x00"))));
for(k_at = (0LL), k_au = (3LL); k_at <= k_au; k_at += (1LL)){
(k_av) = ((k_ap)->At(k_at));
for(k_az = (0LL), k_ba = ((k_ao) - ((k_av)->Len())); k_az <= k_ba; k_az += (1LL)){
for(k_be = (0LL), k_bf = (((k_av)->Len()) - (1LL)); k_be <= k_bf; k_be += (1LL)){
if(((k_am)->At((k_az) + (k_be))) != ((k_av)->At(k_be))){
goto k_ay;
}
}
for(k_bl = (0LL), k_bm = (((k_av)->Len()) - (1LL)); k_bl <= k_bm; k_bl += (1LL)){
((k_am)->At((k_az) + (k_bl))) = (u'\u0020');
}
k_ay:;
}
...
(k_bn) = (true);
for(k_br = (0LL), k_bs = ((k_ao) - (1LL)); k_br <= k_bs; k_br += (1LL)){
if(((k_am)->At(k_br)) != (u'\u0020')){
(k_bn) = (false);
}
}
(k_bv)((((k_bn) ? (newArrayBin_<char16_t>(3,"\x59\x00\x45\x00\x53\x00")) : (newArrayBin_<char16_t>(2,"\x4E\x00\x4F\x00")))->Cat(newArrayBin_<char16_t>(1,"\x0A\x00"))));
}
// _finVars
static void k_i(){
(k_al) = (nullptr);
...
// input
static type_(Array_<char16_t>) k_an(){
type_(Array_<char16_t>) k_es; // buf
int64_t k_et; // ptr
char16_t k_ex; // c
(k_es) = (newArrays_<type_(Array_<char16_t>)>((1024LL)));
(k_et) = (0LL);
while(true){
(k_ex) = ((k_ey)());
if((k_ex) == (u'\u000D')){
...
if((k_et) == ((k_es)->Len())){
(k_es) = ((k_es)->Cat(newArrays_<type_(Array_<char16_t>)>(((k_es)->Len()))));
}
((k_es)->At(k_et)) = (k_ex);
(k_et) = ((k_et) + (1LL));
}
return k_fl<type_(Array_<char16_t>)>((k_es), (nullptr), (0LL), (k_et));
}
// print
static void k_bv(type_(Array_<char16_t>) k_fm /*str*/){
if(k_fm==nullptr){
