...
static int64_t k_ak();
static int64_t k_an();
static int64_t k_as(int64_t k_eb /*me_*/);
static void k_av(type_(Array_<char16_t>) k_ec /*str*/);
template<typename T_>static T_ k_cj(T_ k_ed /*me_*/, type_(Array_<int64_t>) k_ee /*type*/, T_ k_ef /*n*/);
template<typename T_>static T_ k_ck(T_ k_eg /*me_*/, type_(Array_<int64_t>) k_eh /*type*/, int64_t k_ei /*n*/);
template<typename T_>static T_ k_cw(T_ k_el /*me_*/, type_(Array_<int64_t>) k_em /*type*/, int64_t k_en /*n*/);
static type_(Array_<char16_t>) k_dv();
static int64_t k_dy(type_(Array_<char16_t>) k_fk /*me_*/, bool* k_fl /*success*/);
static char16_t k_ew();
template<typename T_>static T_ k_fj(T_ k_ga /*me_*/, type_(Array_<int64_t>) k_gb /*type*/, int64_t k_gc /*start*/, int64_t k_gd /*len*/);
...
if((k_am) == (k_ao)){
(k_av)((newArrayBin_<char16_t>(2,"\x3D\x00\x0A\x00")));
}
else if((k_am) < (k_ao)){
(k_av)((newArrayBin_<char16_t>(2,"\x3C\x00\x0A\x00")));
}
else{
(k_av)((newArrayBin_<char16_t>(2,"\x3E\x00\x0A\x00")));
}
}
// _finVars
static void k_i(){
...
#if defined(_WIN32)
fputws(reinterpret_cast<const wchar_t*>(k_ec->B),stdout);
#else
std::u16string s_=k_ec->B;
...
// inputStr
static type_(Array_<char16_t>) k_dv(){
type_(Array_<char16_t>) k_eq; // buf
int64_t k_er; // ptr
char16_t k_ev; // c
(k_eq) = (newArrays_<type_(Array_<char16_t>)>((1024LL)));
(k_er) = (0LL);
while(true){
(k_ev) = ((k_ew)());
if((k_ev) == (u'\uFFFF')){
...
if((k_er) == ((k_eq)->Len())){
(k_eq) = ((k_eq)->Cat(newArrays_<type_(Array_<char16_t>)>(((k_eq)->Len()))));
}
((k_eq)->At(k_er)) = (k_ev);
(k_er) = ((k_er) + (1LL));
}
return k_fj<type_(Array_<char16_t>)>((k_eq), (nullptr), (0LL), (k_er));
}
// _toInt
static int64_t k_dy(type_(Array_<char16_t>) k_fk /*me_*/, bool* k_fl /*success*/){
