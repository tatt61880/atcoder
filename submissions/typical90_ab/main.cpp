...
static void k_i();
static void k_j();
static void k_l(int64_t k_cw /*excpt*/);
static void k_p(type_(k_o) k_cz /*me*/);
static void k_q(k_o* k_da);
static int64_t k_r(type_(k_o) k_db /*me*/, type_(k_o) k_dc /*t*/);
static type_(k_o) k_s(type_(k_o) k_dd /*me*/);
static type_(Array_<uint8_t>) k_t(type_(k_o) k_dg /*me*/);
static type_(k_o) k_u(type_(k_o) k_di /*me*/, type_(Array_<uint8_t>) k_dj /*bin*/, int64_t* k_dk /*idx*/);
static type_(Array_<char16_t>) k_v(type_(k_o) k_dn /*me*/);
static void k_w(type_(k_n) k_do /*me*/);
using k_dq=void(*)(type_(k_o));
static int64_t k_x(type_(k_n) k_dr /*me*/, int64_t k_ds /*min*/, int64_t k_dt /*max*/);
using k_eb=uint64_t(*)(type_(k_n));
static uint64_t k_y(type_(k_n) k_ee /*me*/);
using k_eg=uint32_t(*)(type_(k_n));
static double k_z(type_(k_n) k_ej /*me*/, double k_ek /*min*/, double k_el /*max*/);
static void k_aa(type_(k_n) k_eo /*me*/, uint32_t k_ep /*seed*/);
static uint32_t k_ab(type_(k_n) k_ev /*me*/);
static void k_ac(k_n* k_ex);
static type_(k_n) k_ad(type_(k_n) k_ey);
static type_(Array_<uint8_t>) k_ae(type_(k_n) k_fb);
static type_(k_n) k_af(type_(k_n) k_fh, type_(Array_<uint8_t>) k_fi, int64_t* k_fj);
static type_(k_n) k_ag(type_(k_n) k_fm /*me2*/, uint32_t k_fn /*seed*/);
using k_fo=void(*)(type_(k_n), uint32_t);
template<typename T_>static T_ k_ai(T_ k_fp /*me_*/, type_(Array_<int64_t>) k_fq /*type*/, T_ k_fr /*n*/);
static int64_t k_aj();
static int64_t k_ak();
static int64_t k_an();
static void k_cu(type_(Array_<char16_t>) k_fz /*str*/);
template<typename T_>static type_(Array_<char16_t>) k_cv(T_ k_ga /*me_*/, type_(Array_<int64_t>) k_gb /*type*/);
template<typename T_>static T_ k_eh(T_ k_gc /*me_*/, type_(Array_<int64_t>) k_gd /*type*/, T_ k_ge /*n*/);
template<typename T_>static T_ k_ei(T_ k_gf /*me_*/, type_(Array_<int64_t>) k_gg /*type*/, int64_t k_gh /*n*/);
template<typename T_>static T_ k_eu(T_ k_gk /*me_*/, type_(Array_<int64_t>) k_gl /*type*/, int64_t k_gm /*n*/);
static type_(Array_<char16_t>) k_ft();
static int64_t k_fw(type_(Array_<char16_t>) k_hj /*me_*/, bool* k_hk /*success*/);
static char16_t k_gv();
template<typename T_>static T_ k_hi(T_ k_hz /*me_*/, type_(Array_<int64_t>) k_ia /*type*/, int64_t k_ib /*start*/, int64_t k_ic /*len*/);
static char16_t k_hp();
...
k_n::k_n(): k_o(), k_eq(), k_er(), k_es(), k_et(){ Y = 8; }
k_n::~k_n(){
k_ac(this);
}
// k_main_
...
static void k_g(){
int64_t k_am; // n
type_(Array_<type_(Array_<int64_t>)>) k_ao; // cum2d
int64_t k_aq;
int64_t k_as; // lx
int64_t k_at; // ly
int64_t k_au; // rx
int64_t k_av; // ry
type_(Array_<int64_t>) k_ax;
type_(Array_<int64_t>) k_az;
type_(Array_<int64_t>) k_bb;
type_(Array_<int64_t>) k_bd;
int64_t k_bh;
int64_t k_bm;
type_(Array_<int64_t>) k_bp;
int64_t k_bt;
int64_t k_by;
type_(Array_<int64_t>) k_cb;
type_(Array_<int64_t>) k_cc; // ans
int64_t k_cg;
int64_t k_cl;
int64_t k_co;
int64_t k_cs;
...
int64_t k_ct;
(k_am) = ((k_an)());
(k_ao) = (newArrays_<type_(Array_<type_(Array_<int64_t>)>)>((1001LL), (1001LL)));
for(k_aq = (1LL), k_ar = (k_am); k_aq <= k_ar; k_aq += (1LL)){
(k_as) = ((k_an)());
(k_at) = ((k_an)());
(k_au) = ((k_an)());
(k_av) = ((k_an)());
(k_ax) = ((k_ao)->At(k_av));
((k_ax)->At(k_au)) = (((k_ax)->At(k_au)) + (1LL));
(k_az) = ((k_ao)->At(k_av));
((k_az)->At(k_as)) = (((k_az)->At(k_as)) - (1LL));
(k_bb) = ((k_ao)->At(k_at));
((k_bb)->At(k_au)) = (((k_bb)->At(k_au)) - (1LL));
(k_bd) = ((k_ao)->At(k_at));
((k_bd)->At(k_as)) = (((k_bd)->At(k_as)) + (1LL));
}
for(k_bh = (0LL), k_bi = (999LL); k_bh <= k_bi; k_bh += (1LL)){
for(k_bm = (1LL), k_bn = (999LL); k_bm <= k_bn; k_bm += (1LL)){
(k_bp) = ((k_ao)->At(k_bh));
((k_bp)->At(k_bm)) = (((k_bp)->At(k_bm)) + (((k_ao)->At(k_bh))->At((k_bm) - (1LL))));
}
}
for(k_bt = (0LL), k_bu = (999LL); k_bt <= k_bu; k_bt += (1LL)){
for(k_by = (1LL), k_bz = (999LL); k_by <= k_bz; k_by += (1LL)){
(k_cb) = ((k_ao)->At(k_by));
((k_cb)->At(k_bt)) = (((k_cb)->At(k_bt)) + (((k_ao)->At((k_by) - (1LL)))->At(k_bt)));
}
}
(k_cc) = (newArrays_<type_(Array_<int64_t>)>(((k_am) + (1LL))));
for(k_cg = (0LL), k_ch = (999LL); k_cg <= k_ch; k_cg += (1LL)){
for(k_cl = (0LL), k_cm = (999LL); k_cl <= k_cm; k_cl += (1LL)){
(k_co) = (((k_ao)->At(k_cg))->At(k_cl));
((k_cc)->At(k_co)) = (((k_cc)->At(k_co)) + (1LL));
}
}
for(k_cs = (1LL), k_ct = (k_am); k_cs <= k_ct; k_cs += (1LL)){
(k_cu)((((newArrayBin_<char16_t>(0,""))->Cat(k_cv<int64_t>(((k_cc)->At(k_cs)), (nullptr))))->Cat(newArrayBin_<char16_t>(1,"\x0A\x00"))));
}
}
// _finVars
static void k_i(){
...
type_(k_o) k_de;
type_(k_o) k_df;
(k_de) = (k_df = new_(k_o)(), k_p(k_df), k_df);
return as_<k_o>(classTable_, (k_de), 0);
}
// _toBin
static type_(Array_<uint8_t>) k_t(type_(k_o) k_dg /*me*/){
...
type_(k_o) k_dl;
type_(k_o) k_dm;
(k_dl) = (k_dm = new_(k_o)(), k_p(k_dm), k_dm);
return k_dl;
}
// toStr
static type_(Array_<char16_t>) k_v(type_(k_o) k_dn /*me*/){
...
uint64_t k_dx; // m
uint64_t k_dy; // r
(k_dw) = (static_cast<uint64_t>(((k_dt) - (k_ds)) + (1LL)));
(k_dx) = ((0x0000000000000000ULL) - (((0x0000000000000000ULL) - (k_dw)) % (k_dw)));
(k_dy) = (0x0000000000000000ULL);
if((k_dx) == (0x0000000000000000ULL)){
(k_dy) = ((reinterpret_cast<k_eb>(classTable_[(k_dr)->Y + 9]))((k_dr)));
}
else{
do{
(k_dy) = ((reinterpret_cast<k_eb>(classTable_[(k_dr)->Y + 9]))((k_dr)));
...
return (static_cast<int64_t>((k_dy) % (k_dw))) + (k_ds);
}
// rndBit64
static uint64_t k_y(type_(k_n) k_ee /*me*/){
uint32_t k_ef; // t
(k_ef) = ((reinterpret_cast<k_eg>(classTable_[(k_ee)->Y + 12]))((k_ee)));
return k_eh<uint64_t>((k_ei<uint64_t>((static_cast<uint64_t>(k_ef)), (nullptr), (32LL))), (nullptr), (static_cast<uint64_t>((reinterpret_cast<k_eg>(classTable_[(k_ee)->Y + 12]))((k_ee)))));
}
// rndFloat
static double k_z(type_(k_n) k_ej /*me*/, double k_ek /*min*/, double k_el /*max*/){
return (((static_cast<double>((reinterpret_cast<k_eb>(classTable_[(k_ej)->Y + 9]))((k_ej)))) / (1.844674407370955e+19)) * ((k_el) - (k_ek))) + (k_ek);
}
// setSeed
static void k_aa(type_(k_n) k_eo /*me*/, uint32_t k_ep /*seed*/){
((k_eo)->k_eq) = (0x075BCD15U);
((k_eo)->k_er) = (0x159A55E5U);
((k_eo)->k_es) = ((0x1F123BB5U) * (k_ep));
((k_eo)->k_et) = ((0x05491333U) * (k_eu<uint32_t>(((k_eo)->k_es), (nullptr), (1LL))));
}
// xs128
static uint32_t k_ab(type_(k_n) k_ev /*me*/){
uint32_t k_ew; // t
(k_ew) = (k_ai<uint32_t>(((k_ev)->k_eq), (nullptr), (k_ei<uint32_t>(((k_ev)->k_eq), (nullptr), (11LL)))));
((k_ev)->k_eq) = ((k_ev)->k_er);
((k_ev)->k_er) = ((k_ev)->k_es);
((k_ev)->k_es) = ((k_ev)->k_et);
((k_ev)->k_et) = (k_ai<uint32_t>((k_ai<uint32_t>(((k_ev)->k_et), (nullptr), (k_eu<uint32_t>(((k_ev)->k_et), (nullptr), (19LL))))), (nullptr), (k_ai<uint32_t>((k_ew), (nullptr), (k_eu<uint32_t>((k_ew), (nullptr), (8LL)))))));
return (k_ev)->k_et;
}
// _dtor
static void k_ac(k_n* k_ex){
...
type_(k_n) k_ez;
type_(k_n) k_fa;
(k_ez) = (k_fa = new_(k_n)(), k_w(k_fa), k_fa);
((k_ez)->k_eq) = ((k_ey)->k_eq);
((k_ez)->k_er) = ((k_ey)->k_er);
((k_ez)->k_es) = ((k_ey)->k_es);
((k_ez)->k_et) = ((k_ey)->k_et);
return as_<k_n>(classTable_, (k_ez), 8);
}
// _toBin
static type_(Array_<uint8_t>) k_ae(type_(k_n) k_fb){
type_(Array_<uint8_t>) k_fc;
(k_fc) = (newArrays_<type_(Array_<uint8_t>)>((0LL)));
(k_fc) = ((k_fc)->Cat(toBin_<uint32_t>()(((k_fb)->k_eq))));
(k_fc) = ((k_fc)->Cat(toBin_<uint32_t>()(((k_fb)->k_er))));
(k_fc) = ((k_fc)->Cat(toBin_<uint32_t>()(((k_fb)->k_es))));
(k_fc) = ((k_fc)->Cat(toBin_<uint32_t>()(((k_fb)->k_et))));
return k_fc;
}
// _fromBin
static type_(k_n) k_af(type_(k_n) k_fh, type_(Array_<uint8_t>) k_fi, int64_t* k_fj){
type_(k_n) k_fk;
type_(k_n) k_fl;
(k_fk) = (k_fl = new_(k_n)(), k_w(k_fl), k_fl);
((k_fk)->k_eq) = (fromBin_<uint32_t>()((k_fi), (*k_fj)));
((k_fk)->k_er) = (fromBin_<uint32_t>()((k_fi), (*k_fj)));
((k_fk)->k_es) = (fromBin_<uint32_t>()((k_fi), (*k_fj)));
((k_fk)->k_et) = (fromBin_<uint32_t>()((k_fi), (*k_fj)));
return k_fk;
}
// makeRnd
static type_(k_n) k_ag(type_(k_n) k_fm /*me2*/, uint32_t k_fn /*seed*/){
(reinterpret_cast<k_fo>(classTable_[(k_fm)->Y + 11]))((k_fm), (k_fn));
return k_fm;
}
// _xor
template<typename T_>static T_ k_ai(T_ k_fp /*me_*/, type_(Array_<int64_t>) k_fq /*type*/, T_ k_fr /*n*/){
...
#if defined(_WIN32)
fputws(reinterpret_cast<const wchar_t*>(k_fz->B),stdout);
#else
std::u16string s_=k_fz->B;
...
// inputStr
static type_(Array_<char16_t>) k_ft(){
type_(Array_<char16_t>) k_gp; // buf
int64_t k_gq; // ptr
char16_t k_gu; // c
(k_gp) = (newArrays_<type_(Array_<char16_t>)>((1024LL)));
(k_gq) = (0LL);
while(true){
(k_gu) = ((k_gv)());
if((k_gu) == (u'\uFFFF')){
...
if((k_gq) == ((k_gp)->Len())){
(k_gp) = ((k_gp)->Cat(newArrays_<type_(Array_<char16_t>)>(((k_gp)->Len()))));
}
((k_gp)->At(k_gq)) = (k_gu);
(k_gq) = ((k_gq) + (1LL));
}
return k_hi<type_(Array_<char16_t>)>((k_gp), (nullptr), (0LL), (k_gq));
}
// _toInt
static int64_t k_fw(type_(Array_<char16_t>) k_hj /*me_*/, bool* k_hk /*success*/){
std::u16string u_=k_hj->B;
...
while(true){
(k_ho) = ((k_hp)());
for(k_ht = (0LL), k_hu = (((k_al)->Len()) - (1LL)); k_ht <= k_hu; k_ht += (1LL)){
if(((k_ho) == ((k_al)->At(k_ht))) || (((k_ho) == (u'\u000D')) && (((k_al)->At(k_ht)) == (u'\u000A')))){
return u'\u0000';
}
}
if((k_ho) == (u'\u000D')){
